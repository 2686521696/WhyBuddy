...
function runFullTurn(
  ...
  const { newState, plan } = orchestrateReasoningTurn(intake.preparedState, {
    turnId,
    userText,
  });
  ...
  plan.selected.forEach((sel: any, idx: number) => {
    ...
    const { updatedState, committed } = commitArtifact(
      ...
      inputs,
      "pilot-template"
    ); // sim-driven fullpath helper now feeds real builder content for report (aligns with production serialize rule); pilot baseline keeps demo paths relaxed while quality gate participates
...
describe("S1 · cold start first turn (new_goal single-door intake)", () => {
  it("classifies the first message as new_goal and only the first message", () => {
    ...
    const first = intakeMessage(fresh, {
      turnId: "S1-t1",
      userText: "做一个权限管理系统",
    });
    ...
    const second = intakeMessage(turn1.state, {
      turnId: "S1-t2",
      userText: "再补充一点",
    });
  ...
  it("grows the decision ledger with full {saw, chose, skipped, rationale} fields on the first turn", () => {
    ...
    const { newState } = orchestrateReasoningTurn(
      intakeMessage(fresh, { turnId: "S1-d1", userText: COMBO_TEXT })
        .preparedState,
      {
        turnId: "S1-d1",
        userText: COMBO_TEXT,
  ...
  it('every committed artifact passedGates includes "commit" and trustLevel ∈ {gated_pass, audited}', () => {
    const turn1 = runFullTurn(
      createInitialSessionState("做一个权限管理系统", "S1-trust"),
      "S1-trust1",
      COMBO_TEXT
    );
    const committed = turn1.state.artifacts.filter(a =>
    ...
    expect(turn1.state.artifacts.every(a => TRUSTED.has(a.trustLevel))).toBe(
      true
    );
  });
  ...
  it("parks at AWAIT with the round's turnId and the ledger entry count equals the capabilityRuns count", () => {
    ...
    const turn1 = runFullTurn(
      createInitialSessionState("做一个权限管理系统", "S1-await"),
      turnId,
      COMBO_TEXT
    );
    ...
    expect(getSessionLedger(turn1.state).length).toBe(
      turn1.state.capabilityRuns.length
    );
...
describe("S2 · standard convergence to clear (soul loop, first lap)", () => {
  ...
  it("P1 replayCoverage lines up item-by-item with the contract required capabilities", () => {
    ...
    const contractRequired = (state.coverageContract as any)
...
describe("S3 · GCOV hard-block + forced scheduling", () => {
  ...
  it("a converge turn hard-blocks (GCOV_BLOCKED): empty plan, partial AWAIT, goal stays needs_refinement", () => {
    ...
    expect(
      (newState.conversation || []).some((c: any) =>
        /\[GCOV\] blocked/.test(c.text)
      )
    ).toBe(true);
  ...
  it("the missing capability is force-scheduled on the next converge turn (through the budget gate)", () => {
    ...
    const budget = evaluateBudgetBeforeOrchestrate(blocked, {
      turnId: "S3-f1",
      userText: CONVERGE_TEXT,
    });
    ...
    const { plan } = orchestrateReasoningTurn(blocked, {
      turnId: "S3-f1",
      userText: CONVERGE_TEXT,
    });
...
describe("S4 · challenge -> cascade -> reconverge (soul loop second lap, C-1 / C-2)", () => {
  it("challenging risk cascades stale to the downstream report and marks graph nodes challenged", () => {
    const { state, reportId, riskId } =
      buildClearStateWithTrustedReport("S4-cascade");
    expect(state.goal.status).toBe("clear");
    ...
    const challengedNodes = (challenged.graph.nodes || []).filter(
      (n: any) => n.status === "challenged"
    );
  ...
  it("C-1: the next picks re-include BOTH risk.analyze and report.write after the challenge", () => {
    ...
    const caps = pickNextCapabilities(
      challenged,
      "请基于现有证据重新生成最终报告"
  ...
  it("reconverge reaches clear again with a fresh trusted report whose evidenceRefs are all non-stale (healthy) upstreams", () => {
    ...
    const { state, reportId } =
      buildClearStateWithTrustedReport("S4-reconverge");
    const challenged = invalidateForIntervention(state, {
  ...
  it("reconverging after challenging an UPSTREAM risk should produce a fresh trusted report (currently blocked by stale upstream grab in findInputsForCapability)", () => {
    const { state, riskId } =
      buildClearStateWithTrustedReport("S4-upstream-fail");
    const challenged = invalidateForIntervention(state, {
      targetArtifactId: riskId,
      intent: "challenge",
      text: "请重新评估风险并生成最终报告",
  ...
  it("two-and-a-half loops: challenging the 2nd clear still drives a working 3rd loop (stale set does not poison the new healthy set)", () => {
    // Loop 1: clear.
    const { state: clear1, reportId: report1 } =
      buildClearStateWithTrustedReport("S4-loops");
    ...
    const reconverged1 = driveConvergeTurn(
      challenged1,
      "S4-l2",
      "请基于现有证据重新生成最终报告"
    );
    const clear2 = orchestrateReasoningTurn(reconverged1, {
      turnId: "S4-l2cv",
      userText: CONVERGE_TEXT,
    }).newState;
    ...
    const freshReport2 = clear2.artifacts
      .filter(
        a =>
          a.kind === "report" &&
          TRUSTED.has(a.trustLevel) &&
          !(clear2.staleArtifactIds || []).includes(a.id)
      )
      .slice(-1)[0];
    ...
    const reconverged2 = driveConvergeTurn(
      challenged2,
      "S4-l3",
      "请基于现有证据重新生成最终报告"
    );
    const clear3 = orchestrateReasoningTurn(reconverged2, {
      turnId: "S4-l3cv",
      userText: CONVERGE_TEXT,
    }).newState;
  ...
  it("STRONG (monotonic stale set): a later unrelated challenge does not un-stale an earlier challenge's victims (no supersede/resolve between them)", () => {
    const { state, reportId } =
      buildClearStateWithTrustedReport("S4-monotonic");
