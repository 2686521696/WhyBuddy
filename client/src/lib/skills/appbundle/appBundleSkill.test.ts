...
describe("appBundleSkill - the gate", () => {
  ...
  it("warns instead of failing when upstream skill surfaces are not supplied yet", () => {
    ...
    expect(report.errors).toHaveLength(0);
    expect(
      report.warnings.some(w => w.code === "APPBUNDLE_ENTITY_UNRESOLVED")
    ).toBe(true);
    expect(
      report.warnings.some(w => w.code === "APPBUNDLE_ROLE_UNRESOLVED")
    ).toBe(true);
    expect(
      report.warnings.some(w => w.code === "APPBUNDLE_WORKFLOW_UNRESOLVED")
    ).toBe(true);
    expect(
      report.warnings.some(w => w.code === "APPBUNDLE_PAGE_UNRESOLVED")
    ).toBe(true);
  });
  ...
  it("catches a bundled entity that DataModel never defined", () => {
    ...
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_ENTITY")
    ).toBe(true);
  });
  ...
  it("catches a bundled role that RBAC never defined", () => {
    ...
    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_ROLE")
    ).toBe(true);
  });
  ...
  it("catches page bindings that point at missing pages or workflows", () => {
    ...
    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_PAGE")
    ).toBe(true);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_WORKFLOW")
    ).toBe(true);
  });
  ...
  it("catches duplicate menu entry ids and missing menu page targets", () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.errors.some(e => e.code === "APPBUNDLE_DUP_MENU_ID")).toBe(
      true
    );
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_PAGE")
  ...
  it("validates purchase app closure matrix including permissions and fields", () => {
    const report = appBundleSkill.validate(purchaseApprovalAppBundle, {
      external: purchaseFullSurface,
    });

    expect(report.ok).toBe(true);
    expect((purchaseApprovalAppBundle as any).permissionRefs).toContain(
      "purchase:finance_approve"
    );
    expect((purchaseApprovalAppBundle as any).fieldRefs).toContain(
  ...
  it("catches missing permission and field refs through the closure matrix", () => {
    ...
    (broken as any).permissionRefs = [
      ...((broken as any).permissionRefs ?? []),
      "purchase:ghost_permission",
    ];
    (broken as any).fieldRefs = [
      ...((broken as any).fieldRefs ?? []),
      "purchase_request.ghost_field",
    ];

    const report = appBundleSkill.validate(broken, {
      external: purchaseFullSurface,
    });
    ...
    expect(
      report.errors.some(
        e =>
          e.code === "APPBUNDLE_REF_MISSING_PERMISSION" &&
          e.path.includes("permissionRefs")
      )
    ).toBe(true);
    expect(
      report.errors.some(
        e =>
          e.code === "APPBUNDLE_REF_MISSING_FIELD" &&
          e.path.includes("fieldRefs")
      )
    ).toBe(true);
  });
});
...
describe("appBundleSkill - surface, projector, and cross-skill refs", () => {
  it("exposes application package refs for later materialization", () => {
    ...
    expect(surface.pageBinding).toContain(
      "page_leave_request->wf_leave_approval"
    );
  });
  ...
  it("derives an application-center diagram with menu and binding edges", () => {
    ...
    expect(
      projection.nodes.some(
        n => n.id === "app_app_leave_approval" && n.kind === "app"
      )
    ).toBe(true);
    expect(
      projection.edges.some(
        e =>
          e.from === "app_app_leave_approval" &&
          e.to === "menu_menu_leave_request"
      )
    ).toBe(true);
  ...
  it("declares refs to DataModel, RBAC, Workflow, and Page for the combined diagram", () => {
    ...
    expect(
      refs.some(
        r =>
          r.toSkill === "rbac" && r.toKind === "role" && r.toValue === "manager"
      )
    ).toBe(true);
    expect(
      refs.some(
...
describe("appBundleSkill - V2 version pins and runtime snapshot", () => {
  it("pins every assembled Skill surface plus AppBundle itself", () => {
    ...
    const pinnedSkills = [
      ...new Set(app.versionPins.map((pin: any) => pin.skillId)),
    ].sort();
    ...
    expect(app.versionPins.every((pin: any) => pin.version === "1.0.0")).toBe(
      true
    );
  ...
  it("keeps runtime snapshot refs pinned and separate from mutable design-time refs", () => {
    ...
    expect(app.runtimeSnapshot.pinnedRefs).toContain(
  ...
  it("fails validate when runtimeSnapshot omits a pinned child ref for assembled version (negative, proves closure requirement)", () => {
    ...
    broken.runtimeSnapshot!.pinnedRefs =
      broken.runtimeSnapshot!.pinnedRefs.filter(
        (r: string) => !r.includes("leave_request@1.0.0")
      );
    const report = appBundleSkill.validate(broken, { external: fullSurface });

    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_SNAPSHOT_INCOMPLETE")
    ).toBe(true);
    expect(
      report.errors.some(e =>
        e.message.includes("datamodel:leave_request@1.0.0")
      )
    ).toBe(true);
  });
});
...
describe("appBundleSkill - V2 publish gate", () => {
  it("blocks missing assembled refs with APPBUNDLE_PUBLISH_REF_MISSING", () => {
    ...
    const gate = validateAppBundlePublishGate(broken, {
      external: fullSurface,
    });

    expect(gate.publishable).toBe(false);
    expect(
      gate.blockers.some(b => b.code === "APPBUNDLE_PUBLISH_REF_MISSING")
    ).toBe(true);
  });

  it("blocks unpinned assembled surfaces with APPBUNDLE_VERSION_UNPINNED", () => {
    const broken = clone(leaveApprovalAppBundle);
    broken.versionPins = broken.versionPins?.filter(
      pin => !(pin.skillId === "page" && pin.ref === "page_leave_request")
    );
    ...
    expect(gate.publishable).toBe(false);
    expect(
      gate.blockers.some(b => b.code === "APPBUNDLE_VERSION_UNPINNED")
    ).toBe(true);
  });
  ...
  it("blocks unresolved cross-skill surfaces with APPBUNDLE_GHOST_REF", () => {
    const gate = validateAppBundlePublishGate(leaveApprovalAppBundle, {
      external: {
        ...
        workflow: fullSurface.workflow,
        page: fullSurface.page,
      },
    });

    expect(gate.publishable).toBe(false);
    expect(gate.blockers.some(b => b.code === "APPBUNDLE_GHOST_REF")).toBe(
      true
    );
  });
  ...
  it("blocks Page or Workflow PEP bypass errors with APPBUNDLE_PEP_BYPASS", () => {
    ...
    expect(pageReport.errors.some(e => e.code === "PAGE_PEP_BYPASS")).toBe(
      true
    );
    expect(gate.publishable).toBe(false);
    expect(gate.blockers.some(b => b.code === "APPBUNDLE_PEP_BYPASS")).toBe(
      true
    );
  });
  ...
  it("projects assembly root, closure gate, and runtime snapshot", () => {
    ...
    expect(
      projection.nodes.some(
        n => n.id === "gate_app_leave_approval" && n.kind === "publishGate"
      )
    ).toBe(true);
    expect(
      projection.nodes.some(
        n => n.id === "snap_app_leave_approval" && n.kind === "runtimeSnapshot"
      )
    ).toBe(true);
    expect(
      projection.edges.some(
    ...
    expect(
      projection.edges.some(
        e =>
          e.from === "gate_app_leave_approval" &&
          e.to === "snap_app_leave_approval" &&
          e.kind === "runtimeSnapshot"
      )
    ).toBe(true);
  });
  ...
  it("reports precise source path for top-level broken role ref (negative)", () => {
    ...
    expect(gate.publishable).toBe(false);
    const b = gate.blockers.find(
      bb =>
        bb.code === "APPBUNDLE_PUBLISH_REF_MISSING" &&
        bb.path.includes("roleRefs")
    );
  ...
  it("reports precise source path for menuEntries broken role ref (negative)", () => {
    ...
    expect(gate.publishable).toBe(false);
    expect(
      gate.blockers.some(bb => bb.path === "menuEntries[0].roleRefs[2]")
    ).toBe(true);
    expect(
      gate.unresolvedRefs?.some(
        u =>
  ...
  it("reports precise source path for broken page in menuEntries and pageBindings (negative)", () => {
    ...
    broken.menuEntries.push({
      id: "m2",
      label: "m2",
      pageRef: "ghost_menu_page",
      roleRefs: [],
    });
    broken.pageBindings.push({
      pageRef: "ghost_bind_page",
      workflowRef: "wf_leave_approval",
      mode: "view",
    });
    ...
    expect(gate.publishable).toBe(false);
    expect(gate.blockers.some(bb => bb.path === "menuEntries[1].pageRef")).toBe(
      true
    );
    expect(
      gate.blockers.some(bb => bb.path === "pageBindings[1].pageRef")
    ).toBe(true);
    expect(
      gate.unresolvedRefs?.some(
        u =>
          u.path === "menuEntries[1].pageRef" &&
          u.kind === "page" &&
          u.targetValue === "ghost_menu_page"
      )
    ).toBe(true);
    expect(
      gate.unresolvedRefs?.some(
        u => u.path === "pageBindings[1].pageRef" && u.kind === "page"
      )
    ).toBe(true);
  ...
  it("reports precise source path for broken workflowRef in pageBindings (negative)", () => {
    ...
    expect(gate.publishable).toBe(false);
    expect(
      gate.blockers.some(bb => bb.path === "pageBindings[0].workflowRef")
    ).toBe(true);
    expect(
      gate.unresolvedRefs?.some(
        u =>
  ...
  it("reports precise source path and per-skill for broken AIGC ref (negative)", () => {
    ...
    const gate = validateAppBundlePublishGate(broken, {
      external: purchaseAigcSurface,
    });

    expect(gate.publishable).toBe(false);
    const aigcB = gate.blockers.find(
      bb => bb.code === "APPBUNDLE_PUBLISH_REF_MISSING"
    );
  ...
  it("includes per-skill summaries and unresolvedRefs for missing pins (negative)", () => {
    ...
    expect(
      gate.blockers.some(
        b => b.code === "APPBUNDLE_VERSION_UNPINNED" && b.path.includes("rbac")
      )
    ).toBe(true);
    expect(
      gate.unresolvedRefs?.some(
        u => u.kind === "versionPin" && u.targetValue.includes("rbac")
      )
    ).toBe(true);
    expect(gate.perSkillSummaries?.rbac).toBeDefined();
  });
  ...
  it("blocks latest-style version pin ('latest') via gate with APPBUNDLE_VERSION_UNPINNED (negative)", () => {
    const broken = clone(leaveApprovalAppBundle);
    const pin = broken.versionPins?.find(
      p => p.skillId === "datamodel" && p.ref === "employee"
    );
  ...
  it("blocks wildcard and range pins ('*', '^1.0.0', '1.x') via gate with APPBUNDLE_VERSION_UNPINNED (negative)", () => {
    ...
    const p1 = pins.find(p => p.skillId === "rbac" && p.ref === "requester");
    if (p1) p1.version = "*";
    const p2 = pins.find(
      p => p.skillId === "workflow" && p.ref === "wf_purchase_approval"
    );
    if (p2) p2.version = "^1.0.0";
    const p3 = pins.find(
      p => p.skillId === "page" && p.ref === "page_purchase_request"
    );
    if (p3) p3.version = "1.x";
  ...
  it("passes publish gate when runtimeSnapshot exactly matches pinned child versions (positive gate case)", () => {
    ...
    expect(
      gate.blockers.some(
        b =>
          b.code === "APPBUNDLE_SNAPSHOT_INCOMPLETE" ||
          b.code === "APPBUNDLE_SNAPSHOT_REF_NOT_PINNED"
      )
    ).toBe(false);
  });
  ...
  it("blocks via gate when runtimeSnapshot does not cover all assembled pinned child versions (negative gate case)", () => {
    ...
    broken.runtimeSnapshot!.pinnedRefs =
      broken.runtimeSnapshot!.pinnedRefs.filter(
        (r: string) => !r.includes("aigc:budget_risk_summary@")
      );
    const gate = validateAppBundlePublishGate(broken, {
      external: purchaseAigcSurface,
    });
...
describe("appBundleSkill - AIGC assembly refs (114.10)", () => {
  it("assembles AIGC capability refs with version pins", () => {
    ...
    expect(report.ok).toBe(true);
    expect(purchaseApprovalAppBundle.aigcCapabilityRefs).toContain(
      "budget_risk_summary"
    );
    expect(
      purchaseApprovalAppBundle.versionPins?.some(
        pin => pin.skillId === "aigc" && pin.ref === "budget_risk_summary"
      )
    ).toBe(true);
    expect(purchaseApprovalAppBundle.runtimeSnapshot?.pinnedRefs).toContain(
  ...
  it("warns on unresolved AIGC surfaces and fails on ghost AIGC capability refs", () => {
    ...
    expect(
      unresolved.warnings.some(w => w.code === "APPBUNDLE_AIGC_UNRESOLVED")
    ).toBe(true);

    const broken = clone(purchaseApprovalAppBundle);
    broken.aigcCapabilityRefs = ["ghost_ai_capability"];
    const missing = appBundleSkill.validate(broken, {
      external: purchaseAigcSurface,
    });

    expect(missing.ok).toBe(false);
    expect(
      missing.errors.some(e => e.code === "APPBUNDLE_REF_MISSING_AIGC")
    ).toBe(true);
  });
  ...
  it("blocks missing AIGC version pins before publish", () => {
    ...
    expect(
      gate.blockers.some(
        blocker => blocker.code === "APPBUNDLE_VERSION_UNPINNED"
      )
    ).toBe(true);
  });

  it("pins the purchase approval workflow version for AppBundle publish against immutable wf definition (positive)", () => {
    expect(purchaseApprovalAppBundle.workflowRefs).toContain(
      "wf_purchase_approval"
    );
    expect(
      purchaseApprovalAppBundle.versionPins?.some(
        pin =>
          pin.skillId === "workflow" &&
          pin.ref === "wf_purchase_approval" &&
          pin.version === "1.0.0"
      )
    ).toBe(true);
    expect(purchaseApprovalAppBundle.runtimeSnapshot?.pinnedRefs).toContain(
  ...
  it("blocks missing purchase approval workflow version pin before publish (negative gate case)", () => {
    const broken = clone(purchaseApprovalAppBundle);
    broken.versionPins = broken.versionPins?.filter(
      pin => !(pin.skillId === "workflow" && pin.ref === "wf_purchase_approval")
    );
  ...
  it("purchase approval pages resolve to pinned versions (positive)", () => {
    ...
    expect(purchaseApprovalPage.snapshotRefs).toContain(
    ...
    expect((surf as any).snapshotRefs).toContain(
      "page:page_purchase_request@1.0.0"
    );
  });
  ...
  it("blocks missing purchase approval page version pin before publish (negative gate case)", () => {
    ...
    broken.versionPins = (broken.versionPins || []).filter(
      (pin: any) =>
        !(pin.skillId === "page" && pin.ref === "page_purchase_request")
    );
    ...
    const hasUnpinned = gate.blockers.some(
      (blocker: any) => blocker.code === "APPBUNDLE_VERSION_UNPINNED"
    );
...
describe("appBundleSkill - V2 release artifact metadata (115.50.06)", () => {
  it("carries release artifact with traceId and publish gate evidence (positive)", () => {
    ...
    expect(app.releaseArtifact.publishGateEvidence.evidenceSummary).toContain(
      "115.50"
    );
  });
  ...
  it("fails validate when release artifact is missing traceId (negative)", () => {
    ...
    expect(
      report.errors.some(
        e => e.code === "APPBUNDLE_RELEASE_ARTIFACT_MISSING_TRACE"
      )
    ).toBe(true);
  });
  ...
  it("fails validate when release artifact missing publishGateEvidence (negative)", () => {
    ...
    expect(
      report.errors.some(
        e => e.code === "APPBUNDLE_RELEASE_ARTIFACT_MISSING_GATE_EVIDENCE"
      )
    ).toBe(true);
  });
  ...
  it("blocks via publish gate when release artifact appId mismatches (negative gate case)", () => {
    ...
    expect(
      gate.blockers.some(
        b => b.code === "APPBUNDLE_RELEASE_ARTIFACT_APP_MISMATCH"
      )
    ).toBe(true);
  });
});
...
describe("appBundleSkill - V2 rollback targets exist and immutable (115.50.06)", () => {
  ...
  it("passes validate and publish gate with valid prior immutable rollback target (positive gate case)", () => {
    ...
    expect(gate.publishable).toBe(true);
    expect(gate.blockers.some(b => b.code && b.code.includes("ROLLBACK"))).toBe(
      false
    );
  });
  ...
  it("blocks rollback target that does not exist (negative)", () => {
    ...
    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_ROLLBACK_TARGET_NOT_EXISTS")
    ).toBe(true);
  });
  ...
  it("blocks rollback target that is not immutable (negative)", () => {
    ...
    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_ROLLBACK_TARGET_MUTABLE")
    ).toBe(true);
  });
  ...
  it("blocks rollback target with movable version (negative)", () => {
    ...
    expect(report.ok).toBe(false);
    expect(
      report.errors.some(e => e.code === "APPBUNDLE_ROLLBACK_TARGET_MOVABLE")
    ).toBe(true);
  });
  ...
  it("blocks rollback target via gate when not prior version (negative gate case)", () => {
    ...
    expect(gate.publishable).toBe(false);
    expect(
      gate.blockers.some(b => b.code === "APPBUNDLE_ROLLBACK_TARGET_NOT_PRIOR")
    ).toBe(true);
  });
});
...
describe("appBundleSkill - runtime closure (117)", () => {
  it("exposes required runtime symbols", () => {
    ...
    expect(APPBUNDLE_RUNTIME_CLOSURE_BLOCKED).toBe(
      "APPBUNDLE_RUNTIME_CLOSURE_BLOCKED"
    );
    expect(runtimeClosure).toBeDefined();
    expect(typeof runtimeClosure.evaluateAppBundleRuntimeClosure).toBe(
      "function"
    );
    expect(typeof classifyAppBundleRuntimeClosureFinding).toBe("function");
    expect(typeof runtimeClosure.classifyAppBundleRuntimeClosureFinding).toBe(
      "function"
    );
    expect(runtimeClosure.APPBUNDLE_CLOSURE_TIERS).toEqual([
      "hard_blocker",
      "warning",
      "info",
    ]);
    expect(APPBUNDLE_CLOSURE_TIERS).toEqual([
      "hard_blocker",
      "warning",
      "info",
    ]);
  ...
  it("classifies AppBundle runtime closure findings into hard_blocker / warning / info tiers (deterministic mapping, positive + fail-closed)", () => {
    ...
    expect(classifyAppBundleRuntimeClosureFinding(evidenceFinding)).toBe(
      "info"
    );
    expect(
      runtimeClosure.classifyAppBundleRuntimeClosureFinding(evidenceFinding)
    ).toBe("info");
    ...
    const blockerFinding = {
      ...
      path: "aigc",
      message: "Missing AIGC...",
    };
    expect(classifyAppBundleRuntimeClosureFinding(blockerFinding)).toBe(
      "hard_blocker"
    );

    // other warning -> warning tier
    const otherWarning = {
      code: "APPBUNDLE_OTHER_WARN",
      severity: "warning" as const,
      path: "x",
      message: "w",
    };
    expect(classifyAppBundleRuntimeClosureFinding(otherWarning)).toBe(
      "warning"
    );
    ...
    expect(classifyAppBundleRuntimeClosureFinding(plainError)).toBe(
      "hard_blocker"
    );
    ...
    expect(
      posReport.classifiedFindings?.some(
        f =>
          f.tier === "info" && f.code === "APPBUNDLE_RUNTIME_EVIDENCE_PRESENT"
      )
    ).toBe(true);
    ...
    expect(
      (negReport.findingsByTier?.hard_blocker ?? []).length
    ).toBeGreaterThan(0);
    expect(
      negReport.classifiedFindings?.some(
        f =>
          f.tier === "hard_blocker" &&
          f.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED
      )
    ).toBe(true);
    // also check Classified shape (augmented Finding) using imported typed schema
    const firstClassified: ClassifiedAppBundleClosureFinding =
      negReport.classifiedFindings![0];
  ...
  it("keeps one canonical positive sample closed across all six Skills", () => {
    ...
    expect(
      Object.values(report.perSkillEvidence).every(
        evidence => evidence.evidencePresent
      )
    ).toBe(true);
    expect(report.closureId).toBe(
      "appbundle:app_purchase_approval@1.0.0:runtime-closure"
  ...
  it("blocks one canonical negative sample while still traversing all six Skills", () => {
    ...
    expect(
      report.blockers.some(
        blocker =>
          blocker.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED &&
          blocker.path === "workflow"
      )
    ).toBe(true);
    expect(
      report.blockers.some(
    ...
    expect(
      Object.values(report.perSkillEvidence).filter(
        evidence => evidence.evidencePresent
      )
    ).toHaveLength(4);
    expect(report.closureId).toBe(
    ...
    expect(report.findingsByTier?.hard_blocker.length).toBeGreaterThanOrEqual(
      2
    );
  });
  ...
  it("passes positive runtime closure for leave approval (no AIGC required, Page + core evidence)", () => {
    ...
    expect(report.closureId).toBe(
      "appbundle:app_leave_approval@1.0.0:runtime-closure"
    );
    expect(report.findingsByTier?.hard_blocker ?? []).toHaveLength(0);
  });
  ...
  it("blocks runtime closure on missing Page evidence for leave approval (fail-closed negative per-skill)", () => {
    ...
    expect(
      report.blockers.some(
        b =>
          b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED &&
          b.message.includes("page")
      )
    ).toBe(true);
    ...
    expect(
      classifyAppBundleRuntimeClosureFinding(
        report.blockers.find(b => b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED)!
      )
    ).toBe("hard_blocker");
  });
  ...
  it("accepts RBAC PDP explain evidence for both allow (positive) and fail-closed/deny (negative) paths as closure evidence (119 objective)", () => {
    ...
    expect(allowEvidence.evidenceKey).toContain(
      `${RBAC_PDP_EXPLAIN_EVIDENCE}:allow`
    );
    expect(failClosedEvidence.evidenceKey).toContain(
      `${RBAC_PDP_EXPLAIN_EVIDENCE}:fail-closed`
    );
    expect(denyEvidence.evidenceKey).toContain(
      `${RBAC_PDP_EXPLAIN_EVIDENCE}:deny`
    );
  });
  ...
  it("blocks runtime closure on missing AIGC runtime evidence for purchase (negative fail-closed)", () => {
    ...
    expect(report.stableDigest).toMatch(/^[0-9a-f]{8}$/);
    expect(report.findingsByTier?.hard_blocker.length).toBeGreaterThan(0);
    expect(classifyAppBundleRuntimeClosureFinding(report.blockers[0])).toBe(
      "hard_blocker"
    );
  });
  ...
  it("blocks via runtime closure when runtimeSnapshot is missing (negative)", () => {
    ...
    expect(
      report.blockers.some(
        b =>
          b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED &&
          b.path.includes("runtimeSnapshot")
      )
    ).toBe(true);
  });

  it("reports per-skill evidence listing and does not weaken purchase gate compatibility", () => {
    const gate = validateAppBundlePublishGate(purchaseApprovalAppBundle, {
      external: purchaseFullSurface,
    });
    expect(gate.publishable).toBe(true);
    ...
    expect(report.blocked).toBe(false);
    expect(Object.keys(report.perSkillEvidence).length).toBeGreaterThanOrEqual(
      5
    );
  });
  ...
  it("attaches runtime closure summary to release artifact without mutating the original", () => {
    ...
    const attached = attachRuntimeClosureSummaryToReleaseArtifact(
      original,
      report
    );
    ...
    expect(attached.runtimeClosureSummary?.stableDigest).toBe(
      report.stableDigest
    );
    expect(
      attached.runtimeClosureSummary?.evidencePresentCount
    ).toBeGreaterThan(0);
  });
  ...
  it("attaches blocked runtime closure summary for fail-closed report", () => {
    ...
    const report = evaluateAppBundleRuntimeClosure(models);
    const attached = attachRuntimeClosureSummaryToReleaseArtifact(
      purchaseApprovalAppBundle.releaseArtifact!,
      report
    );
  ...
  it("attaches and validates closure evidence digest on publish manifest when present", () => {
    ...
    const invalidReport = appBundleSkill.validate({
      ...purchaseApprovalAppBundle,
      publishManifest: invalid,
    });
    expect(invalidReport.ok).toBe(false);
    expect(
      invalidReport.errors.some(
        e => e.code === "APPBUNDLE_PUBLISH_MANIFEST_ILLEGAL_CLOSURE_DIGEST"
      )
    ).toBe(true);
    ...
    expect(blockedManifest.closureEvidenceDigest).toBe(
      blockedReport.stableDigest
    );
    // Digest attached on manifest surface; validate still enforces format (no weakening).
    const blockedValidate = appBundleSkill.validate({
      ...purchaseApprovalAppBundle,
      publishManifest: blockedManifest,
    });
    // Note: model without aigc may have validation issues, but digest format check passes.
    expect(
      blockedManifest.closureEvidenceDigest &&
        /^[0-9a-f]{6,}$/i.test(blockedManifest.closureEvidenceDigest)
    ).toBe(true);
  });
  ...
  it("119 evaluate: pageBindings declared but supplied pageModel id mismatches -> fail-closed workflowPageTaskViewConsistency false + blocked", () => {
    ...
    expect(
      report.blockers.some(
        b => b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED && b.path === "page"
      )
    ).toBe(true);
  });
});
...
describe("appBundleSkill - runtime snapshot and rollback (117)", () => {
  ...
  it("planAppBundleRollback returns APPBUNDLE_ROLLBACK_UNPINNED when no pinned versions (negative/fail-closed)", () => {
    ...
    expect(planAppBundleRollback(current, unpinnedTarget)).toBe(
      APPBUNDLE_ROLLBACK_UNPINNED
    );

    const noPinsCurrent: any = {
      appId: "app_x",
      appVersion: "1.0.0",
      refMode: "pinned",
      pinnedRefs: [],
    };
    expect(planAppBundleRollback(noPinsCurrent, current)).toBe(
      APPBUNDLE_ROLLBACK_UNPINNED
    );

    const bad: any = { appId: "app_x", appVersion: "1.0.0" }; // missing refMode/pins
    expect(planAppBundleRollback(bad, current)).toBe(
      APPBUNDLE_ROLLBACK_UNPINNED
    );
  });
  ...
  it("create/plan preserve purchase and leave approval compatibility (positive compat)", () => {
    ...
    const lTarget: AppBundleRuntimeSnapshot = {
      ...lSnap,
      appVersion: "0.9.0",
      pinnedRefs: lSnap.pinnedRefs.map(r => r.replace("@1.0.0", "@0.9.0")),
    };
...
describe("appBundleSkill - compare rollback target snapshots by runtime closure hash (119)", () => {
  it("compares rollback target snapshots by closure hash (positive: hash match yields no changed closure refs)", () => {
    ...
    const cmp = compareAppBundleRollbackTargetSnapshotsByClosureHash(
      current,
      targetSame
    );
  ...
  it("returns APPBUNDLE_ROLLBACK_UNPINNED for rollback target compare when pins or closureHash absent (fail-closed negative)", () => {
    ...
    ).toBe(APPBUNDLE_ROLLBACK_UNPINNED);

    const emptyPinsTarget: any = {
      appId: current.appId,
      appVersion: "0.9.0",
      refMode: "pinned",
      pinnedRefs: [],
    ...
    expect(
      compareAppBundleRollbackTargetSnapshotsByClosureHash(badCurrent, current)
    ).toBe(APPBUNDLE_ROLLBACK_UNPINNED);
  });

  it("exposes compare helper on direct import and keeps deterministic for identical rollback targets", () => {
    expect(typeof compareAppBundleRollbackTargetSnapshotsByClosureHash).toBe(
      "function"
    );
    ...
    if (
      cmp1 !== APPBUNDLE_ROLLBACK_UNPINNED &&
      cmp2 !== APPBUNDLE_ROLLBACK_UNPINNED
    ) {
      expect(
        (cmp1 as AppBundleRollbackClosureComparison).changedClosureRefs
      ).toEqual(
        (cmp2 as AppBundleRollbackClosureComparison).changedClosureRefs
      );
      expect(
        (cmp1 as AppBundleRollbackClosureComparison).closureHashMatch
      ).toBe((cmp2 as AppBundleRollbackClosureComparison).closureHashMatch);
    }
  });
});
...
describe("appBundleSkill - 120 rollback closure diff evidence between current/target publish artifacts", () => {
  it("compares publish artifacts by stableDigest (positive closed path: digest match yields no changed refs)", () => {
    const closed = {
      ...
      perSkillEvidence: {
        datamodel: {
          ...
          digest: "deadbeef120",
          evidenceRef: "evidence:datamodel:closed-120",
        },
    ...
    const diffEv = comparePublishArtifactsForRollbackClosureDiff(
      closed,
      sameTarget
    );
  ...
  it("compares publish artifacts and exposes changed per-skill refs (positive closed: mismatch path)", () => {
    const current = {
      ...
      perSkillEvidence: {
        datamodel: { digest: "deadbeef120" },
        appbundle: { digest: "deadbeef120" },
      },
    };
    const target = {
      appId: "app_purchase_approval",
      appVersion: "0.9.0",
      runtimeClosureSummary: {
        stableDigest: "c0ffee99",
        evidencePresentCount: 5,
      },
      perSkillEvidence: {
        datamodel: { digest: "olddigest" },
        appbundle: { digest: "c0ffee99" },
      },
    };
    const diffEv = comparePublishArtifactsForRollbackClosureDiff(
      current,
  ...
  it("returns sentinel for missing digest or absent artifacts (fail-closed / degraded negative path)", () => {
    const closed: any = {
      appId: "x",
      appVersion: "1.0.0",
      runtimeClosureSummary: { stableDigest: "deadbeef120" },
    };
    const noDigest: any = {
      appId: "x",
      appVersion: "0.9.0",
      runtimeClosureSummary: { blocked: true },
    };
    expect(
      comparePublishArtifactsForRollbackClosureDiff(closed, noDigest)
    ).toBe(APPBUNDLE_ROLLBACK_UNPINNED);
    expect(comparePublishArtifactsForRollbackClosureDiff(null, closed)).toBe(
      APPBUNDLE_ROLLBACK_UNPINNED
    );
    expect(comparePublishArtifactsForRollbackClosureDiff(closed, {})).toBe(
      APPBUNDLE_ROLLBACK_UNPINNED
    );
    // also direct publishClosure shape (projection)
    const pcCurrent = {
      stableDigest: "d1",
      perSkillEvidence: { appbundle: { digest: "d1" } },
    };
    const pcTarget = {
      stableDigest: "d2",
      perSkillEvidence: { appbundle: { digest: "d2" } },
    };
    const d2 = comparePublishArtifactsForRollbackClosureDiff(
      pcCurrent,
      pcTarget
    );
    expect(d2).not.toBe(APPBUNDLE_ROLLBACK_UNPINNED);
    expect((d2 as AppBundleRollbackClosureDiffEvidence).digestMatch).toBe(
      false
    );
  });

  it("exposes the compare helper (deterministic, importable)", () => {
    expect(typeof comparePublishArtifactsForRollbackClosureDiff).toBe(
      "function"
    );
    ...
    if (
      r1 !== APPBUNDLE_ROLLBACK_UNPINNED &&
      r2 !== APPBUNDLE_ROLLBACK_UNPINNED
    ) {
      expect((r1 as AppBundleRollbackClosureDiffEvidence).digestMatch).toBe(
        (r2 as AppBundleRollbackClosureDiffEvidence).digestMatch
      );
    }
  });
});
...
describe("appBundleSkill - 119 version pin vs runtime snapshot mismatch (fail-closed negative)", () => {
  it("validateAppBundleVersionPinVsRuntimeSnapshot returns matched=true and no blockers when pins exactly cover snapshot (positive)", () => {
    const res = validateAppBundleVersionPinVsRuntimeSnapshot(
      purchaseApprovalAppBundle
    );
    expect(res.matched).toBe(true);
    expect(res.blockers).toHaveLength(0);
    const res2 = validateAppBundleVersionPinVsRuntimeSnapshot(
      leaveApprovalAppBundle
    );
  ...
  it("validateAppBundleVersionPinVsRuntimeSnapshot blocks on pin missing from snapshot (negative fail-closed)", () => {
    ...
    broken.runtimeSnapshot!.pinnedRefs =
      broken.runtimeSnapshot!.pinnedRefs.filter(
        (r: string) => !r.includes("budget_risk_summary@1.0.0")
      );
    ...
    expect(
      res.blockers.some(
        b =>
          b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED &&
          b.message.includes("mismatch")
      )
    ).toBe(true);
    expect(
      res.blockers.some(b => b.message.includes("budget_risk_summary"))
    ).toBe(true);
  });
  ...
  it("validateAppBundleVersionPinVsRuntimeSnapshot blocks on snapshot ref with no pin (negative fail-closed)", () => {
    ...
    expect(
      res.blockers.some(
        b =>
          b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED &&
          /mismatch|no corresponding version pin/.test(b.message)
      )
    ).toBe(true);
  });
  ...
  it("evaluateAppBundleRuntimeClosure fails closed on versionPins vs runtimeSnapshot mismatch (negative)", () => {
    ...
    const pin = broken.versionPins!.find(
      p => p.skillId === "aigc" && p.ref === "budget_risk_summary"
    )!;
    const orig = { ...pin };
  ...
  it("runtimeClosure surface exposes validateAppBundleVersionPinVsRuntimeSnapshot (119)", () => {
    expect(
      typeof runtimeClosure.validateAppBundleVersionPinVsRuntimeSnapshot
    ).toBe("function");
    const r = runtimeClosure.validateAppBundleVersionPinVsRuntimeSnapshot!(
      purchaseApprovalAppBundle
    );
...
describe("appBundleSkill - 118 cross-runtime evidence", () => {
  ...
  it("builds positive appbundle to aigc evidence without external side effects", () => {
    const sample = createAppBundleAigcPositivePathSample(
      purchaseApprovalAppBundle,
      purchaseAigcSurface.aigc
    );

    expect(sample.evidence.evidenceKey).toBe(
      APPBUNDLE_AIGC_POSITIVE_RUNTIME_PATH
    );
  ...
  it("exposes AIGC negative sample evidence fail-closed when policy or schema evidence absent (119)", () => {
    const sample = createAppBundleAigcNegativePathSample(
      purchaseApprovalAppBundle
    );
    ...
    expect(sample.evidence.reasonCode).toBe(
      "APPBUNDLE_AIGC_POLICY_SCHEMA_EVIDENCE_ABSENT"
    );
  });

  it("fails closed for appbundle to page when upstream page evidence is absent", () => {
    const sample = createAppBundlePageNegativePathSample(
      leaveApprovalAppBundle
    );
  ...
  it("normalizes per-target runtime context and preserves pinned refs", () => {
    ...
    expect(
      buildAppBundleCrossRuntimeEdges(purchaseApprovalAppBundle).map(
...
describe("appBundleSkill - 119 deterministic closed/blocked runtime closure report fixtures", () => {
  it("exports deterministic closed (positive) and blocked (fail-closed negative) AppBundle runtime closure report fixtures", () => {
    ...
    expect(
      closedAppBundleRuntimeClosureReport.findingsByTier?.hard_blocker
    ).toHaveLength(0);
    expect(
      closedAppBundleRuntimeClosureReport.perSkillEvidence.appbundle
        ?.evidencePresent
    ).toBe(true);

    expect(blockedAppBundleRuntimeClosureReport.blocked).toBe(true);
    expect(
      blockedAppBundleRuntimeClosureReport.blockers.some(
        b => b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED
      )
    ).toBe(true);
    expect(
      blockedAppBundleRuntimeClosureReport.findingsByTier?.hard_blocker.length
    ).toBeGreaterThan(0);
    expect(
      blockedAppBundleRuntimeClosureReport.classifiedFindings?.some(
        f => f.tier === "hard_blocker"
      )
    ).toBe(true);
    expect(
      blockedAppBundleRuntimeClosureReport.perSkillEvidence.aigc
        ?.evidencePresent
    ).toBe(false);
  });

  it("exposes the 119 fixtures via runtimeClosure namespace (stable public surface)", () => {
    expect(runtimeClosure.closedAppBundleRuntimeClosureReport).toBe(
      closedAppBundleRuntimeClosureReport
    );
    expect(runtimeClosure.blockedAppBundleRuntimeClosureReport).toBe(
      blockedAppBundleRuntimeClosureReport
    );
  });
  ...
  it("evaluate produces report compatible with blocked fixture shape (fail-closed negative)", () => {
    ...
    expect(report.blocked).toBe(true);
    expect(
      report.blockers.some(b => b.code === APPBUNDLE_RUNTIME_CLOSURE_BLOCKED)
    ).toBe(true);
    expect(classifyAppBundleRuntimeClosureFinding(report.blockers[0])).toBe(
      "hard_blocker"
    );
...
describe("appBundleSkill - 119 appbundle aggregate edge validation across all six surfaces", () => {
  ...
  it("validates aggregate edges across all six Skill runtime evidence surfaces (positive)", () => {
    ...
    expect(result.surfacesChecked).toEqual([
      "datamodel",
      "rbac",
      "workflow",
      "page",
    ...
    expect(
      result.perSurfaceValidation.workflow.positive ||
        result.perSurfaceValidation.page.positive
    ).toBe(true);
    expect(result.perSurfaceValidation.aigc.positive).toBe(true);
    expect(
      result.perSurfaceValidation.appbundle.positive ||
        result.perSurfaceValidation.appbundle.failClosedSampled
    ).toBe(true);
    expect(result.closureEvidencePresent).toBe(true);
    // symbol present
    expect(APPBUNDLE_AGGREGATE_EDGE_VALIDATION).toBe(
      "APPBUNDLE_AGGREGATE_EDGE_VALIDATION"
    );
  });
  ...
  it("exercises aggregate via runtimeClosure export and covers page negative sample path", () => {
    ...
    expect(
      viaExport.perSurfaceValidation.page.positive ||
        viaExport.perSurfaceValidation.page.failClosedSampled
    ).toBe(true);
...
const closureReportSkillLinkageMatrix = [
  ...
  },
  {
    ...
    buildModels: () => {
      const m = buildPurchaseModelsForMatrix();
      delete (m as any).aigc;
      return m;
    },
    expectBlocked: true,
  ...
  },
  {
    ...
    buildModels: () => {
      const m = buildPurchaseModelsForMatrix();
      (m as any).page = { id: "page_purchase_request" };
      return m;
    },
    expectBlocked: true,
    expectCode: APPBUNDLE_RUNTIME_CLOSURE_BLOCKED,
  },
  {
    ...
    buildModels: () => {
      const broken = clone(purchaseApprovalAppBundle);
      delete (broken as any).runtimeSnapshot;
      return { ...buildPurchaseModelsForMatrix(), appbundle: broken };
    },
    expectBlocked: true,
    expectCode: APPBUNDLE_RUNTIME_CLOSURE_BLOCKED,
  },
...
describe("focused vitest matrix: AppBundle closure/reports/Skill linkage (119 precheck)", () => {
  it.each(closureReportSkillLinkageMatrix)("$name", row => {
    ...
    if (row.expectBlocked) {
      expect(report.blockers.some((b: any) => b.code === row.expectCode)).toBe(
        true
      );
      expect(classifyAppBundleRuntimeClosureFinding(report.blockers[0])).toBe(
        "hard_blocker"
      ...
      if (row.expectSkills) {
        expect(report.runtimeClosure?.skillsChecked).toEqual(
          expect.arrayContaining(row.expectSkills)
        );
      }
  ...
  it("attach* report helpers link closure report to artifact/manifest (positive linkage)", () => {
    const report = evaluateAppBundleRuntimeClosure(
      buildPurchaseModelsForMatrix()
    );
    const attachedArtifact = attachRuntimeClosureSummaryToReleaseArtifact(
      purchaseApprovalAppBundle.releaseArtifact!,
      report
    );
    const attachedManifest = attachClosureEvidenceDigestToPublishManifest(
      purchaseApprovalAppBundle.publishManifest!,
      report.stableDigest
    );

    expect(attachedArtifact.runtimeClosureSummary?.blocked).toBe(false);
    expect(attachedArtifact.runtimeClosureSummary?.closureId).toBe(
      report.closureId
    );
