...
const eventSequenceArb = fc.array(renderModeEventArb, {
  minLength: 1,
  maxLength: 50,
});
...
describe("nextRenderMode (transition rules)", () => {
  describe("from ue-stream", () => {
    it("transitions to connecting on CONNECTION_LOST", () => {
      expect(nextRenderMode("ue-stream", { type: "CONNECTION_LOST" })).toBe(
        "connecting"
      );
    });

    it("transitions to error on CONNECTION_ERROR", () => {
      expect(nextRenderMode("ue-stream", { type: "CONNECTION_ERROR" })).toBe(
        "error"
      );
    });
    ...
    it("ignores irrelevant events", () => {
      expect(
        nextRenderMode("ue-stream", { type: "RECONNECT_SUCCESS" })
      ).toBeNull();
      expect(
        nextRenderMode("ue-stream", { type: "THREEJS_FAILED" })
      ).toBeNull();
  ...
  describe("from connecting", () => {
    it("transitions to ue-stream on RECONNECT_SUCCESS", () => {
      expect(nextRenderMode("connecting", { type: "RECONNECT_SUCCESS" })).toBe(
        "ue-stream"
      );
    });

    it("transitions to threejs on RECONNECT_FAILED", () => {
      expect(nextRenderMode("connecting", { type: "RECONNECT_FAILED" })).toBe(
        "threejs"
      );
    });

    it("transitions to error on CONNECTION_ERROR", () => {
      expect(nextRenderMode("connecting", { type: "CONNECTION_ERROR" })).toBe(
        "error"
      );
    });
    ...
    it("ignores irrelevant events", () => {
      expect(
        nextRenderMode("connecting", { type: "CONNECTION_LOST" })
      ).toBeNull();
      expect(
        nextRenderMode("connecting", { type: "THREEJS_FAILED" })
      ).toBeNull();
    });
  });
  ...
  describe("from threejs", () => {
    it("transitions to ue-stream on UE_AVAILABLE", () => {
      expect(nextRenderMode("threejs", { type: "UE_AVAILABLE" })).toBe(
        "ue-stream"
      );
    });

    it("transitions to prerender on THREEJS_FAILED", () => {
      expect(nextRenderMode("threejs", { type: "THREEJS_FAILED" })).toBe(
        "prerender"
      );
    });
    ...
    it("ignores irrelevant events", () => {
      ...
      expect(
        nextRenderMode("threejs", { type: "RECONNECT_SUCCESS" })
      ).toBeNull();
    });
  });
  ...
  describe("from prerender (terminal state)", () => {
    it("only allows MANUAL_RECONNECT", () => {
      expect(nextRenderMode("prerender", { type: "MANUAL_RECONNECT" })).toBe(
        "connecting"
      );
    });
    ...
    it("blocks all automatic transitions", () => {
      expect(
        nextRenderMode("prerender", { type: "CONNECTION_LOST" })
      ).toBeNull();
      expect(
        nextRenderMode("prerender", { type: "RECONNECT_SUCCESS" })
      ).toBeNull();
      expect(
        nextRenderMode("prerender", { type: "RECONNECT_FAILED" })
      ).toBeNull();
      expect(nextRenderMode("prerender", { type: "UE_AVAILABLE" })).toBeNull();
      expect(
        nextRenderMode("prerender", { type: "THREEJS_FAILED" })
      ).toBeNull();
      expect(
        nextRenderMode("prerender", { type: "CONNECTION_ERROR" })
      ).toBeNull();
    });
  });
  ...
  describe("from error", () => {
    it("allows MANUAL_RECONNECT", () => {
      expect(nextRenderMode("error", { type: "MANUAL_RECONNECT" })).toBe(
        "connecting"
      );
    });
...
describe("RenderModeMachine property-based tests", () => {
  ...
  it("always stays in a valid render mode after any sequence of events", () => {
    ...
    fc.assert(
      fc.property(renderModeArb, eventSequenceArb, (initialMode, events) => {
  ...
  it("nextRenderMode is deterministic", () => {
    fc.assert(
      fc.property(renderModeArb, renderModeEventArb, (mode, event) => {
