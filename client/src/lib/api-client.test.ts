...
describe("fetchJsonSafe", () => {
  ...
  it("normalizes 502 python LLM failure as degraded with python source (105)", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ error: "python LLM failed for evidence.search" }),
        {
          status: 502,
          headers: { "Content-Type": "application/json" },
        }
    ...
    const { fetchJsonSafe, isPythonBackendFailure } = await import(
      "./api-client"
    );
    const result = await fetchJsonSafe<any>(
  ...
  it("normalizes timeout/504 and legacy fallback indicators (105)", async () => {
    ...
    const { fetchJsonSafe, isPythonBackendFailure, getLegacyFallbackReason } =
...
describe("resolveApiTarget (backend-python-total-cutover-105)", () => {
  ...
  it("PYTHON_API_TARGET overrides python target for owned routes", async () => {
    ...
    expect(resolveApiTarget("/api/agent-loop", ov)).toBe(
      "http://py-local:9705"
    );
    expect(resolveApiTarget("/api/sliderule", ov)).toBe("http://py-local:9705");
    expect(resolveApiTarget("/api/blueprint/spec-documents/z", ov)).toBe(
      "http://py-local:9705"
    );
  ...
  it("unlisted /api always resolves to Node (explicit retained compatibility shell) even if python-first enabled", async () => {
    const { resolveApiTarget } = await import("../../../api-target");
    expect(resolveApiTarget("/api", { VITE_PYTHON_FIRST_API: "true" })).toBe(
      NODE
    );
    expect(
      resolveApiTarget("/api/foo", { PYTHON_API_TARGET: "http://x:9" })
    ).toBe(NODE);
