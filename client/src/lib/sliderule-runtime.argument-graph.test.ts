...
describe("argument graph P0: proposition root + structural depends_on", () => {
  it("createInitialSessionState inserts a single proposition root (G-ROOT-1)", () => {
    ...
    const questionNodes = (s.graph.nodes || []).filter(
      n => n.type === "question"
    );
  ...
  it("intakeMessage keeps the root idempotent on refine turns", () => {
    ...
    expect(
      (preparedState.graph.nodes || []).filter(n => n.type === "question")
    ).toHaveLength(1);
  });
  ...
  it("orchestrate adds new nodes from plan without intake pre-scaffold", () => {
    ...
    const { newState, newGraphNodes, plan } = orchestrateReasoningTurn(
      preparedState,
      context
    );
    expect(newGraphNodes.length).toBe(plan.selected.length);
    expect((newState.graph.nodes || []).length).toBe(
      beforeCount + newGraphNodes.length
    );
  });
  ...
  it("orchestrateReasoningTurn attaches depends_on to scaffold slots or root (G-ROOT-2)", () => {
    ...
    const { newState, newGraphNodes } = orchestrateReasoningTurn(
      preparedState,
      context
    );
    ...
    for (const node of newGraphNodes) {
      ...
      expect((node as { derivedFrom?: string[] }).derivedFrom?.[0]).toBe(
        edge?.source
      );
    }
  });
  ...
  it("provenance label resolves parent node title (root or scaffold slot)", () => {
    const longGoal =
      "分析权限管理系统在 RBAC + 数据范围场景下的越权风险与审计缺口，并给出可落地的 MVP 建议与后续演进路径";
  ...
  it("challenge re-entry hangs new nodes under the challenged artifact node", () => {
    ...
    const capNode = (newState.graph.nodes || []).find(
      (n: any) => n.capabilityRunId === `${turnId}-run-0`
    );
    ...
    const { preparedState: challengedPrep, context: chCtx } = intakeMessage(
      newState,
      {
    ...
    const { newState: afterChallenge, newGraphNodes } =
      orchestrateReasoningTurn(challengedPrep, chCtx);
    expect(newGraphNodes.length).toBeGreaterThan(0);
