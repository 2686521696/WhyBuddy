...
describe("sliderule-runtime V5 closed loop (behavioral regression)", () => {
  ...
  it("combo input plans the full capability team and produces a gated_pass report with real upstream refs after sequential fresh-input commits", () => {
    ...
    plan.selected.forEach((sel, idx) => {
      ...
      const rawWithContent = {
        ...
        summary:
          cap === "evidence.search"
            ? "【来源: F1_Github_Source 取数】"
            : contentForThis.slice(0, 200),
    ...
    const { newState: afterPlan2, plan: plan2 } = orchestrateReasoningTurn(
      working,
      {
        turnId: turn2Id,
  ...
  it("challenge on risk cascades stale to report and makes verify fail", () => {
    ...
    working = {
      ...
      dependencyGraph: [
        {
          fromArtifactId: riskId,
          toArtifactId: reportId,
          reason: "produced-by-report.write",
        },
      ],
      capabilityRuns: [],
      staleArtifactIds: [],
  ...
  it("force gate fail on upstream makes report untrusted and verify fail", () => {
    ...
    plan.selected.forEach((sel, idx) => {
      ...
      const isUpstream =
        sel.capabilityId.includes("risk") || sel.capabilityId.includes("argue");
    ...
    const finalReport = current.artifacts.find(
      a => a.producedBy?.capabilityId === "report.write"
    );
  ...
  it("rejects a report with zero upstream refs and keeps the attempt in runtime state", () => {
    ...
    const persistedReport = updatedState.artifacts.find(
      a => a.id === "report-without-upstream"
    );
  ...
  it("buildStructuredReport produces the fixed 9-section evidence-grade schema and pulls real fragments from upstreams", () => {
    ...
    const riskRaw = createRawArtifact(
      "r1",
      "risk.analyze",
      "安全",
      "risk",
      SEMANTIC_CONTENTS["risk.analyze"]
    );
    const counterRaw = createRawArtifact(
      "c1",
      "counter.argue",
      "挑刺",
      "risk",
      SEMANTIC_CONTENTS["counter.argue"]
    );
    const synthRaw = createRawArtifact(
      "s1",
      "synthesis.merge",
      "综合",
      "synthesis",
      SEMANTIC_CONTENTS["synthesis.merge"]
    );

    let after = commitArtifact(
      s,
      riskRaw as any,
      "t-run-r",
      false,
    ...
    after = commitArtifact(after, counterRaw as any, "t-run-c", false, [
      riskRaw.id,
    ]).updatedState;
    after = commitArtifact(after, synthRaw as any, "t-run-s", false, [
      riskRaw.id,
      counterRaw.id,
    ]).updatedState;
    ...
    const built = buildStructuredReport({
      state: after,
      inputArtifactIds: reportInputIds,
      roleId: "综合",
    });
  ...
  it("intake new_goal sets goal.text and proposition root from the first user message", () => {
    ...
    expect(preparedState.goal.text).toBe(
  ...
  it("intake classifies new_goal only on empty state, refine otherwise", () => {
    ...
    const { controlSignal: s1 } = intakeMessage(empty, {
      turnId: "t1",
      userText: "first",
    });
    ...
    const nonEmpty = {
      ...
      conversation: [{ id: "c1", role: "user", text: "x" }],
    };
    const { controlSignal: s2 } = intakeMessage(nonEmpty as any, {
      turnId: "t2",
      userText: "second",
    });
  ...
  it("second ordinary message via intake appends conversation and does NOT reset prior artifacts (no hidden restart)", () => {
    ...
    const { updatedState: after1 } = commitArtifact(
      w,
      createRawArtifact("art1", "risk.analyze", "安全", "risk"),
      "turn-1-run-0",
      false,
      []
    );
  ...
  it("after a turn (orchestrate + commits) we can markAwaiting and the phase is observable; next intake sees prior state", () => {
    ...
    let afterOrch = orchestrateReasoningTurn(preparedState, {
      turnId: "ta-1",
      userText: "做个报告",
    }).newState;
    ...
    const { updatedState: committed } = commitArtifact(
      afterOrch,
      createRawArtifact("r1", "report.write", "综合", "report"),
      "ta-1-run-9",
      false,
      []
    );
    ...
    const next = intakeMessage(awaiting, {
      turnId: "ta-2",
      userText: "再来一条",
    });
  ...
  it("createInitial with different sessionId produces isolated states (artifacts do not cross)", () => {
    ...
    const { updatedState: s1After } = commitArtifact(
      s1,
      createRawArtifact("a-alpha", "risk.analyze", "安全", "risk"),
      "r-a",
      false,
      []
    );
  ...
  it("K3.1 integration: thin report via commitArtifact -> quality failed + untrusted (production baseline)", () => {
    ...
    const thinReport = createRawArtifact(
      "thin-report",
      "report.write",
      "综合",
      "report",
      "结论：可行。"
    ); // very thin, < minContent, missing headings
    ...
    expect(committed).toBeNull();
    const landed = (updatedState.artifacts || []).find(
      (a: any) => a.id === "thin-report"
    );
  ...
  it("marks only the graph node for the challenged run when the same turn has duplicate capability nodes", () => {
    ...
    const working = {
      ...createInitialSessionState(
        "duplicate capability run binding",
        "sess-graph-binding"
      ),
      graph: {
        id: "graph-binding",
    ...
    const node0 = invalidated.graph.nodes.find(
      node => node.id === "turn-bind-node-0"
    );
    const node1 = invalidated.graph.nodes.find(
      node => node.id === "turn-bind-node-1"
    );
  ...
  it("loads or creates sessions from an in-memory store and preserves saved state by sessionId", async () => {
    ...
    const loadedAgain = await loadOrCreateSessionState(
      "store-alpha",
      "新目标不应覆盖已有会话"
    );
    expect(loadedAgain.sessionId).toBe("store-alpha");
    expect(loadedAgain.goal.text).toBe("权限系统");
    expect(loadedAgain.artifacts.map(artifact => artifact.id)).toEqual([
      "store-risk",
    ]);
  ...
  it("listSessions + deleteSession work on the store and are isolated", async () => {
    ...
    const listed =
      listedRaw && typeof (listedRaw as any).then === "function"
        ? await listedRaw
        : listedRaw;
    expect((listed as any[]).length).toBeGreaterThanOrEqual(2);
    expect((listed as any[]).some((x: any) => x.sessionId === "list-s1")).toBe(
      true
    );
    ...
    const listedAfter =
      listedAfterRaw && typeof (listedAfterRaw as any).then === "function"
        ? await listedAfterRaw
        : listedAfterRaw;
    expect(
      (listedAfter as any[]).some((x: any) => x.sessionId === "list-s1")
    ).toBe(false);
  });
  ...
  it("SlideRuleSessionStore is swappable and public load/save functions delegate to the injected impl", async () => {
    ...
    try {
      ...
      const s = await loadOrCreateSessionState(
        "swappable-sess",
        "可替换 store"
      );
      ...
      const reloaded = await loadOrCreateSessionState(
        "swappable-sess",
        "这个goal不应该覆盖"
      );
  ...
  it("loadOrCreate + save + deriveNodeStatus ensures graph node statuses are derived from artifacts + stale (single source of truth)", async () => {
    ...
    const n0 = (projected.graph.nodes || []).find(
      (n: any) => n.capabilityRunId === "derive-turn-run-0"
    );
    const n1 = (projected.graph.nodes || []).find(
      (n: any) => n.capabilityRunId === "derive-turn-run-1"
    );
  ...
  it("synthesis/report content simulates multi-role dissent when stale present (multi-agent divergence)", async () => {
    ...
    const { updatedState: after1 } = commitArtifact(
      s,
      createRawArtifact("d-risk", "risk.analyze", "安全", "risk"),
      "d-run-0",
      false,
      []
    );
    const { updatedState: after2 } = commitArtifact(
      after1,
      createRawArtifact("d-counter", "counter.argue", "挑刺", "risk"),
      "d-run-1",
      false,
      []
    );
    const staled = invalidateForIntervention(after2, {
      targetArtifactId: "d-risk",
      intent: "challenge",
      text: "异议",
    } as any);
    ...
    const hasStale = upstreams.some((u: any) =>
      staled.staleArtifactIds.includes(u.id)
    );
  ...
  it("simulateCapabilityExecution produces state-dependent richer content (prototype real-exec feel)", async () => {
    ...
    const { updatedState: c } = commitArtifact(
      s,
      createRawArtifact("sim-risk", "risk.analyze", "安全", "risk"),
      "sim-r0",
      false,
      [],
      "pilot-template"
    ); // pilot simulation seed, use pilot baseline
    ...
    const staled = invalidateForIntervention(c, {
      targetArtifactId: "sim-risk",
      intent: "challenge",
      text: "x",
    } as any);
  ...
  it("full loop with simulator + ledger + derived view stays consistent after save/load", async () => {
    ...
    let afterO = orchestrateReasoningTurn(preparedState, {
      turnId: "f1",
      userText: "生成报告",
    }).newState;
    ...
    const { updatedState: c } = commitArtifact(
      afterO,
      createRawArtifact("f-report", "report.write", "综合", "report"),
      reportRunId,
      false,
      []
    );
    ...
    const n = (reloaded.graph.nodes || []).find(
      (n: any) => n.capabilityRunId === reportRunId
    );
  ...
  it("picker is fully state-driven: openQuestions, ledger, gaps drive picks beyond keywords", async () => {
    ...
    expect(
      caps.some(c => /clarify|decompose|gap\.ask|question\.expand/.test(c))
    ).toBe(true);
  });
  ...
  it("challenge uses exact produced target from enriched state (binding resolution)", async () => {
    ...
    let o = orchestrateReasoningTurn(preparedState, {
      turnId: "c1",
      userText: "风险",
    }).newState;

    // Adopt the planned runId lookup (contract hygiene, consistent with full-loop tests).
    const riskNode = (o.graph.nodes || []).find(
      (n: any) => n.capabilityId === "risk.analyze"
    );
    const plannedRunId = riskNode ? riskNode.capabilityRunId : "c1-run-0";

    const { updatedState: c } = commitArtifact(
      o,
      createRawArtifact("c-risk", "risk.analyze", "安全", "risk"),
      plannedRunId,
      false,
      []
    );
    ...
    const enrichedNode = (enriched.graph.nodes || []).find(
      (n: any) => n.capabilityRunId === plannedRunId
    );
    expect(enrichedNode).toBeTruthy();
    ...
    const challengedNode = invalidated.graph.nodes.find(
      (node: any) => node.capabilityRunId === plannedRunId
    );
  ...
  it("simulator + ledger + derive in full cycle with dissent", async () => {
    ...
    const reportNode = (o.graph.nodes || []).find(
      (n: any) => n.capabilityId === "report.write"
    );
    const reportRunId = reportNode ? reportNode.capabilityRunId : "cy1-run-3";
    const { updatedState: c } = commitArtifact(
      o,
      createRawArtifact("cy-r", "report.write", "综合", "report"),
      reportRunId,
      false,
      []
    );
  ...
  it("CapabilityExecutor can be swapped via setCapabilityExecutor and affects committed artifact content (fake injection)", async () => {
    ...
    try {
      const fakeContent =
        "【FAKE EXECUTOR OUTPUT from test】turn=fake-t1 cap=risk.analyze";
      const fake = {
      ...
      const { newState: afterO } = orchestrateReasoningTurn(preparedState, {
        turnId: "f1",
        userText: "风险分析",
      });

      // Pick first capability from the plan (risk.analyze or whatever the picker chose)
      const first = (afterO as any).plan?.selected?.[0] || {
        capabilityId: "risk.analyze",
        roleId: "安全",
        inputArtifactIds: [],
      };
      ...
      const { updatedState: committedState } = commitArtifact(
        afterO,
        payload as any,
        runId,
        false,
      ...
      const art = (committedState.artifacts || []).find(
        (a: any) => a.id === "f1-art-0"
      );
  ...
  it("LlmCapabilityExecutor (via useLlmCapabilityExecutor) can be swapped, only affects risk.analyze + report.write, returns strict raw shape, and falls back on other caps", async () => {
    ...
    try {
      ...
      let s = await loadOrCreateSessionState(
        "llm-test",
        "llm executor pilot test"
      );
      const { preparedState } = intakeMessage(s, {
        turnId: "l1",
        userText: "分析风险并生成结构化报告",
      });
      const { newState: afterO } = orchestrateReasoningTurn(preparedState, {
        turnId: "l1",
        userText: "分析风险并生成结构化报告",
      ...
      const riskEntry = planSelected.find(
        (e: any) => e.capabilityId === "risk.analyze"
      ) || {
        capabilityId: "risk.analyze",
        roleId: "安全",
        inputArtifactIds: [],
      };
      const reportEntry = planSelected.find(
        (e: any) => e.capabilityId === "report.write"
      ) || {
        capabilityId: "report.write",
        roleId: "综合",
        inputArtifactIds: [],
      ...
      const otherCap = planSelected.find(
        (e: any) =>
          e.capabilityId !== "risk.analyze" && e.capabilityId !== "report.write"
      );
      if (otherCap) {
    ...
    try {
      ...
      // returns the exact raw contract shape, and that LlmCapabilityExecutor falls back on error.
      {
        ...
        try {
          ...
          const openaiProvider = createOpenAILlmCapabilityProvider({
            apiKey: "sk-test-123",
          });
          ...
          const openaiFailing = createOpenAILlmCapabilityProvider({
            apiKey: "sk-test-123",
          });
      ...
      // Uses the same seam + fallback contract. We spy fetch so the test stays hermetic.
      {
        ...
        try {
          ...
          fetchSpy.mockResolvedValueOnce(
            new Response(JSON.stringify({ error: "llm_execution_failed" }), {
              status: 500,
            }) as any
          );
  ...
  it("test helper routes (__clear / __reload) are disabled under production (production guard)", async () => {
    ...
    try {
      ...
      try {
        ...
        const getMissing = await fetch(
          `${base}/sessions/__guard-test-missing-id`
        );
        ...
        const delRes = await fetch(`${base}/sessions/__guard-test-del`, {
          method: "DELETE",
        });
        ...
        const clearRes = await fetch(`${base}/sessions/__clear`, {
          method: "POST",
        });
        expect(clearRes.status).toBe(404);

        const reloadRes = await fetch(`${base}/sessions/__reload`, {
          method: "POST",
        });
  ...
  it("PilotRealCapabilityExecutor (via usePilotRealExecutor) produces richer content for risk.analyze + report.write while preserving 9-section schema + fallback for other caps", async () => {
    ...
    try {
      ...
      const riskPayload = {
        ...
        provenance: "ai_generated" as const,
        producedBy: {
          capabilityRunId: riskRunId,
          capabilityId: riskEntry.capabilityId,
          roleId: riskEntry.roleId || "安全",
        },
        title: riskRes.title,
        summary: riskRes.summary,
        content: riskRes.content,
      };
      const { updatedState: withRisk } = commitArtifact(
        afterO,
        riskPayload as any,
        riskRunId,
        false,
        riskEntry.inputArtifactIds || []
      );
      const withGrounded = commitGroundedEvidence(
        withRisk,
        "pilot-ev-ground",
        "p1-run-ev"
      );
      ...
      expect(reportRes.content).toContain(
        "Pilot 验证：本报告由 PilotRealCapabilityExecutor 产生"
      );
      ...
      const reportPayload = {
        ...
        provenance: "ai_generated" as const,
        producedBy: {
          capabilityRunId: reportRunId,
          capabilityId: reportEntry.capabilityId,
          roleId: reportEntry.roleId || "综合",
        },
        title: reportRes.title,
        summary: reportRes.summary,
        content: reportRes.content,
      };
      const { updatedState: withReport } = commitArtifact(
        withGrounded,
        reportPayload as any,
        reportRunId,
        false,
        reportEntry.inputArtifactIds || []
      );

      // Verify a non-pilot capability still falls back to simulator style
      const synthEntry = planSelected.find((e: any) =>
        String(e.capabilityId).includes("synth")
      ) || {
      ...
      const finalReportArt = (withReport.artifacts || []).find(
        (a: any) => a.id === "pilot-report-art"
      );
  ...
  it("Budget Gate v1: high distinct turns -> parks at AWAIT partial, empty plan, auditable conv note", () => {
    ...
    const notes = (afterO.conversation || []).filter(
      (c: any) => c.id && c.id.includes("-budget")
    );
  ...
  it("Budget Gate v1: repeat capability exceeds maxRepeat -> blocks further ORCH, partial AWAIT", () => {
    ...
    expect(
      (afterO.conversation || []).some((c: any) =>
        (c.text || "").includes("maxRepeatPerCapability")
      )
    ).toBe(true);
  });
  ...
  it("Budget Gate v1 + recordCapabilityRunCost: evaluate uses current runs; record is safe no-op stub for v1", () => {
    ...
    const eval1 = evaluateBudgetBeforeOrchestrate(s, {
      turnId: "r1",
      userText: "x",
    });
  ...
  it("Decision Ledger v1: normal orchestrate appends a decision; chose matches plan.selected exactly", () => {
    ...
    expect(
      d.skipped.every((sk: any) => !d.chose.includes(sk.capabilityId))
    ).toBe(true);
  });
  ...
  it("Decision Ledger v1 + Budget block: records special blocked_by_budget decision (per decided policy)", () => {
    ...
    expect(
      last.skipped.some((sk: any) => sk.reason === "blocked_by_budget")
    ).toBe(true);
  });
  ...
  it("complex goal requires risk.analyze before report.write can be treated covered", () => {
    ...
    const hasReportInPlan = plan.selected.some(
      (p: any) => p.capabilityId === "report.write"
    );
    if (hasReportInPlan) {
  ...
  it("GCOV blocks premature report when required capability missing", () => {
    ...
    const synthArt = (s2.artifacts || []).find(
      (a: any) => a.kind === "synthesis"
    );
    if (synthArt) {
  ...
  it("GCOV passes when required capability runs are committed and trusted", () => {
    ...
    const riskArt = (sRisk.artifacts || []).find(
      (a: any) => a.producedBy?.capabilityId === "risk.analyze"
    );
    if (riskArt) {
    ...
    const critArt = (sCrit.artifacts || []).find(
      (a: any) => a.producedBy?.capabilityId === "critique.generate"
    );
    if (critArt) {
    ...
    const synArt = (sBoth.artifacts || []).find(
      (a: any) => a.kind === "synthesis"
    );
    if (synArt) {
  ...
  it("DLEDGER records coverage addresses", () => {
    ...
    if (hasCov) {
      expect(
        adds.some(
          (a: string) =>
            a.includes("risk.analyze") || a.includes("report.write")
        )
      ).toBe(true);
    } else {
  ...
  it("GCOV fail + insufficient afford blocks report.write from plan and parks AWAIT", () => {
    ...
    const hasGcovNote = (afterO.conversation || []).some((c: any) =>
      (c.text || "").includes("[GCOV] blocked")
    );
  ...
  it("FLOWB strips critique/rebuttal protocol nodes from formal report input", () => {
    ...
    const polluted = createRawArtifact(
      "polluted-up",
      "risk.analyze",
      "安全",
      "risk",
    ...
    const { updatedState: sUp } = commitArtifact(
      s,
      polluted,
      "f1-run-up",
      false,
    ...
    const { preparedState } = intakeMessage(sGrounded, {
      turnId: "f1",
      userText: "生成报告",
    });
    const { newState: afterO } = orchestrateReasoningTurn(preparedState, {
    ...
    const rawReport = createRawArtifact(
      "rpt",
      "report.write",
      "综合",
      "report",
    ...
    const { updatedState: afterCommit, committed } = commitArtifact(
      afterO,
      rawReport,
      reportRunId,
      false,
    ...
    expect(committed).toBeNull();
    const landed = (afterCommit.artifacts || []).find(
      (a: any) => a.id === "rpt"
    );
    expect(landed).toBeTruthy();
    ...
    expect(
      lastCheck.strippedProtocolNodes.some((n: string) =>
        /critique|rebuttal/i.test(n)
      )
    ).toBe(true);
  });
  ...
  it("FLOWB records boundary assertion in ledger", () => {
    ...
    const rawSyn = createRawArtifact(
      "syn",
      "synthesis.merge",
      "综合",
      "synthesis",
    ...
    const { updatedState: afterC } = commitArtifact(
      sUp,
      rawSyn,
      "f2-run2",
      false,
    ...
    expect(
      ledger.some(
        (c: any) =>
          ...
          (c.assertions || []).some((a: string) => /strip|protocol/i.test(a))
      )
    ).toBe(true);
  });
  ...
  it("FLOWB leaves ordinary evidence text unchanged", () => {
    ...
    const { updatedState: afterC, committed } = commitArtifact(
      s,
      clean,
      "f3-run",
      false,
  ...
  it("decision-level challenge marks target decision as challenged", () => {
    ...
    const { newState: afterO1 } = orchestrateReasoningTurn(s, {
      turnId: "d1",
      userText: "分析风险并报告",
    });
    ...
    const { preparedState } = intakeMessage(afterO1, {
      turnId: "d2",
      userText: "挑战这条调度决策",
      intervention: challengeInt,
    });
  ...
  it("decision challenge re-enters through intakeMessage single door", () => {
    ...
    const { newState: o1 } = orchestrateReasoningTurn(s, {
      turnId: "c1",
      userText: "生成报告",
    });
    ...
    const intakeRes = intakeMessage(o1, {
      turnId: "c2",
      userText: "挑战决策",
      intervention: intv,
    });
  ...
  it("decision challenge influences next DLEDGER rationale / selected reconsideration", () => {
    ...
    const { newState: o2, plan: plan2 } = orchestrateReasoningTurn(
      intakeRes.preparedState,
      intakeRes.context
    );
  ...
  it("cost ledger survives commit/run flow", () => {
    ...
    expect(ledger.some((c: any) => c.capabilityId === "risk.analyze")).toBe(
      true
    );
  });
  ...
  it("budget snapshot includes cost summary", () => {
    ...
    s = recordCapabilityRunCost(
      ...
      { tokens: 500, durationMs: 10, source: "estimated" }
    );
    ...
    expect(
      snap.perCapTokens && snap.perCapTokens["risk.analyze"]
    ).toBeGreaterThan(0);
  ...
  it("maxTokensPerSession blocks orchestrate when costLedger exceeds ceiling", () => {
    ...
    const res = evaluateBudgetBeforeOrchestrate(s, {
      turnId: "t1",
      userText: "继续",
    });
  ...
  it("authored CoverageContract creates frozen baseline + blocking gaps", () => {
    ...
    expect(
      gaps.some(
        (g: CoverageGap) =>
          g.kind === "missing_capability" &&
          g.requiredCapabilityId === "risk.analyze"
      )
    ).toBe(true);
  ...
  it("committing required capability resolves corresponding gap", () => {
    ...
    const gap = (after.coverageGaps || []).find(
      (g: CoverageGap) => g.requiredCapabilityId === "risk.analyze"
    );
  ...
  it("GCOV passes when blocking gaps are resolved or waived", () => {
    ...
    const { updatedState: sR } = commitArtifact(
      s,
      createRawArtifact("r4", "risk.analyze", "安全", "risk"),
      "r4",
      false,
      []
    );
    const { updatedState: sC } = commitArtifact(
      sR,
      createRawArtifact("c4", "critique.generate", "挑刺", "risk"),
      "r4c",
      false,
      []
    );
    const { updatedState: s2 } = commitArtifact(
      sC,
      createRawArtifact("sy4", "synthesis.merge", "综合", "synthesis"),
      "r4s",
      false,
      []
    );
    // waive the evidence gap if present
    const evGap = (s2.coverageGaps || []).find(
      (g: CoverageGap) => g.kind === "missing_evidence"
    );
    let s3 = s2;
    if (evGap) s3 = waiveCoverageGap(s2, evGap.id, "demo waive for test");
    s3 = commitGroundedEvidence(s3, "ev-cov7", "t4-run-ev");
    const { newState: afterO } = orchestrateReasoningTurn(s3, {
      turnId: "t4",
      userText: "现在报告",
    });
    const gate = afterO.coverageGate as any;
  ...
  it("Budget stops redundant converge when contract sufficient", () => {
    ...
    const satisfiedGaps = gaps.map((g: any) => ({
      ...g,
    ...
    const repArt = (sWithReport.artifacts || []).find(
      (a: any) => a.id === "rep-stop"
    );
    if (repArt) {
    ...
    const hasStopNote = (afterO.conversation || []).some((c: any) =>
      (c.text || "").includes("contract already sufficient")
    );
  ...
  it("Budget does not stop challenge/revise even when contract sufficient", () => {
    ...
    const hasContractStopNote = (afterO.conversation || []).some((c: any) =>
      (c.text || "").includes("contract already sufficient")
    );
  ...
  it("waived gap is reflected in coverage summary and sufficiency stop can proceed", () => {
    ...
    let sAfterWaive = s;
    const evGap = (sAfterWaive.coverageGaps || []).find(
      (g: any) => g.kind === "missing_evidence"
    );
    expect(evGap).toBeTruthy(); // ensure complex goal produced the evidence gap
    if (evGap) {
      sAfterWaive = waiveCoverageGap(
        sAfterWaive,
        evGap.id,
        "demo waive in test"
      );
    ...
    const anyWaived = (sAfterWaive.coverageGaps || []).some(
      (g: any) => g.status === "waived" && g.kind === "missing_evidence"
    );
    ...
    sAfterWaive = sRisk;
    const riskArt = (sAfterWaive.artifacts || []).find(
      (a: any) => a.id === "risk-waive"
    );
    if (riskArt) {
      (riskArt as any).trustLevel = "gated_pass";
      (riskArt as any).passedGates = ["commit"];
    ...
    for (const [cid, capId, role, kind] of [
      ...
      const { updatedState } = commitArtifact(
        ...
        createRawArtifact(
          cid,
          capId as V5CapabilityId,
          role,
          kind as Artifact["kind"]
        ),
        `t-waive-run-${cid}`,
        false,
        []
    ...
    sAfterWaive = commitGroundedEvidence(
      sAfterWaive,
      "ev-waive",
      "t-waive-run-ev"
    );
  ...
  it("fallback estimate still works when usage missing", () => {
    ...
    const costed2 = recordCapabilityRunCost(s, run, {
      tokens,
      source: "estimated",
    } as any);
  ...
  describe("R1 orchestrate proposedPlan (B3/B4/B9)", () => {
    ...
    it("challenge + stale + LLM proposal with report.write schedules reconvergence (B4)", () => {
      ...
      expect((preparedState.staleArtifactIds || []).includes("rep1")).toBe(
        true
      );
      ...
      expect(plan.selected.some(p => p.capabilityId === "report.write")).toBe(
        true
      );
      const dec = getDecisionLedger(newState).pop();
      expect(dec?.source).toBe("llm");
