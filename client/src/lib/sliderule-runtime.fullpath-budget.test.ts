...
describe("S5 · challenge a scheduling decision (DLEDGER reverse edge)", () => {
  ...
  it.fails(
    ...
    () => {
      ...
      const resolvedGap = (state.coverageGaps || []).find(
        g => g.status === "resolved"
      );
...
describe("S6 · budget over-limit -> park partial AWAIT", () => {
  ...
  it("orchestrate parks at AWAIT with an empty plan + blocked_by_budget marker when over the (default) budget", () => {
    ...
    expect(
      (newState.conversation || []).some(c =>
        /\[BUDGET\] exceeded/.test(c.text)
      )
    ).toBe(true);
    ...
    expect(
      (blocked?.skipped || []).every(s => s.reason === "blocked_by_budget")
    ).toBe(true);
...
describe("S8 · maxRepeat cutoff", () => {
  it("the (N+1)th repeat of the same capability is cut off by BUDGET with a maxRepeat reason", () => {
    ...
    expect(budget.snapshot.perCapRuns["risk.analyze"]).toBeGreaterThanOrEqual(
      2
    );
...
describe("S9 · contract early-stop (够了就停)", () => {
  it("a generic unrelated message after convergence stops via contract sufficiency without expanding capabilities", () => {
    ...
    const ledger = getDecisionLedger(newState);
    const stop = ledger.find(d =>
      /stopped_by_contract_sufficiency/.test(d.rationale)
    );
    expect(stop).toBeTruthy();
    expect(stop?.chose).toEqual([]);
    expect(
      (stop?.skipped || []).every(
        s => s.reason === "stopped_by_contract_sufficiency"
      )
    ).toBe(true);
...
describe("S10 · commit gate reject (verify-fail path)", () => {
  ...
  it("risk/counter are untrusted with a failed commit gate, and the report referencing them auto gate-fails", () => {
    const { state, riskRun, counterRun, reportRun } =
      buildVerifyFailSession("S10-reject");
    ...
    expect(
      riskRun.run.gateResults.some(
        g => g.gateId === "commit" && g.status === "failed"
      )
    ).toBe(true);
    expect(
      counterRun.run.gateResults.some(
        g => g.gateId === "commit" && g.status === "failed"
      )
    ).toBe(true);
    ...
    expect(
      reportRun.run.gateResults.some(
        g => g.gateId === "commit" && g.status === "failed"
      )
    ).toBe(true);
  });
  ...
  it("C-1 untrusted variant: the next picks should re-include risk/counter/report (untrusted-only kind must not count as present)", () => {
