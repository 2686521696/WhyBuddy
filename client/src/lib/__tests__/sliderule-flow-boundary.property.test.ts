...
const PROTOCOL_LINE = fc
  .constantFrom(...MARKERS)
  .map(m => `${m} injected noise`);
...
describe("Property 25: flow boundary strip idempotent zero residual", () => {
  it("output has no protocol markers and second pass is unchanged", () => {
    fc.assert(
      fc.property(
        fc.array(fc.string({ minLength: 0, maxLength: 60 }), {
          minLength: 1,
          maxLength: 8,
        }),
        ...
        (cleanLines, protocolLines, source) => {
          ...
          const first = sanitizeThroughFlowBoundary(mixed, {
            turnId: "t-p25",
            source,
          });
          const second = sanitizeThroughFlowBoundary(first.cleanedText, {
            turnId: "t-p25",
            source,
          });
          expect(second.cleanedText).toBe(first.cleanedText);
          for (const m of MARKERS) {
            expect(first.cleanedText.toLowerCase()).not.toContain(
              m.toLowerCase()
            );
          }
...
describe("Property 26: flow boundary ledger entry", () => {
  it("each sanitize adds exactly one ledger check with matching stripped count", () => {
    fc.assert(
      fc.property(
        fc.array(PROTOCOL_LINE, { minLength: 1, maxLength: 4 }),
        protocolLines => {
          ...
          const rescan = sanitizeThroughFlowBoundary(cleanedText, {
            turnId: "t-p26",
            source: "discussion",
          });
