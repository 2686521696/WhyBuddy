...
const sharedDurableMeta = new Map<
  string,
  { createdAt: string; lastActive: string }
>();
...
describe("S21 · runtime projection & replay", () => {
  ...
  it("edge 117: JOB→REPLAY / REPLAY→STORE — replay events are session-isolated", async () => {
    ...
    const aRunIds = new Set(
      replayA.filter(e => e.capabilityRunId).map(e => e.capabilityRunId)
    );
    const bRunIds = new Set(
      replayB.filter(e => e.capabilityRunId).map(e => e.capabilityRunId)
    );
    for (const id of aRunIds) expect(bRunIds.has(id!)).toBe(false);

    const aTurnIds = replayA
      .map(e => e.turnId)
      .filter(Boolean)
      .join("|");
    const bTurnIds = replayB
      .map(e => e.turnId)
      .filter(Boolean)
      .join("|");
  ...
  it("Replay isolation by sessionId: loading session A never surfaces session B events/artifacts", async () => {
    ...
    const aConvIds = (loadedA.conversation || [])
      .map((c: any) => c.id)
      .join("|");
...
describe("S22 · persistence & session isolation", () => {
  it("round-trip: a converged session reloads by the SAME sessionId with goal/artifacts/staleArtifactIds/decisionLedger fully restored, then resumes a challenge from the breakpoint", async () => {
    ...
    const { state, reportId, riskId } =
      buildClearStateWithTrustedReport(sessionId);
    ...
    expect(restored.staleArtifactIds || []).toEqual(
    ...
    const restoredReport = (restored.artifacts || []).find(
      a => a.id === reportId
    );
