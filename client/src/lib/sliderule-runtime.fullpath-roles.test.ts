...
function runDeliberationTurn(
  ...
  plan.selected.forEach(
    (sel: { capabilityId: V5CapabilityId; roleId?: string }, idx: number) => {
      ...
      const raw = {
        ...
        ...(payloadByCap?.[cap] !== undefined
          ? { payload: payloadByCap[cap] }
          : {}),
      };
...
describe("S16 · brainstorm complex path + FLOWB", () => {
  ...
  it("D_BO→D_SYN: after panel critique run, chain advances toward synthesis/report (panel chain)", () => {
    ...
    expect(
      (s.capabilityRuns || []).some(r => r.capabilityId === "critique.generate")
    ).toBe(true);
    ...
    expect(
      picks.some(
        p =>
          p.capabilityId === "synthesis.merge" ||
          p.capabilityId === "report.write"
      )
    ).toBe(true);
  ...
  it("N5 + FLOWB: formal report content has zero protocol markers; ledger strip record; payload preserved", () => {
    ...
    const upstreamCritique = (s.artifacts || []).find(
      a => (a.producedBy?.capabilityId as string) === "critique.generate"
    );
    ...
    const reportRunId =
      (reportNode as { capabilityRunId?: string })?.capabilityRunId ??
      "S16-rpt-run-0";
    ...
    const landed = (afterReport.artifacts || []).find(
      (a: any) => a.id === "S16-report"
    );
  ...
  it("3D debate wall: stripDebateProtocolNodes removes critique/rebuttal from formal graph only", () => {
    ...
    expect(
      stripped.nodes.some(n => n.type === "critique" || n.type === "rebuttal")
    ).toBe(false);
...
describe("S17 · brainstorm degraded fallback", () => {
  ...
  afterEach(() => {
    if (origDegradeEnv === undefined)
      delete process.env.SLIDERULE_BRAINSTORM_DEGRADE;
  ...
  it("deliberation exec degraded marks D_DEG and subsequent picks omit brainstorm chain", async () => {
    ...
    expect(
      afterDegPicks.some(p => p.capabilityId === "critique.generate")
    ).toBe(false);
    expect(
      (driven.finalState.conversation || []).some(c =>
        /D_DEG|degraded/i.test(c.text || "")
      )
    ).toBe(true);
  });

  it("intake sets roleMode complex on debate keywords (RL→D_GATE)", () => {
    const { preparedState } = intakeMessage(
      createInitialSessionState(COMPLEX_GOAL_TEXT, "S17-intake"),
      {
        turnId: "S17-i1",
        userText: "来个多角色辩论",
      }
  ...
  it("scheduling pairs remain (capability, role) after degraded drive", async () => {
    ...
    const { plan } = orchestrateReasoningTurn(
      intakeMessage(s, { turnId: "S17-p", userText: "多角色辩论" })
        .preparedState,
