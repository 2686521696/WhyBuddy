...
const TYPES = ["KeyboardCommandPalette","NotificationCenterDrawer","FilterPresetDrawer","ExportJobDrawer","CompareSelectionTray","DetailInspectorDrawer","HelpContextPanel","AuditDiffDrawer","SavedSearchPanel","RecentItemsPanel","RelatedEntityPanel","PermissionSummaryPanel","SelectionInspector","ValidationIssuePanel","ContextHelpDrawer","ChangeImpactPanel"] as const;
...
describe("上下文任务面第二批",()=>{
  ...
  it("目录 16 条均有来源与合法域",()=>{const entries=(catalogJson as {blocks:Array<Record<string,unknown>>}).blocks.filter(entry=>TYPES.includes(String(entry.type) as never));expect(entries).toHaveLength(16);for(const entry of entries){expect(entry.rendererStatus).toBe("real");expect(entry.source).toBeTruthy();expect(entry.bindingSchema).toBeTruthy();expect(entry.allowedRegions).toBeTruthy();expect(entry.pageKinds).toBeTruthy()}});
});
