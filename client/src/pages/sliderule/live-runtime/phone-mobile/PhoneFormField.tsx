 ...
 *
 * **两个不可达档位**（2026-07-29 审查覆盖度时才发现）：`switch` 和 `dateTime`
 * 在判定表里有分支，但 five_system_legal.json 的字段类型闭集是
 * `string/number/date/ref/enum/text`——**没有 boolean、没有 datetime**，
 * 模型生成不出这两种字段。分支留着是防御性的（闭集哪天放开就自动生效），
 * 但别把它们当成"修好的缺陷"：真实生成的应用里从来走不到。
 * 布尔语义现在由模型写成两个取值的 enum 表达，落到 Selector 档，反而更清楚
 *（标签是「是/否」还是「已归档/未归档」由业务自己说了算）。
