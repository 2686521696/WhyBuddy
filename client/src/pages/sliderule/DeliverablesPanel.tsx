                ...
                {available.map((c) => (
                  <button
                    ...
                    className={`flex items-center gap-2.5 rounded px-3 py-2 text-[13px] font-semibold transition ${
                      active === c ? "bg-[#e6f4ff] text-[#1677ff]" : "text-stone-600 hover:bg-white hover:text-stone-800"
                    }`}
                  >
...
function DeliverableViewer({
  ...
  if (category === "report") {
    ...
    return (
      <div className="space-y-4">
        <h2 className="font-display text-lg font-semibold text-[#1f2329]">{artifact.title || "可行性报告"}</h2>
        {closureSummary && (
          <div
        ...
        {sections.map((sec) => (
          <section key={sec.id} className="border-b border-[#e8eaee] pb-4 last:border-0">
                    ...
                    <>
                      {roles.length > 0 && (
                        <div className="grid grid-cols-1 gap-2">
                          {roles.map((r, idx) => (
                            <div key={idx} className="rounded border border-[#e5e7eb] bg-[#eef0f4] p-2 text-[12px]">
  ...
  if (category === "arch") {
    ...
    return (
      <div className="space-y-2">
        <h2 className="font-display text-lg font-semibold text-[#1f2329]">{artifact.title || "架构图"}</h2>
  ...
  if (category === "spec_tree") {
    ...
    return (
      <div className="space-y-2">
        <h2 className="font-display text-lg font-semibold text-[#1f2329]">{artifact.title || "规格树"}</h2>
  ...
  return (
    <div className="space-y-2 text-[13px] leading-relaxed text-stone-700">
      <h2 className="font-display text-lg font-semibold text-[#1f2329]">{artifact.title || CATEGORY_META[category].label}</h2>
      {content.trim() ? (
