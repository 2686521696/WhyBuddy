...
export interface TourStageHandle {
  ...
  /** 诊断口（间距回归验收脚本用）：可见角色的当前位置与走位状态 */
  positionsSnapshot: () => Array<{
    npcId: string;
    x: number;
    z: number;
    moving: boolean;
...
const SAY_LIFT = 0.95;
// 走位局部避让半径：进入他人身位（带臂宽 ~1.5）就侧向绕开，
// 不再直线穿过别人身体——无寻路的轻量替代（用户实测开场穿模）。
// 必须 < 停靠间距 1.7 - 到位容差，否则停靠点进不去；
// 1.35/1.6 实测擦身到 0.80，加大到 1.55/2.2 后实测 ≥1.1
const BODY_GAP = 1.55;
      ...
      for (const [npcId, rig] of rigs) {
        if (rig.target) {
          ...
          if (dist < 0.08) {
            ...
            // 局部避让：身位内有别人（站着或走着）就叠加斥力绕行，
            // 目标点由 SpotAllocator 保证 1.7 间距 > BODY_GAP，必能到达
            for (const other of rigs.values()) {
              if (other === rig || !other.group.visible) continue;
              const away = rig.group.position.clone().sub(other.group.position);
              away.y = 0;
              const d = away.length();
              if (d > 1e-4 && d < BODY_GAP) {
                dir.add(
                  away
                    .normalize()
                    .multiplyScalar(
                      ((BODY_GAP - d) / BODY_GAP) * AVOID_STRENGTH
                    )
                );
              }
            }
            if (dir.lengthSq() < 1e-4) {
              // 正对顶死（斥力恰好抵消前进方向）：侧移打破僵持
              dir.set(forward.z, 0, -forward.x);
            }
            dir.normalize();
