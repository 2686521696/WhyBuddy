...
describe("画布台面改深底之后，前景跟着改了吗", () => {
  it("台面用调色板常量，不是散在 className 里的魔法色", () => {
    expect(STAGE).toContain("const SURFACE = {");
    expect(STAGE).toContain("backgroundColor: SURFACE.base");
    expect(STAGE).toContain("SURFACE.dot");
  });

  it("点阵不跟缩放走（跟着走的话低缩放下会消失）", () => {
    /*
     * ⚠ 2026-08-25 真机：第一版用 React Flow 的 <Background variant=Dots>，
     *   它跟着 viewport transform 缩放。画布常用缩放 18%~57%，1.4px 的点
     *   缩到 0.25px，亚像素直接消失——截图上看像"点阵没生效"。
     *   现在画在台面上，尺寸恒定。
     */
    expect(STAGE).toContain("backgroundSize:");
    expect(STAGE).toContain("SURFACE.dotGap");
    // 反向：不许再用会跟缩放的那个
    expect(STAGE).not.toContain("BackgroundVariant.Dots");
    expect(STAGE).not.toContain("<Background");
  });

  it("白底真的去掉了（不是叠一层深色在白底上）", () => {
    // ⚠ 反向判据：旧的 bg-[#fbfbfc] / border-[#e9edf2] 留在容器上的话，
    //   圆角处会露出一圈白边，而且截图上很难一眼看出来。
    const host = STAGE.slice(
      STAGE.indexOf('data-testid="sliderule-canvas-surface"') - 400,
      STAGE.indexOf('data-testid="sliderule-canvas-surface"') + 400
    );
    expect(host).not.toContain("bg-[#fbfbfc]");
    expect(host).not.toContain("border-[#e9edf2]");
  });

  it("聚光灯层真的挂上了，且不许吃掉画布手势", () => {
    expect(STAGE).toContain('data-testid="sliderule-canvas-spotlight"');
    const lamp = STAGE.slice(
      STAGE.indexOf('data-testid="sliderule-canvas-spotlight"') - 300,
      STAGE.indexOf('data-testid="sliderule-canvas-spotlight"') + 300
    );
    /*
     * ⚠ 2026-08-25 真机变异验过：这层排在 <ReactFlow> 前面（同层后来居上），
     *   删掉 pointer-events-none 手势判据 B/C/D **照样全过**。所以这条判据
     *   不是"顺手钉一下"，它是这件事**唯一**的回归闸——留着这行是让
     *   "不吃手势"不依赖 DOM 顺序。
     */
    expect(lamp).toContain("pointer-events-none");
  });

  it("聚光灯真的跟指针（挂了 onPointerMove，且不走 React state）", () => {
    expect(STAGE).toContain("onPointerMove={onSpotlightMove}");
    expect(STAGE).toContain("spotlightAnchor(box, clientX, clientY)");
    expect(STAGE).toContain('el.style.setProperty("--spot-x"');
    // ⚠ 反向：不许 setState —— 画布上挂着 4 个 iframe，每次 mousemove
    //   重渲一轮会把平移拖成一卡一卡。
    const handler = STAGE.slice(
      STAGE.indexOf("const onSpotlightMove"),
      STAGE.indexOf("const onSpotlightMove") + 900
    );
    expect(handler).toContain("requestAnimationFrame");
    // ⚠ 钉的是"不许调 React setState"——那是**裸调用**。第一版写成
    //   /set[A-Z]\w*\(/ 把 `el.style.setProperty(` 一起误伤了，
    //   红的是判据自己不是功能。加负向后顾排掉属性调用。
    expect(handler).not.toMatch(/(?<![.\w])set[A-Z]\w*\(/);
  });

  it("rAF 要在卸载时取消（不然切档之后还在跑）", () => {
    expect(STAGE).toContain("cancelAnimationFrame(spotlightFrame.current)");
  });

  it("画布平面上的前景都改成了浅色（背景变深、字看不见是本仓最忌的形状）", () => {
    const label = STAGE.slice(
      STAGE.indexOf('data-testid="sliderule-canvas-artboard-label"') - 400,
      STAGE.indexOf('data-testid="sliderule-canvas-artboard-label"') + 200
    );
    expect(label).toContain("text-slate-300");
    expect(label).not.toContain("text-stone-500");

    const asset = STAGE.slice(
      STAGE.indexOf('data-testid="sliderule-canvas-asset-label"') - 300,
      STAGE.indexOf('data-testid="sliderule-canvas-asset-label"') + 200
    );
    expect(asset).toContain("text-slate-300");
    expect(asset).not.toContain("text-stone-500");
  });

  it("缩略图也跟着改了（浅底缩略图贴在深底上是最扎眼的一块）", () => {
    expect(STAGE).not.toContain('!bg-white"');
    expect(STAGE).toContain("!bg-[#0f1420]");
  });

  it("连线和连线上的标签在深底上都要看得见", () => {
    // 旧的 #94a3b8 / #1677ff 是按浅底挑的，深底上偏暗
    expect(STAGE).not.toContain('stroke: "#94a3b8"');
    expect(STAGE).not.toContain('stroke: "#1677ff"');
    // ⚠ 标签底色是**白的**，深底上是一块块白斑；改深底时最容易漏这一处，
    //   因为它写在 edge 对象里，不在 className 里，grep 颜色时看不见。
    expect(STAGE).not.toContain('labelBgStyle: { fill: "#ffffff"');
  });
});
