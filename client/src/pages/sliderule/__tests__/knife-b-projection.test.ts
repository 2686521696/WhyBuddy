...
describe("Knife B · projection density", () => {
  it("detailed mode expands evidence child nodes from evidenceRefs", () => {
    const { state, reportId, riskId } =
      buildClearStateWithTrustedReport("knife-b");
    const report = (state.artifacts || []).find(a => a.id === reportId);
    if (report) {
      report.evidenceRefs = [riskId, "ev-ground-1"];
    }

    const compact = deriveSlideRuleReasoningViewModel(state, {
      density: "compact",
    });
    const detailed = deriveSlideRuleReasoningViewModel(state, {
      density: "detailed",
    });
    expect(detailed.visibleNodes.length).toBeGreaterThan(
      compact.visibleNodes.length
    );
  ...
  it("phase children cite real capabilityRun gate facts", () => {
    ...
    expect(
      phaseNodes.some(n =>
        /观察|思考|完成|接地|提交|风险|综合|证据/.test(n.body || "")
      )
    ).toBe(true);
  });
  ...
  it("phase completed nodes project artifact summary (title/summary/kind) not raw ids", () => {
    const { state, reportId, riskId } =
      buildClearStateWithTrustedReport("knife-b-phase-art");
  ...
  it("spec_tree expands formatTreeContent production serialization", () => {
    ...
    const treeContent = formatTreeContent(buildTemplateTree("权限 MVP"), {
      source: "template",
      gateNote:
        "C_PROMPT:built · G_SCHEMA:attempt1:passed · G_INV:attempt1:passed",
    });
