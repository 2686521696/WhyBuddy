...
describe("V5.3 reasoning chain projection (P3 collaboration + P4 reasoning)", () => {
  it("collaboration mode expands panel roles + verdict + challenges edges (non-depends_on)", () => {
    ...
    (report as any).payload = {
      panel: {
        ...
        critiques: [
          { fromRole: "security", targetRole: "product", content: "成本过高" },
        ],
    ...
    // 角色立场子节点(::role-)出现
    const roleNodes = vm.visibleNodes.filter(
      (n: any) => n.id && n.id.includes("::role-")
    );
    expect(roleNodes.length).toBeGreaterThanOrEqual(2);
    // 收敛裁决节点(::role-_verdict)出现,带收敛分
    const verdict = vm.visibleNodes.find(
      (n: any) => n.id && n.id.includes("::role-_verdict")
    );
  ...
  it("overview mode keeps node count low (no full expand)", () => {
    const state: V5SessionState = {
      ...
      goal: { text: "o" },
      artifacts: [],
      graph: { nodes: [{ id: "root" }], edges: [] } as any,
      capabilityRuns: [],
    } as any;
    const vm = deriveSlideRuleReasoningViewModel(state, {
      viewMode: "overview",
  ...
  it("viewMode switch is pure (no state mutate)", () => {
    ...
    const vm1 = deriveSlideRuleReasoningViewModel(state, {
      viewMode: "overview",
    } as any);
    const vm2 = deriveSlideRuleReasoningViewModel(state, {
      viewMode: "collaboration",
    } as any);
  ...
  it("reasoning mode expands think/observe/tool substeps under cap nodes (count matches events)", () => {
      ...
      artifacts: [
        {
          ...
          kind: "risk",
          trustLevel: "audited",
          producedBy: { capabilityRunId: "t1-run-risk.analyze" },
        } as any,
      ],
      ...
      reasoningEvents: [
        {
          kind: "think",
          order: 0,
          capabilityRunId: "t1-run-risk.analyze",
          text: "thinking",
        },
        {
          kind: "observe",
    ...
    const hasSub = vm.visibleNodes.some(
      (n: any) =>
        n.id && (n.id.includes("::step-") || (n as any)._reasoningSubsteps)
    );
  ...
  it("overview mode adds fold badges and keeps node count ~ turn view", () => {
    const state: V5SessionState = {
      ...
      reasoningEvents: [
        { kind: "think", order: 0, capabilityRunId: "r1" },
        { kind: "observe", order: 1, capabilityRunId: "r1" },
      ] as any,
    } as any;
    const vm = deriveSlideRuleReasoningViewModel(state, {
      viewMode: "overview",
    } as any);
    const hasBadge = vm.visibleNodes.some(
      (n: any) => n.overviewBadge && n.overviewBadge.includes("💭")
    );
