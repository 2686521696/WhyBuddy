...
describe("deriveSlideRuleReasoningViewModel", () => {
  it("intake projection shows only proposition root until ORCH schedules capabilities", () => {
    ...
    expect(
      vm.visibleNodes.filter(n => !n.id.endsWith("-proposition")).length
    ).toBe(0);
    expect(
      vm.visibleNodes.find(n => n.id.endsWith("-proposition"))?.conclusionBadge
    ).toBe("用户命题");
  });
  ...
  it("marks trusted artifact nodes as 结论明确 and untrusted as 结论待完善", () => {
    ...
    const vm = deriveSlideRuleReasoningViewModel(s);
    const evNode = vm.visibleNodes.find(
      n => (n as any).producedArtifactId === "ev-1"
    );
    if (evNode) {
    ...
    const { updatedState } = commitArtifact(
      s,
      {
        ...createGroundedEvidenceRaw("ev-bad"),
        id: "ev-bad",
        content: "无来源",
      },
      "r-bad",
