...
describe("SkillsLibraryPage", () => {
  ...
  it("marketplace 双 tab + 安装动作：有语义档案出安装钮，纯图文诚实禁用", () => {
    ...
    const semTopicIds = new Set(
      (
        skillSemantics as {
          items: Array<{ description: string; topicIds: number[] }>;
        }
      ).items
        .filter(s => s.description)
        .flatMap(s => s.topicIds)
    );
    const top20 = [...skillsIndex.items]
    ...
    if (installable.length > 0) {
      expect(html).toContain(
        `data-testid="skill-install-${installable[0].topicId}"`
      );
    }
    // 纯图文帖：安装钮禁用（诚实——没有可执行定义就不装样子）
    if (notInstallable.length > 0) {
      expect(html).toContain(
        `data-testid="skill-install-disabled-${notInstallable[0].topicId}"`
      );
    }
  });
});
