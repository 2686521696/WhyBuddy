...
export function useSlideRuleSession(options: UseSlideRuleSessionOptions = {}) {
  ...
  const [driveMode, setDriveMode] =
    useState<SlideRuleRuntime.SlideRuleDriveMode>(() => {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("sliderule:driveMode", driveMode);
    } catch {}
  }, [driveMode]);
    ...
    try {
      ...
      try {
        if (driveMode === "marathon") {
          const { driveMarathon } = await import(
      ...
      if (
        ...
        try {
          const recentIds = (final.artifacts || [])
            .slice(-6)
            .map((a: any) => a.id);
          const { createRoundDigest, proposeFrontier } = await import(
            "@/lib/sliderule-marathon-driver"
          );
