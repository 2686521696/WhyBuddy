  ...
  for (let i = 0; i < storage.length; i++) {
    ...
    try {
      ...
      const instances = Array.isArray(parsed.instances)
        ...
        : 0;
      out.push({
        sessionId: key.slice(RUNTIME_KEY_PREFIX.length),
        rows,
        instances,
      });
    } catch {
...
export function summarizeBaseline(payload: unknown): BaselineSummary | null {
  ...
  const domains = Array.isArray(p.domains)
    ? (p.domains as Array<Record<string, unknown>>)
    : null;
  ...
  for (const d of domains) {
    ...
    contentWarns += findings.filter(
      f =>
        f &&
        typeof f === "object" &&
        (f as Record<string, unknown>).severity === "warn"
    ).length;
  ...
  return {
    ...
    judgeAvg:
      judgeAvgs.length > 0
        ? Math.round(
            (judgeAvgs.reduce((a, b) => a + b, 0) / judgeAvgs.length) * 100
          ) / 100
        : null,
  };
}

export const SESSION_PHASE_LABEL: Record<
  string,
  { label: string; tone: "ok" | "busy" | "idle" }
> = {
...
function HealthChip({
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-xl px-4 py-3 ring-1 ${tone}`}
      data-testid={`wb-health-${label}`}
    >
      <span
...
export function MainlineWorkbench() {
  const [nodeHealth, setNodeHealth] = React.useState<Fetched<unknown>>({
    kind: "loading",
  });
  const [pyHealth, setPyHealth] = React.useState<Fetched<unknown>>({
    kind: "loading",
  });
  ...
  const [sessions, setSessions] = React.useState<
    Fetched<{ sessions: SessionRow[] }>
  >({ kind: "loading" });
  const [baseline, setBaseline] = React.useState<Fetched<unknown>>({
    kind: "loading",
  });
  const [runtimeStores, setRuntimeStores] = React.useState<
    RuntimeStoreSummary[]
  >([]);
  ...
  const refresh = React.useCallback(() => {
    ...
    void fetchJson<{
      ...
      keyPresent: boolean;
      keyMasked: string;
    }>("/api/sliderule/llm-channel").then(setChannel);
    void fetchJson<{ sessions: SessionRow[] }>("/api/sliderule/sessions").then(
      setSessions
    );
  ...
  const baselineSummary =
    baseline.kind === "ok" ? summarizeBaseline(baseline.data) : null;

  return (
    <div
      className="mx-auto max-w-[1200px] space-y-4 p-5"
      data-testid="mainline-workbench"
    >
      <div className="flex items-end justify-between">
        <div>
          <h2 className="text-[19px] font-bold text-stone-800">主线观察台</h2>
          <p className="mt-0.5 text-[12px] text-stone-400">
            黄金路径的运行状态：服务 · 推演通道 · 话题会话 · 排练数据 ·
            生成质量基线
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <HealthChip
          label="Node API"
          state={healthState(nodeHealth)}
          detail=":3001"
        />
        <HealthChip
          label="Python 推演引擎"
          state={healthState(pyHealth)}
          detail=":9700"
        />
        <HealthChip
          label="LLM 推演通道"
          state={
            channel.kind === "loading"
              ? "loading"
              : channel.kind === "ok" && channel.data.keyPresent
                ? "ok"
                : "down"
          }
          detail={
            channel.kind === "ok"
              ? `${channel.data.provider} · ${channel.data.model}`
              : channel.kind === "error"
                ? "不可用"
                : undefined
          }
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
            ...
            extra={
              sessions.kind === "ok" ? (
                <span className="text-[11px] text-stone-400">
                  共 {sessions.data.sessions.length} 个 · 显示最近 10
                </span>
              ) : undefined
            }
          >
            {sessions.kind === "loading" && (
              <div className="h-24 animate-pulse rounded-lg bg-[#eef0f4]" />
            )}
            {sessions.kind === "error" && (
              <div className="rounded-lg bg-red-50/60 px-3 py-2 text-[12px] text-red-600 ring-1 ring-red-200">
                会话列表不可用：{sessions.detail}（python
                服务未启动时如实显示，不摆假数据）
              </div>
            )}
            {sessions.kind === "ok" && (
              <table className="w-full text-[12px]" data-testid="wb-sessions">
                ...
                <tbody>
                  {sessionRows.map(s => {
                    const meta = SESSION_PHASE_LABEL[s.phase] ?? {
                      label: s.phase,
                      tone: "idle" as const,
                    };
                    return (
                      <tr
                        key={s.sessionId}
                        className="border-b border-[#f8f9fb] last:border-0"
                      >
                        <td
                          className="max-w-[280px] truncate py-1.5 pr-3 text-stone-700"
                        ...
                        <td className="py-1.5 pr-3 font-mono text-stone-500">
                          {s.artifactCount}
                        </td>
                  ...
                  {sessionRows.length === 0 && (
                    <tr>
                      <td
                        colSpan={4}
                        className="py-4 text-center text-stone-300"
                      >
                        暂无会话——去「推演」发一句意图
                      </td>
                    </tr>
        ...
        <div className="space-y-4 lg:col-span-2">
          ...
          <SectionCard title="生成质量基线（五域评测）">
            {baseline.kind === "loading" && (
              <div className="h-20 animate-pulse rounded-lg bg-[#eef0f4]" />
            )}
            {baseline.kind === "error" && (
            ...
            {baseline.kind === "ok" && baselineSummary && (
              <div data-testid="wb-baseline">
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="rounded-lg bg-[#f8f9fb] px-2 py-2.5">
                    <div
                      className={`text-[17px] font-bold ${baselineSummary.contentFails ? "text-red-600" : "text-stone-800"}`}
                    >
                      {baselineSummary.contentFails} /{" "}
                      {baselineSummary.contentWarns}
                    </div>
            ...
            {runtimeStores.length === 0 ? (
              ...
              <ul className="space-y-1.5" data-testid="wb-runtime-stores">
                {runtimeStores.slice(0, 6).map(s => (
                  ...
                  >
                    <span
                      className="max-w-[200px] truncate font-mono text-stone-500"
                      title={s.sessionId}
                    >
                      {s.sessionId}
                    </span>
      ...
      <div className="text-center text-[10px] text-stone-300">
        legacy 任务队列驾驶舱仍可直达{" "}
        <code className="font-mono">/agent-loop/workbench/legacy</code>
      </div>
    </div>
  );
}
