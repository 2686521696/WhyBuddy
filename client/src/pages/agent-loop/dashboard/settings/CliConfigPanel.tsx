...
export function CliConfigForm({
  ...
  const runtimeLocked = (f: string) =>
    isRunning &&
    ["fixAgent", "reviewAgent", "queuePath", "worktreeScope"].includes(f);
  ...
  useEffect(() => {
    if (initial) {
      form.setFieldsValue({
        ...
        queuePath:
          initial.queuePath || "agent-loop/scripts/migration-queue.json",
  ...
  const handleFinish = (values: any) => {
    if (
      runtimeLocked("fixAgent") ||
      runtimeLocked("reviewAgent") ||
      runtimeLocked("queuePath") ||
      runtimeLocked("worktreeScope")
    ) {
  ...
  return (
    ...
    >
      ...
      <Row gutter={[36, 18]}>
        ...
        <Col xs={24} md={12}>
          <Form.Item
            label="默认 Review Worker"
            name="reviewAgent"
            extra="用于代码 Review 的默认 Worker 实现。"
          >
        ...
        <Col xs={24} md={12}>
          <Form.Item
            label="队列文件路径"
            name="queuePath"
            extra="待处理任务队列文件的路径（相对或绝对）。"
          >
            <Input
              placeholder="agent-loop/scripts/migration-queue.json"
              disabled={runtimeLocked("queuePath")}
            />
          </Form.Item>
        </Col>
        <Col xs={24} md={12}>
      ...
      <Form.Item className="native-cli-submit">
        <Space size="middle" wrap>
          <Button
            ...
            htmlType="submit"
            icon={<SaveOutlined />}
            disabled={runtimeLocked("fixAgent") || runtimeLocked("reviewAgent")}
          >
            保存 CLI 配置
          </Button>
