...
export function SettingsView({
    ...
    () => [
      {
        ...
        children: (
          <SettingsPanel
            title="CLI 基础配置"
            description="配置 AgentLoop CLI 的运行参数与工作环境。"
          >
            <CliConfigForm
              initial={data?.nonSensitive}
              onSave={onSave}
      ...
      },
      {
        ...
        children: (
          <SettingsPanel
            title="LLM Keys 状态"
            description="管理模型提供方状态与本地 Worker CLI 健康检查。"
          >
            <LlmKeyForm
              initial={data || {}}
              onSave={onSave}
              ...
              onTestWorkerCli={onTestWorkerCli}
              queueRunning={data?.queueRunning}
            />
          </SettingsPanel>
        ),
      },
      {
        ...
        children: (
          <SettingsPanel
            title="队列默认值"
            description="预览并应用队列层默认参数，只写入受支持字段。"
          >
            <QueueDefaultsView
              data={queueDefaultsData}
              preview={queuePreview}
              onPreview={onPreviewQueue || (() => {})}
              applyResult={queueApply}
      ...
      },
      {
        ...
        children: (
          <SettingsPanel
            title="Diagnostics（只读）"
            description="查看运行环境、配置来源与关键能力状态。"
          >
            <DiagnosticsView
      ...
      },
      {
        ...
        children: (
          <SettingsPanel
            title="Profiles 管理"
            description="维护不同运行场景的非敏感配置 Profile。"
          >
            <ProfileCrudView
              data={profilesData}
              queueRunning={data?.queueRunning}
  ...
  const footer = (
    <SettingsPanel
      title="设置导入 / 导出（redacted）"
      description="可导出当前 CLI 配置，或从文件导入配置以快速恢复环境。"
      className="native-settings-footer-card"
    >
      ...
      <Text type="secondary" className="native-settings-safety-note">
        敏感 Key 使用安全存储，导出内容只包含非敏感配置与 key
        状态；写入后建议重新加载 CLI 或重新启动运行。
      </Text>
    </SettingsPanel>
  );
  ...
  return (
    <SettingsLayout
      ...
      tabs={
        <Tabs
          className="native-settings-tabs"
