...
export function ProfileCrudView({
  ...
  const submitCreate = () => {
    ...
    if (vals.injectKeysToWorker !== undefined)
      value.injectKeysToWorker = vals.injectKeysToWorker;
    onCreate(name, value);
    setCreateOpen(false);
  };
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="native-profiles-actions">
          <Button
            onClick={() => doSelect(record.name)}
            disabled={record.isActive || isRunning}
          >
            <ActionLabel>选择</ActionLabel>
          </Button>
          <Button onClick={() => openRename(record.name)} disabled={isRunning}>
            <ActionLabel>重命名</ActionLabel>
          </Button>
          <Button onClick={() => openDuplicate(record.name)}>
            <ActionLabel>复制</ActionLabel>
          </Button>
          <Button
            danger
            disabled={isRunning || entries.length <= 1}
            onClick={() => doDelete(record.name)}
          >
            <ActionLabel>删除</ActionLabel>
          </Button>
        </div>
      ),
    },
  ...
  return (
    <div className="native-profiles-panel">
      <div className="native-profiles-toolbar">
        ...
        <Space size={12} wrap>
          <Button icon={<ReloadOutlined />} onClick={onList}>
            刷新
          </Button>
          <Button
            icon={<PlusOutlined />}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="Max Turns" name="workerMaxTurns">
            <InputNumber min={1} />
          </Form.Item>
          <Form.Item label="Max Retries" name="workerMaxRetries">
            <InputNumber min={0} />
          </Form.Item>
