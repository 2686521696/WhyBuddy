...
function DiffTable({ rows }: { rows: DiffRow[] }) {
  if (!rows.length) {
    return (
      <Text type="secondary" className="native-queue-empty-diff">
        no diff（与当前一致或空 patch）
      </Text>
    );
  }
  return (
...
export function QueueDefaultsView({
  data,
  preview,
  onPreview,
  applyResult,
  onApply,
  settingsData,
}: QueueDefaultsPanelProps) {
  const current = (data && data.defaults) || {};
  const supported = (data && data.supportedKeys) || [];
  const isUnsupported = Boolean(
    data && (data.unsupported || (supported.length === 0 && !data.defaults))
  );
  const safeSupported = supported.length
    ? supported
    : [
        "fixAgent",
        "reviewAgent",
  ...
  const [proposedPatch, setProposedPatch] = useState<Record<string, unknown>>({
    workerMaxTurns: 512,
  });
  const [lastRejected, setLastRejected] = useState<string[]>([]);

  const localDry = useMemo(
    () => computeClientDryRun(current, proposedPatch),
    [current, proposedPatch]
  );

  const previewPatch = (patchSource: Record<string, unknown>) => {
    const { patch, rejected } = filterSupportedQueuePatch(
      patchSource,
      safeSupported
    );
  ...
  const doSyncFromSettings = () => {
    const source =
      (settingsData && (settingsData.nonSensitive || settingsData)) || {};
    ...
    <div className="native-queue-defaults-panel">
      ...
      <section className="native-queue-patch-card">
        ...
        <div className="native-queue-actions">
          <Button icon={<SyncOutlined />} onClick={doSyncFromSettings}>
            从 Settings 同步并预览 diff
          </Button>
          <Button icon={<DiffOutlined />} onClick={doPreview}>
            预览 structured diff（不写入）
          </Button>
          <Button
            type="primary"
        ...
        <Text type="secondary" className="native-queue-apply-note">
          预览确认后 apply 仅写入 owned 支持键；workerEnv/secrets 拒绝；写后强制
          JSON 校验 + tasks 数组保留；失败时回滚。
        </Text>
      </section>
      ...
      {preview ? (
        <section
          className={`native-queue-result-card ${preview.ok === false || preview.unsupported ? "native-queue-result-muted" : ""}`}
        >
          {preview.ok === false || preview.unsupported ? (
            <Alert
              ...
              message={
                preview.error ||
                preview.note ||
                "unsupported / read-only (bridge 返回 honest 状态)"
              }
      ...
      {applyResult ? (
        <section
          className={`native-queue-result-card ${applyResult.ok === false || applyResult.unsupported ? "native-queue-result-muted" : ""}`}
        >
          {applyResult.ok === false || applyResult.unsupported ? (
            <Alert
              ...
              showIcon
              message={
                (applyResult.rolledBack ? "rolled back: " : "") +
                (applyResult.error || "unsupported / read-only")
              }
            ...
            <>
              ...
              <Text type="secondary" className="native-queue-apply-note">
                applied: {JSON.stringify(applyResult.applied || {})}
              </Text>
            </>
          )}
        </section>
