...
function resultTone(status?: string) {
  ...
  if (["ok", "ready", "success", "configured"].includes(status))
    return "success";
...
function ProviderHealth({ result }: { result?: any }) {
  ...
  return (
    <div className="native-provider-result">
      ...
      <Text type="secondary">
        Provider Health：{duration} · {result.reason || "cached"}
        {formatResultTime(result.checkedAt)}
      </Text>
    </div>
  );
}
...
function WorkerResultLine({ label, result }: { label: string; result?: any }) {
  return (
    <div className="native-worker-health-row">
      ...
      <Text type="secondary">
        {result
          ? `${result.status || "unknown"} · ${typeof result.durationMs === "number" ? `${result.durationMs}ms` : "-"} · ${result.reason || "cached"}`
          : "尚未探测"}
      </Text>
    </div>
  );
...
function LlmKeyForm({
  ...
  const getProviderResult = (provider: ProviderKey) => {
    return (providerTests || []).find(
      (result: any) => result.provider === provider
    );
  };

  const getWorkerResult = (worker: string) => {
    return (workerCliTests || []).find(
      (result: any) => result.worker === worker
    );
  };
  ...
  const handleClearKey = () => {
    message.warning(
      "Web 控制台仅显示 Key 状态；清除密钥需要后端 SecretStorage 能力接入后启用。"
    );
  };
  ...
  const handleClearAll = async () => {
    ...
    message.warning(
      "清除全部 Keys 暂未写入后端密钥存储；当前仅保留非敏感配置。"
    );
  };
    ...
    >
          ...
          <section className="native-provider-card" key={provider.key}>
            <div className="native-provider-card-head">
              <div className="native-provider-title">
                <span className="native-provider-icon">
                  <KeyOutlined />
                </span>
              ...
              <div className="native-provider-actions">
                ...
                <Button onClick={() => onTestProvider?.(provider.key)}>
                  测试
                </Button>
              </div>
            </div>
            <Input.Password
              ...
              readOnly
            />
            <Text type="secondary" className="native-provider-help">
              {provider.help}
            </Text>
        ...
        <section className="native-worker-health-card">
          ...
          <div className="native-worker-health-actions">
            <Button
              ...
              onClick={() => onTestWorkerCli?.("grok")}
            >
              Probe grok
            </Button>
            <Button
              icon={<ThunderboltOutlined />}
              onClick={() => onTestWorkerCli?.("codex")}
            >
              Probe codex
            </Button>
          </div>
      ...
      <div className="native-llm-note-stack">
        <InfoStrip icon={<InfoCircleOutlined />}>
          敏感 Key 使用安全存储，不会写入项目文件。点击“测试”触发 provider
          health check（不自动执行）。
        </InfoStrip>
        <InfoStrip icon={<ApiOutlined />} tone="neutral">
          非敏感配置保存在工作区设置；敏感 Key 只展示已配置状态。当前 Web
          控制台不会回传明文 Key。
        </InfoStrip>
      </div>
      ...
      {providerTests && providerTests.length > 0 ? (
        <Alert
          ...
          description={providerTests.map((result: any) => (
            <div
              key={`${result.provider}-${result.checkedAt || result.status}`}
            >
              {result.provider} · {result.status || "unknown"} ·{" "}
