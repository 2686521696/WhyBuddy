...
## 边界声明（与 ue-state-sync-bridge 的分工）

**本 spec 负责**：瞬时事件通知。"动画播完了"、"镜头到位了"、"角色到达目标位置了"、"场景加载完成了"——这类一次性、有明确发生时刻的事件。

**不负责**：持续状态同步。角色当前位置、当前镜头参数、当前动作状态等连续变化的数据由 `ue-state-sync-bridge` 处理。

**区分原则**：如果前端需要"等某件事发生再继续"→ 走 event-callback（`waitForEvent()`）。如果前端需要"随时知道当前值是什么"→ 走 state-sync（订阅快照）。
