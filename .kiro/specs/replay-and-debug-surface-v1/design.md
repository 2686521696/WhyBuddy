...
为了降低与主壳收敛 spec 的冲突，本轮建议分两段推进：

- A 段：盘点低频入口、保证 replay 稳定、设计 debug 信息架构
- B 段：在主壳路由与导航定稿后，再接 `/debug` 入口与主导航移除动作
...
- `App.tsx`、`Toolbar.tsx` 与导航配置是热点文件，应尽量等待主壳 spec 锁定后再做最终接线
