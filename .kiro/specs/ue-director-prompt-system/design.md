...
## 边界声明（与 ue-shot-list-planner 的分工）

**本 spec 负责**：高层导演语言生成。输入是任务描述和阶段信息，输出是人类可读的分镜脚本（如"第一幕：Planner 在白板前规划，特写镜头，持续 5 秒"）。

**不负责**：将分镜脚本转换为可执行的指令序列。这是 `ue-shot-list-planner` 的职责。

**接口约定**：本 spec 的输出格式是 `DirectorScript[]`，每个元素包含 `act`（幕号）、`description`（自然语言描述）、`characters`（参与角色）、`cameraHint`（镜头建议）、`durationHint`（时长建议）。`ue-shot-list-planner` 消费这个数组，转换为精确的 `SceneCommand[]` 指令序列。
