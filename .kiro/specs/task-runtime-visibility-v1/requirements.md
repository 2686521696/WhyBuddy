...
- 本 spec 当前完成度约 `93%`
- 首页已经接入统一步骤流与 `Logs / Artifacts / Runtime` 三类运行证据
- 当前运行证据容器已合并进首页共享折叠区，而不是另起一个独立底部浮层
- 剩余工作主要是清掉少量重复日志 / artifact 入口，以及继续细化 `socket / callback` 的事件粒度
...
当前实现说明：
- 运行证据已合并进首页共享辅助 / runtime 容器，以 tab 形式承接 `Logs / Artifacts / Runtime`
- 只要三类运行证据维持唯一主出口，就不要求必须保留一个物理意义上的独立底部 dock 外壳
...
当前实现说明：
- 以上三类内容当前已通过共享 tab 容器提供，并与 `support` 辅助区共用折叠承载层
- 本 spec 的验收重点是“唯一主出口”与“信息归位”，不是容器必须长成独立底边面板
