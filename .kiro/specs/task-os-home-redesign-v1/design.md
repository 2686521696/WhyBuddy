...
- 复用现有 `workspace-*`、`studio-*` token 与面板风格
- 保留当前项目的玻璃层、暖色工作台与 3D 空间氛围
- 优先调整信息层级、密度、骨架与主次关系
- 不另起一套全新的设计系统或主题语言

## 模块归位建议

- `Scene3D` 保留在左侧场景区，降级为状态氛围层
- `OfficeTaskCockpit` 内的高频任务能力应拆分归位到中间任务主线区与右侧控制区
- `TaskHubCommandPanel`、`ClarificationPanel` 优先归入右侧控制区
- `WorkflowPanel`、`TelemetryDashboard` 不应继续以与主任务流并列的强主视图存在，应降级、抽取或折叠进附属区域
...
- 需要先明确现有高频模块的归位关系，否则容易一边改骨架、一边复制旧面板
