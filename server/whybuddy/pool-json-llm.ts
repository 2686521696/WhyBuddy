/**
 * WhyBuddy V5.1 — route auxiliary capabilities through the 5-key LLM pool
 ...
 * WhyBuddy defaults to the same "race" semantics — first successful key wins, not 5× serial wait.
 */
...
export function resetWhyBuddyCapabilityPoolCache(): void {
  cachedPool = undefined;
}

export function resolveWhyBuddyPoolTimeoutMs(poolDefault?: number): number {
  const raw = process.env.WHYBUDDY_POOL_TIMEOUT_MS;
  if (raw) {
...
export function resolveWhyBuddyPoolRaceMode(): "parallel" | "sequential" {
  const raw = (process.env.WHYBUDDY_POOL_RACE_MODE || "parallel").toLowerCase();
...
export function shouldSkipPrimaryLlmAfterPoolExhausted(): boolean {
  if (process.env.WHYBUDDY_SKIP_PRIMARY_AFTER_POOL === "0") return false;
  return isWhyBuddyCapabilityPoolEnabled();
}

export function isWhyBuddyCapabilityPoolEnabled(): boolean {
  if (process.env.WHYBUDDY_CAPABILITY_POOL_ENABLED === "0") return false;
  return getWhyBuddyCapabilityPool() !== null;
}

export function getWhyBuddyCapabilityPool(): LlmKeyPool | null {
    ...
    for (const entry of keys) {
      tryPoolKeyOnce<T>(entry, config, systemPrompt, userPrompt)
        ...
        .catch((e) => {
          ...
          pending -= 1;
          console.warn(
            `[whybuddy-pool] key ${entry.label} failed:`,
            String((e as Error)?.message || e).slice(0, 160)
          );
          if (pending === 0) {
            if (failures > 0) {
              console.warn(`[whybuddy-pool] race exhausted (${failures}/${keys.length} keys failed)`);
            }
            finish(null);
          }
        });
...
async function callPoolJsonLlmSequential<T extends Record<string, unknown>>(
  ...
  if (lastErr) {
    console.warn(
      "[whybuddy-pool] all keys exhausted:",
...
export async function callPoolJsonLlm<T extends Record<string, unknown>>(
  ...
  if (!pool || process.env.WHYBUDDY_CAPABILITY_POOL_ENABLED === "0") return null;

  const config: LlmKeyPoolConfig = {
    ...pool.config,
    timeoutMs: resolveWhyBuddyPoolTimeoutMs(pool.config.timeoutMs),
  };

  const mode = resolveWhyBuddyPoolRaceMode();
  if (mode === "parallel") {
