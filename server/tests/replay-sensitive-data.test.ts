...
describe("sensitive-data", () => {
  ...
  describe("encryptMessage / decryptMessage", () => {
    ...
    it("fails to decrypt with tampered ciphertext", () => {
      ...
      // flip a character in ciphertext（必须保证与原末位不同：固定 'X' 有
      // 1/64 概率撞上随机密文的原字符，篡改等于没改 → CI 概率性假红）
      const flipped = encrypted.ciphertext.endsWith("X") ? "Y" : "X";
      const tampered = {
        ...encrypted,
        ciphertext: encrypted.ciphertext.slice(0, -1) + flipped,
      };
