...
先判断状态，再花钱问模型。我们原来只数总轮数（当时一回合 8 轮），
...
阈值一旦被改回 grok 的原值，或者有人把回合轮数预算调小，立刻红。
──────────────────────────────────────────────────────────────────────────
...
#:   控制面一回合的轮数由 `loop_budget.max_rounds` 定（2026-09-17/18 起
#:   工程档和对话档都是 10_000）。
...
#:   不是「读了很多但不写」）。
#:
#: ⚠ 2026-09-23：grok 的 `max_turns` 默认 `None`（不限轮），本仓当年拿「我们
#:   一回合最多 8 轮」当过推理前提。**那个前提没了**——2026-09-17 / 09-18
#:   起工程档和对话档都是 10_000 轮。所以「只捅不掐」在真机上等于没有兜底：
#:   一个每轮都换实参、从不落地的模型，打转闸不响、压缩让 token 闸够不着、
#:   墙钟 24 小时，会一路烧到 10_000 轮。硬停那一档见
#:   `STOP_AFTER_READONLY_ROUNDS`。
#:
#: 第二档 7：跨回合之后不再受单回合轮数限制，但留着这个数
#: 是为了「捅两次还不动就别再刷屏」。
#:
...
#: 第三档，**硬停**：连着这么多轮整轮都是 READ，这一回合就停。
#:
#: ⚠ 2026-09-23：这道闸原本只捅不掐，理由写在上面——「读源码是正当动作，
#:   掐断会把『多读两轮再下手』的正常行为变成事故」。那个判断在当时是对的，
#:   因为**一回合最多 8 轮**，第二档 7 捅完基本就到头了，轮次上限自己会兜底。
#:   2026-09-17 / 09-18 轮次放开到 10_000 之后那层兜底没了：
#:     · 打转闸只认**重复**调用，每轮换实参就不响；
#:     · token 闸的 include_context 只在压缩之后打开，而压缩把占用压回
#:       12 万以下 → 20 万那条永远够不到；
#:     · 墙钟 86400 秒 = 24 小时。
#:   于是「一直在往前走但永远不落地」——本档头注记的那起真机事故
#:   （17 个动作、project_patch 零次、人工连催 6 次）——会一路烧到 10_000 轮。
#:
#: 12 这个数的依据（§6：要动连同这段一起重跑）：
#:   · 真机**正当**流程量到的最长只读连胜是 6（2026-09-14 那趟，两个回合
#:     各 3 轮，见上面第一档的头注）。12 = 2×，留足余量。
#:   · 两档提醒在 4 / 7，第二次捅完还有 5 轮改口的余地。
#:   · 病态那趟是 17 轮只读，12 提前 5 轮把它停住。
#:   · 够得着（§一之二）：10_000 轮的预算下 12 轻松成立；反过来当年
#:     8 轮预算下它也只差一点——所以阈值不再依赖轮数预算有多大。
STOP_AFTER_READONLY_ROUNDS = 12
...
class ReadOnlyStreak:
    ...
    def should_hard_stop(self) -> bool:
        """捅两次仍不写，到这一档就停这一回合。"""
        return self.rounds >= STOP_AFTER_READONLY_ROUNDS

    def clear(self) -> None:
        """停过之后清零。

        ⚠ 这道闸是**跨回合**累积的，不清零的话用户一说「继续」，下一轮第一发
          读就又到 12，会话当场焊死在同一处。停一次、说清楚、下一回合重新给
          12 轮——每回合有界，要不要继续由用户决定。
        """
        self.rounds = 0
        self.nudged_at = 0
