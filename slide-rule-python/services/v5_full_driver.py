...
_ENRICH_STAGE_LABELS: Dict[str, tuple] = {
    ...
    #   直接返回 None，左侧就是一片空白。
    #
    # 区间取实测（连锁宠物医院 / 二手车两轮）：spec 62s · pages 228s(5 页并发) ·
    # structure 81s · assemble 24s · bind 231s(6 页)。按老规矩取 p25~p75 当
    # "通常"、另说上限——**写窄的提示比不写更糟，它把正常说成异常**。
    "specfirst.spec": ("起草规格：成功判据、需求节点与页面清单", "通常 60~90 秒"),
    "specfirst.pages": ("逐页画界面（并发）", "通常 3~4 分钟，页数越多越久"),
    "specfirst.structure": ("从界面反推数据模型与关联关系", "通常 80~120 秒"),
    "specfirst.semantics": ("推导权限、工作流与不变式", "通常 60~120 秒"),
    "specfirst.assemble": ("汇合五系统模型并过结构闸", "通常 20~40 秒"),
    "specfirst.bind": ("给界面接上数据", "通常 3~4 分钟"),
