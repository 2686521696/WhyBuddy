...
#: 1 pt = 12700 EMU。幻灯片高度换成磅，字号才能按**幻灯片**的比例缩。
_EMU_PER_POINT = 12700


def _slides_html(payload: dict[str, Any]) -> str | None:
    """幻灯片预览页。**没有脚本，也没有行内 style 属性。**

    ⚠ 2026-09-23 review 两条：
      · office_preview_payload 抽了 fontSize，这里一行没用——每页文字
        都是浏览器默认字号，标题和正文一样大，和真机 PPT 对不上（§4
        生成侧 / 消费侧）。字号按幻灯片高度的比例给（cqh），舞台缩多少
        字就缩多少；老浏览器认不出 cqh 时留在前面那条 pt 上。
      · 上一版翻页靠一段行内 <script>，于是路由必须发
        `script-src 'unsafe-inline'`——等于对这一页放开了整类注入。
        翻页改成 radio + :checked 兄弟选择器，一行脚本都不要；每个形状的
        位置/颜色/字号也从 style="" 挪进 <style> 里的生成规则，
        于是 style 也能用 hash 钉死（见 office_preview_csp）。
    """
    ...
    if width <= 0 or height <= 0:
        width, height = 12192000, 6858000
    slide_points = height / _EMU_PER_POINT
    ...
    for slide in slides[:40]:
        ...
        for shape in shapes[:30]:
            ...
            try:
                ...
                continue
            name = f"p{page}f{len(body)}"
            declarations = [
                f"left:{x:.3f}%", f"top:{y:.3f}%",
                f"width:{w:.3f}%", f"height:{h:.3f}%",
                "color:" + _css_color(shape.get("color"), "#111827"),
                "background:" + _css_color(shape.get("fill"), "transparent"),
            ]
            points = _font_points(shape.get("fontSize"))
            if points and slide_points > 0:
                # 先 pt 后 cqh：认不出容器查询单位的浏览器停在 pt 上，
                # 认得出的用比例，舞台缩多少字就缩多少。
                declarations.append(f"font-size:{points}pt")
                declarations.append(f"font-size:{points / slide_points * 100:.3f}cqh")
            rules.append(f".{name}{{{';'.join(declarations)}}}")
            body.append(
                f'<div class="shape {name}">'
        ...
        rules.append(f".pg{page}{{background:{background}}}")
        # 只有被选中的那一页显示。radio 在 .stage 前面，用兄弟选择器。
        rules.append(f"#s{page}:checked~.stage .pg{page}{{display:block}}")
        rules.append(
            f"#s{page}:checked~.thumbs label[for=s{page}]"
            "{outline:2px solid #38bdf8;background:#1f2937}"
        )
        rules.append(
            f"#s{page}:focus-visible~.thumbs label[for=s{page}]"
            "{outline:3px solid #f59e0b}"
        )
        checked = " checked" if page == 0 else ""
        inputs.append(f'<input type="radio" name="wb-slide" id="s{page}"{checked}>')
        sections.append(f'<section class="slide pg{page}">{"".join(body)}</section>')
        label = html.escape(str(slide.get("text") or "").split("\n", 1)[0][:24] or str(page + 1))
        labels.append(f'<label for="s{page}">{label}</label>')
    if not sections:
        return None
    style = (
        "html,body{margin:0;height:100%;background:#111;font-family:sans-serif}"
        "body{display:flex;flex-direction:column;height:100%}"
        # 焦点要留得住，所以不是 display:none，是挪到看不见的地方。
        "input[name=wb-slide]{position:absolute;width:1px;height:1px;opacity:0}"
        ".stage{flex:1;display:flex;align-items:center;justify-content:center;padding:16px}"
        ".slide{display:none;"
        f"width:min(100%,calc((100vh - 120px) * {width} / {height}));"
        f"aspect-ratio:{width}/{height};"
        # cqh 要有查询容器才解析得出来。
        "container-type:size;position:relative;overflow:hidden;background:#fff}"
        ".shape{position:absolute;overflow:hidden;white-space:pre-wrap;line-height:1.25}"
        ".plain{position:absolute;inset:8%;white-space:pre-wrap;color:#111827}"
        ".thumbs{display:flex;gap:8px;overflow:auto;padding:8px 12px}"
        ".thumbs label{cursor:pointer;color:#e5e7eb;font-size:12px;padding:4px 8px;"
    ...
    return (
        ...
        + style + "</style></head><body>" + "".join(inputs)
        + '<div class="stage">' + "".join(sections) + "</div>"
        + '<div class="thumbs">' + "".join(labels) + "</div>"
        + "</body></html>"
    )


def _font_points(value: Any) -> int | None:
    """rPr 的 sz 已经在 office_preview_payload 里除过 100，这里就是磅。"""
    try:
        points = int(value)
    except (TypeError, ValueError):
        return None
    return points if 1 <= points <= 200 else None


_STYLE_BLOCK = re.compile(r"<style>(.*?)</style>", re.S)


def office_preview_csp(document: Any) -> str:
    """这一页的 CSP。**hash 从真要发出去的那份文档里取。**

    ⚠ 2026-09-23 review：上一版是 `script-src 'unsafe-inline';
      style-src 'unsafe-inline'`。那一页是从用户的 .pptx 里生成的 HTML，
      转义一旦漏一处就直接是同源执行。现在页面自己不带脚本
      （翻页是 radio + :checked），样式只剩一个 <style> 块，按它的 sha256 钉死。

    ⚠ 另算一份字符串去 hash = 迟早两边对不上，而且只在浏览器控制台报错、
      页面白屏，服务端一切正常。所以这里只认调用方手上那份 document。
    """
    ...
    return (
        ...
        "style-src " + (" ".join(digests) or "'none'") + "; "
