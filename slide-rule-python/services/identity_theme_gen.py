...
- 饱和度要**适中偏高**，不要选接近无彩色的灰/米白/浅灰蓝这类低饱和度颜色
  ——派生算法会按这个种子色的饱和度等比例推出中性色/强调色，种子本身饱和
  度太低，整套派生出来的配色会显得"发灰发脏"，缺乏品牌辨识度。
- 也不要选过曝的高饱和荧光色（比如纯 #ff0000/#00ff00 这类），那类颜色
  派生出的深色侧边栏/浅色背景容易过艳刺眼。
- 挑一个你在真实产品里会认得出"这是这家应用的颜色"的中高饱和度色相即可，
