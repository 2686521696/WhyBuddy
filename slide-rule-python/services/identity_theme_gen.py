...
_SHELL_SHAPE_NOTE: dict[str, str] = {
    # 手机档画布是**正方形**（1254x1254——生图端点给不了竖图，四个竖版尺寸
    # 全被归一化成方图，探针记录见 freeform_block._DEVICE_IMAGE_SIZE 上方）。
    # 此前这里写"整体竖屏比例"，跟真实画布对不上，下面还跟着一句"画面本身要
    # 撑满整个画布，边缘到边缘"——两句一起会让模型把手机界面横向拉宽填满方
    # 画布，画出手机上不存在的比例。改成明说画布是方的、手机界面画成居中竖栏。
    "phone": (
        "画手机端形态——上方一条窄标题栏、下方一条底部 Tab 导航栏，中间是内容区。"
        "画布是正方形，手机界面要画成**居中的一条竖栏**（宽度约占画布一半、上下铺满），"
        "左右两侧留干净的同色背景，不要把手机界面横向拉宽去填满画布。"
    ),
...
def _build_reference_image_prompt(app_name: str, goal_text: str, datamodel_summary: str, device: str) -> str:
    ...
    # "铺满画布"这类话对手机档要改口径：那一档画布是正方形而手机界面是竖的，
    # 照原话说会把界面横向拉宽，画出手机上不存在的比例（见 _SHELL_SHAPE_NOTE）。
    if device == "phone":
        use_canvas_note = "充分利用画布高度"
        fill_note = "手机界面本身要竖向撑满画布上下、左右留干净背景，"
    else:
