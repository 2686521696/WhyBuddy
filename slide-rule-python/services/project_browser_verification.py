...
def _check(task, child):
    ...
    if (current.status != "running" or lease is None
            ...
            or parent.runtime is None or parent.runtime.status != "ready"
...
def run_next_project_verification(task):
    ...
    try:
        ...
        _check(task, child)
        grant = access.issue_browser_ticket(task.operation_id, owner_id=task.owner_id,
                                           audience=origin)
        ...
        records.finish(record.verificationId, **_scope(task), status="blocked", assertions=[], artifacts={},
            error_code="project_browser_evidence_unavailable")
    finally:
