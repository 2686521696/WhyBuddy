...
# 生图尺寸按设备走：桌面走宽屏，**手机只能走正方形**。
#
# 原本这里给手机填的是 "1024x1792"，注释写着"手机该是竖屏"——那句是错的，
# 而且错得很安静：这个端点**根本不返回竖图**。活体探针（2026-07-30，四个
# 竖版尺寸逐个真发一轮）全部被归一化成同一张正方形：
#   1024x1792 → 1254x1254   1024x1536 → 1254x1254
#   1536x2048 → 1254x1254   2160x3840 → 1254x1254
# 四个不同长宽比、不同像素量的竖版请求收敛到同一个输出，说明服务端是按
# 档位重排的，不是"传什么给什么"。所以过去每次手机档生图，代码以为拿到的
# 是 9:16 竖图、prompt 也照着竖屏写，实际拿到的是 1:1 方图——参照图的形状
# 一开始就是错的，而这类错误不会报错，只会让设计 LLM 学到错的比例。
#
# 现在改成**如实请求正方形**："1024x1024" 同样返回 1254x1254（同一档，
# 活体探针 2026-07-30），但请求与产出一致，不再是"要竖的、收方的"。
# 像素量跟桌面档对齐（1672x941 ≈ 1.57MP，1254x1254 ≈ 1.57MP），两档参照图
# 清晰度同一水平，不会一档字清楚一档字糊（"2048x2048" 会跳到 4.2MP 的更
# 贵档位，没必要）。
#
# 手机档的 prompt 同步照正方形画布来写（画一条居中的窄栏，不要横着摊开
# 填满宽度）——见 _build_reference_image_prompt 的 device_note 与
...
# 参照板单独一档尺寸。
#
# 活体探针（2026-07-29，逐个尺寸真发一轮）测出这个端点认的是一份**白名单**，
# 不是"任意宽高"：
#   3840x2160  ✅ 实际返回 3840x2160（真 4K 16:9）
#   2048x2048  ✅  2048x1536 ✅  1536x1024 ✅
#   2048x1152  ⚠️ 2026-07-29 首测 ✅，同日复测**稳定 503**——白名单会漂移，
#              别把探针结论当常量，每次改尺寸都要现测
#   1792x1024  ✅ 但**实际只返回 1672x941**（被内部档位吃掉了）  ← 本档选它
#   1024x1792 / 1024x1536 / 1536x2048 / 2160x3840
#              ⚠️ 都不报错，但**一律返回 1254x1254 的正方形**——这个端点上
#              根本拿不到竖版画布（活体探针 2026-07-30，四个竖版尺寸逐个真
#              发一轮，见 _DEVICE_IMAGE_SIZE 上方的完整记录）。所以手机专属
#              档的参照板走横版、改成并排画两屏手机（见
#              _build_overview_sheet_prompt），单区块参照图走正方形。
#   1024x1024  ✅ 返回 1254x1254（1.57MP，与 1792x1024 同档）← 手机档选它
#   2304x1296 / 2048x1024 / 1920x1080 / 2560x1440 / "auto"  ❌ HTTP 400
# 所以"传什么尺寸就出什么尺寸"这个假设不成立，改尺寸必须实测返回的 PNG 头。
#
# 2026-07-29 从 3840x2160 **改回** 1792x1024（实收 1672x941）：4K 那档一张图
# 的生成耗时与费用都是这一档的数倍，而这张板的用途只是**给设计 LLM 看一眼定
# 调性**——它不进产物、不给终端用户看，分辨率只要"中文标签不糊到认不出"就够。
# 代价是画面能塞的东西变少，所以下面的 prompt 同步收了密度预算（图表 3→2、
# 动态列表 5→3 行）：不减密度直接降分辨率，元素会挤成一团，那才是
# 真正让参照失效的原因——不是像素数不够，是每个元素分到的像素不够。
...
def _build_reference_image_prompt(
    ...
    *,
    ...
    charts = "、".join(hint["charts"])
    device_note = {
        # 画布是**正方形**（1254x1254，见 _DEVICE_IMAGE_SIZE 上方的探针记录：
        # 这个端点给不了竖图）。此前这里写"比例偏竖长"，跟真实画布对不上，
        # 而下面还有一句"画面内容要充分利用整个画布"——两句一起把模型引向
        # "把手机内容横着摊开填满方画布"，画出来的比例正好是手机上不可能有
        # 的样子。改成明说画布是方的、内容画成居中的窄栏。
        "phone": (
            "这是手机端内容区里的一张卡片（上方标题栏、下方 Tab 导航另有画面，"
            "不用你画）。画布是正方形，但手机内容区是**窄的**：把这张卡片画成"
            "一条居中的竖向窄栏（宽度约占画布的一半，上下方向铺开），左右两侧"
            "留同色背景即可——不要为了填满画布把内容横向摊开，那个比例在手机上"
            "根本不存在。栏内必须单列纵向排布，字号/图标/间距比桌面收紧一档。"
        ),
        "tablet": "这是平板端内容区里的一张卡片（侧边栏+顶栏另有画面，不用你画），中等宽度。",
    }.get(device, "这是桌面端内容区里的一张卡片（左侧侧边栏、上方顶栏另有画面，不用你画），可以偏宽幅横向布局。")
    fill_note = (
        "内容在竖直方向要铺满画布（上下不要留空），左右按下面说的留背景；"
        if device == "phone"
        else "画面内容要充分利用整个画布，边缘到边缘，不要在四周留一圈空白画布底色、"
    )
    ...
    return (
        ...
        "不要只画示意性的空壳卡片；不要写任何具体数字/真实数据，占位文案用"
        "「示例XX」这类通用字样；"
        ...
        # 手机档画布是正方形而内容区是窄的，"边缘到边缘铺满"这句会把内容横向
        # 摊开，正好画出手机上不存在的比例——所以这句只对宽幅档说。
        f"{fill_note}"
...
def _build_overview_sheet_prompt(
    ...
    *,
    ...
    画布尺寸恒为横版（_SHEET_IMAGE_SIZE）。手机专属档没有走竖版画布——
    活体探针（2026-07-30）实测这个端点把 1024x1792 **压成 1254x1254 的
    正方形**返回，拿不到真正的竖版画布，所以手机档改成在横版画布上并排画
    一到两屏手机（首屏 + 下滑一屏），既填满画布又全是真实版式信息。
    ...
    if device == "desktop":
        zones_text = (
            "画面就是**一整页桌面端总览界面**，铺满整张画布——不是色板拼贴，"
            "不要在旁边留一栏画色块/图标/字号样例，整张画布都用来画这一页真实"
            "完整的版式。就当画布是一个 16:9 的 PC 内容区（1600x900）：顶部一行"
            "指标卡（3-4 张）、下方图表区**最多 2 张图**，多列横向排布；每张卡"
            "要有真实的图标、标题、数据占位和趋势/状态这类辅助信息层级，不要画"
            "成空壳色块。不要画左侧侧边栏和顶栏（那部分另有画面）。这个应用明确"
            "是桌面端专属，不要画手机版式。\n"
        )
        separator_line = ""
        widget_style_line = (
        ...
        zones_text = (
            "画面画的是**手机端总览界面的真实完整版式**——不是色板拼贴，不要"
            "画色块/图标/字号样例，整张画布都用来画真实界面。画布是横版，所以"
            "**并排画两块 9:16 的手机屏**（各自套一个简洁的手机轮廓，高度接近"
            "画布满高）：左边一块是首屏，右边一块是**同一页继续往下滑一屏**看到"
            "的内容，两块合起来把这一页交代完整。内容是**单列纵向**排布：指标卡"
            "一行一张、图表压扁、动态列表**最多 3 行**；每张卡要有真实的图标、"
            "标题、数据占位和状态标签，不要画成空壳色块。不要画顶部标题栏和底部 "
            "Tab 栏（那部分另有画面）。这个应用明确是手机端专属，不要画桌面版式。\n"
        )
        separator_line = "两块手机屏之间留白区分开，各自左上角标一个小标题说明是首屏还是下滑屏。\n"
        widget_style_line = (
    ...
    return (
        ...
        "不要写任何具体数字或真实数据，占位文案用「示例XX」这类通用字样；"
