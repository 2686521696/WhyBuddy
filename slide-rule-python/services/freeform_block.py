...
# 动态列表 5→3 行、图标 8→6）：不减密度直接降分辨率，元素会挤成一团，那才是
...
def _build_reference_image_prompt(
    ...
    *,
    ...
    return (
        ...
        "画面内容要充分利用整个画布，边缘到边缘，不要在四周留一圈空白画布底色、"
...
def _build_overview_sheet_prompt(
    ...
    *,
    ...
    """总览页的参照板：样式风格 + 版式 mockup。

    默认画**三区**（样式风格 + 桌面版式 + 手机版式），两档设计共用一张：
    桌面与手机两档版式要出自同一套视觉语言，分开生三张图各自随机，拼起来
    不像一个产品；一张板上并排画，模型自己会保证三区同色同调——这也正是
    设计行业「Style Tile」这个交付物的用法（Samantha Warren 2011）：色板、
    字体层级、按钮/标签/图标样例摆在同一块板上，先定调性再谈具体页面。

    2026-07-30：`device` 明说 desktop/phone 时改画**两区**（省掉的那一区
    版式 mockup 根本不会用到）——`enrich_monitor_page_overviews` 判定
    preferredDevice 明说桌面时，压根不会再跑手机档那次 freeform 设计生成
    （见 6fe1c13），但这张参照板此前没接 device 参数，一直无条件画三区，
    等于让生图模型白画一块永远用不到的手机 mockup，还挤占了桌面区本该有
    的画布份额（两区版下桌面区能占的比例明显更大，字号也能画得更清楚）。
    跟判设备档同一条纪律：**只在明说的时候才砍**，unspecified/没写仍按
    保守的三区画（不确定是不是纯桌面应用时，两档 mockup 都留着更安全）。

    版面按**真实屏幕比例**来切，不是等宽竖栏（2026-07-29 改）：

      桌面区 = 1600x900（16:9）——真实 PC 内容区的形状
      手机区 = 360x640（9:16）——真实手机视口的形状
      样式风格 = **捡剩下的空位**

    为什么按比例切：两档版式的**宽高比本身就是要传达的信息**（设计 LLM 看
    这张图就是在学"这一档有多宽、一行能放几个"），比例失真参照就带偏。
    色板/图标/字号这些样例没有固有比例，塞哪都行，所以让它们让位、去填边角。

    注意跟单区块参照图（_build_reference_image_prompt）的一个关键差别：那张
    明令"不要画外框/浏览器窗口 mockup，这张图本身就是内容区局部"；这张恰恰
    相反——每个版式区要能看出边界，手机那区还需要一个手机轮廓才说得清是哪
    一档。
    """
    hint = _theme_palette(theme_id, generated_theme)
    charts = "、".join(hint["charts"])
    ...
    # 只在明说的时候才砍掉不会用到的那个版式区——unspecified/没写仍按
    # 保守的三区画（同 6fe1c13 判设备档那条纪律）。
    if device == "desktop":
        zones_text = (
            "上面并排画两块内容，共用同一套视觉语言。**桌面版式的宽高比是这张图"
            "最要紧的信息，必须按下面给的比例画准，不许为了排版方便改形状**：\n"
            "① 「桌面首页」——占画布主体（16:9 宽屏区域，就当它是一个 1600x900 "
            "的 PC 内容区）。内容是桌面端的总览版式：顶部一行指标卡（3-4 张）、"
            "下方图表区**最多 2 张图**，多列横向排布。不要画左侧侧边栏和顶栏"
            "（那部分另有画面）。这个应用明确是桌面端专属，不需要画手机版式。\n"
            "② 「样式风格」——**捡剩下的空位**（桌面区下方留出的横条）：主色/"
            "辅色的色板色块、一组简洁线性图标（**6 个就够**）、按钮与标签与"
            "徽标的样例、两三级字号层级示意。这些样例没有固有形状，随空位横排"
            "竖排都行，但不许反过来挤占①的比例——①才是主角，应该占掉画布"
            "的大部分面积。\n"
        )
        zone_count_word = "两块"
        widget_style_line = (
        ...
        zones_text = (
            "上面并排画两块内容，共用同一套视觉语言。**手机版式的宽高比是这张图"
            "最要紧的信息，必须按下面给的比例画准，不许为了排版方便改形状**：\n"
            "① 「手机首页」——占画布主体，画成一根 9:16 的竖条（就当它是一块 "
            "360x640 的手机屏），高度接近画布满高，外面套一个简洁的手机轮廓。"
            "内容是总览的**单列纵向**排布：指标卡一行一张、图表压扁；下面的"
            "动态列表**最多 3 行**。这个应用明确是手机端专属，不需要画桌面"
            "版式。\n"
            "② 「样式风格」——**捡剩下的空位**（手机区旁边留出的空间）：主色/"
            "辅色的色板色块、一组简洁线性图标（**6 个就够**）、按钮与标签与"
            "徽标的样例、两三级字号层级示意。这些样例没有固有形状，随空位横排"
            "竖排都行，但不许反过来挤占①的比例。\n"
        )
        ...
        zones_text = (
            "上面并排画三块内容，共用同一套视觉语言。**两档版式的宽高比是这张图最"
            "要紧的信息，必须按下面给的比例画准，不许为了排版方便改形状**：\n"
            "① 「桌面首页」——放画布**左上**，画成一块 16:9 的宽屏区域（就当它是一"
            ...
            "是手机。\n"
            "③ 「样式风格」——不给它单独分栏，**哪里有空位就填哪里**（桌面区下方留"
            "出的横条、手机区下方剩的方块）：主色/辅色的色板色块、一组简洁线性图标"
            "（**6 个就够**）、按钮与标签与徽标的样例、两三级字号层级示意。这些样例"
            "没有固有形状，随空位横排竖排都行，但不许反过来挤占①②的比例。\n"
        )
        zone_count_word = "三块"
        widget_style_line = (
    ...
    return (
        "生成一张 1672x941 的 UI 设计参照板（design style sheet），"
        f"{zones_text}"
        ...
        "整张板**宁可留白也不要塞满**：上面给的「最多/就够」是上限不是目标。"
        "硬性底线——画面里任何一个中文字的字高不得小于画布高度的 1.5%"
        "（这张 941 高的板上就是约 14px）；排不下就**减少条目、砍列、拿掉次要"
        "区块**，绝对不许靠缩小字号把东西塞进去。宁可少画三分之一，也不要出现"
        "一个看不清笔画的中文。\n"
        f"{zone_count_word}之间用细分隔线或留白区分开，每块左上角标一个小标题说明它是哪一块。\n"
        f"设计需求：{design_brief}。"
...
def enrich_monitor_page_overviews(model: dict[str, Any]) -> dict[str, Any]:
    ...
    for page in (model.get("page") or {}).get("pages") or []:
        ...
        if allow_shot:
            shot_used += 1
        # 参照板（样式风格 + 会真正生成的那几档版式），共用一张。分开生的话
