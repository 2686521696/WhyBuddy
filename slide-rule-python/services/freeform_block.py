...
_PLACEHOLDER_STYLE_NOTE = (
    ...
    "人名写成「张销售」这类示例名、机构写成「示例科技有限公司」；"
    "状态/分类这种有固定选项的，直接用下面数据字段里给出的真实枚举标签。"
    "**一个真实数据都不许出现**，但要让人一眼看出这一格装的是什么类型的内容；"
)

# 生图尺寸：这个端点上，**尺寸参数只决定像素预算档位，长宽比由提示词内容
# 决定**。这条不直觉，是 2026-07-30 一串活体探针撞出来的，改这里之前先读完。
#
# 第一轮探针用的是一句没有形状线索的中性提示词（"纯浅灰背景，正中一个大号
# 数字 1"），四个竖版尺寸全部回落成同一张方图：
#   1024x1792 → 1254x1254   1024x1536 → 1254x1254
#   1536x2048 → 1254x1254   2160x3840 → 1254x1254
# 连 skill 包那套 {"image_size":"2K","aspect_ratio":"9:16"} 请求体也一样
# （能通、不再像旧端点那样挂到 504，但 aspect_ratio 被忽略）→ 1254x1254。
# 据此一度得出"这个端点给不了竖图"的结论——**那是错的**，错在探针的提示词
# 本身没说要竖图。
#
# 第二轮：同样传 "1024x1024"，但提示词明说"生成一张手机 App 界面草样、手机
# 竖屏比例"，出图 **864x1821，真竖图**。所以形状归提示词管。
#
# 像素预算是尺寸参数管的那部分，两档对齐在同一档：
#   1792x1024 → 1672x941  ≈ 1.57MP（桌面）
#   1024x1024 → 864x1821  ≈ 1.57MP（手机，竖）
# 手机档填 "1024x1024" 不是"想要方图"，是"要这一档的像素预算"；竖不竖完全
# 由 prompt 里那句 9:16 决定（见 _build_reference_image_prompt 的 device_note
# 与 identity_theme_gen._SHELL_SHAPE_NOTE——**那两处的竖屏措辞是功能性的，
# 不是描述性的，删了就会变回方图**）。
...
# 参照板单独一档尺寸。
#
# ⚠️ 下面这份白名单是 **2026-07-29 对上一个端点** 测的，本会话换过
# IMAGE_API_URL，其中至少一条已经不成立——**换端点后必须整份重测**：
#   3840x2160  2026-07-30 复测：**只返回 1672x941**（跟当前档一个样，耗时
#              85s vs 90s 几乎无差）。也就是说当前这家端点**给不了 4K**，
#              "升档换清晰度"这条路在这里走不通；想要更高像素得换服务商
#              （IMAGE_API_URL/MODEL/KEY 全是环境变量，不用改代码）。
#              下面这行是旧端点的结论，留作对照：
#   3840x2160  ✅（旧端点）实际返回 3840x2160（真 4K 16:9）
