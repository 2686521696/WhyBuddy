...
def _build_overview_sheet_prompt(
    ...
    *,
    ...
    版面按**真实屏幕比例**来切，不再是三条等宽竖栏（2026-07-29 改）：

      桌面区 = 1600x900（16:9），画布左上，占大头——这是真实 PC 内容区的形状
      手机区 = 360x640（9:16），画布右侧竖条——这是真实手机视口的形状
      样式风格 = **捡剩下的空位**（桌面区下方的横条 + 手机区下方的方块）

    为什么这么改：前一版给"样式风格"分了一整条等宽竖栏，等于拿画布 1/3 去
    画色板，两档版式反被压窄——桌面区被挤成方的、手机区宽得不像手机，而两
    档版式的**宽高比本身就是要传达的信息**（设计 LLM 看这张图就是在学"这一
    档有多宽、一行能放几个"）。比例失真，参照就带偏。色板/图标/字号这些样例
    没有固有比例，塞哪都行，所以让它们让位、去填边角。
    ...
    return (
        ...
        "一个看不清笔画的中文。\n"
        "三块之间用细分隔线或留白区分开，每块左上角标一个小标题说明它是哪一块。\n"
        f"设计需求：{design_brief}。"
...
def _generate_overview_sheet_b64(
    ...
    *,
    ...
    图上要同时容纳两档版式和一堆样例，小了字就糊。可用尺寸是白名单，见
