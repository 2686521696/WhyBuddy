...
# 才是真正的渲染源），这里只取生成 prompt 用得上的几个字段。两边独立维护，
# 改一边要记得同步改另一边；万一漏改，最坏结果是色板提示不够准，不影响
# 安全边界（颜色值本身仍然过 Pydantic 白名单+危险模式校验）。
_THEME_COLOR_HINTS: dict[str, dict[str, str]] = {
    "azure": {"label": "湛蓝·通用企业", "primary": "#1677ff", "contentBg": "#f0f2f5", "accentBg": "#e6f4ff"},
    "forest": {"label": "松绿·生产运营", "primary": "#2e7d32", "contentBg": "#f4f7f2", "accentBg": "#e8f5e9"},
    "graphite": {"label": "石墨·专业中性", "primary": "#525252", "contentBg": "#f0f0f0", "accentBg": "#e5e5e5"},
    "tangerine": {"label": "橘橙·消费活力", "primary": "#e05d38", "contentBg": "#f8fafc", "accentBg": "#fff0eb"},
    "violet": {"label": "紫罗兰·创意智能", "primary": "#7033ff", "contentBg": "#f7f7f8", "accentBg": "#ede9fe"},
    "amber": {"label": "琥珀·财务审计", "primary": "#d97706", "contentBg": "#fffdf7", "accentBg": "#fffbeb"},
    "clay": {"label": "陶土·温暖人文", "primary": "#c96442", "contentBg": "#faf9f5", "accentBg": "#f5e8df"},
    "indigo": {"label": "靛蓝·数据密集", "primary": "#6366f1", "contentBg": "#f8fafc", "accentBg": "#e0e7ff"},
}
...
def _theme_prompt_fragment(theme_id: str) -> str:
    hint = _THEME_COLOR_HINTS.get(theme_id) or _THEME_COLOR_HINTS[_DEFAULT_THEME_ID]
    return (
        f"这个应用当前用的身份主题是「{hint['label']}」：主色 {hint['primary']}、"
        f"内容区底色 {hint['contentBg']}、强调浅底 {hint['accentBg']}——这些颜色已经"
        "用在真实渲染出来的侧边栏/顶栏/按钮上了。整体配色基调必须贴着这套主题的"
        "色相走（可以用这几个颜色的深浅变体、或者同色相邻近色做多元素区分），"
        "不能整体换成完全不搭的色相体系（比如主题是暖橙系就不要通篇上蓝紫）。"
    )
...
def _build_reference_image_prompt(design_brief: str, *, theme_id: str = "", device: str = "") -> str:
    hint = _THEME_COLOR_HINTS.get(theme_id) or _THEME_COLOR_HINTS[_DEFAULT_THEME_ID]
    device_note = {
...
def _generate_reference_image_b64(design_brief: str, *, theme_id: str = "", device: str = "") -> Optional[str]:
    ...
    try:
        ...
        return None
    try:
        prompt = _build_reference_image_prompt(design_brief, theme_id=theme_id, device=device)
...
def generate_freeform_block(
    ...
    *,
    ...
    if use_reference_image and get_llm_config().supports_image_content_parts:
        reference_image_b64 = _generate_reference_image_b64(design_brief, theme_id=theme_id, device=device)
