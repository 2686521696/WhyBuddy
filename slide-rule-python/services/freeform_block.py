...
def _build_overview_sheet_prompt(
    ...
    *,
    ...
    ── 2026-07-30 精简：3452 → 2549 字，砍掉四类约束 ──────────────

    做法是减法实验：逐类拿掉、真出图、看哪个 bug 复发。结论按"这条约束在替
    模型做什么"分成两类——

    **砍掉的（替模型做它已经会做的事）**
      · 密度预算（字高下限 1.5%）    砍后字一点没糊
      · 控件形态（点名 antd 形态语言）砍后长相仍是 antd 那套
      · 信息层级清单（末尾正面清单）  砍后坐标轴/图例/单位/环比全在
      · 配色使用规则（_CARD_VISUAL_NOTE：卡片装饰/每卡一色/彩色放开/可读性）
                                     砍后配色照样跟着主题种子走

    **保留的（模型自己产生不了、或本能会做反的事）**
      · 版式要求        画布形状与卡/图排布，属于我们的产品决定
      · 技术标识禁令    砍后 blockRef 的 JSON 被原样当代码块画进图里
      · 占位写法        砍后编出 2,385 人 / ¥128,650 / 真人头像，还编得自洽
      · 主题色板（事实）砍后不同业务的出图配色**趋同**，各应用视觉身份消失；
                        而运行时外壳仍按种子渲染，参照图偏色会让内容区跟外壳撞色。
                        2026-07-30 扩到 5 题材 × 2 档位复测（种子依次是湛蓝/墨绿/
                        暖橙/深紫/砖红，摘掉配色段其余逐字节不动）：**10 张里 9 张
                        落到同一套拂晓蓝**，跟各自的种子色相毫无关系；唯一的异类
                        （健身房手机档）跑成青绿，同时把占位数字整套编成了自洽真值
                        （2,568 人 / 1,093+634+516+325 张，加起来正好对上）——
                        砍掉这条不只是丢配色，是模型整体脱离约束

    验证规模：5 个题材（健身房/采购审批/假勤/客服工单/员工入职）× 5 个差异
    明显的主题种子，四项检查（主题贯彻 / 数值占位 / 技术标识 / 枚举贴合）
    20/20 通过。

    ⚠️ **这个结论依赖当前生图模型的能力**。换模型或换生图端点之后，砍掉的
    四条可能重新变成必需——重测方法就是上面那张表：出一张图，逐项对照
    "复发长什么样"。别默认它一直成立。
    """
    hint = _theme_palette(theme_id, generated_theme)
    charts = "、".join(hint["charts"])
    ...
    if device == "desktop":
        zones_text = (
            # 2026-07-30：外壳这条**试过又撤回**。曾照第三方技能包那样把外壳件
            # 逐个点名（顶部导航 + 侧栏），出图确实是个"完整产品"、观感更整；
            # 但人工核对后决定不要壳——参照图的职责是交代**内容区**该长什么样，
            # 壳由运行时 AppRuntimeScreen 用 antd 固定渲染，画进参照图只会让
            # 设计 LLM 有样学样往内容树里塞导航。撤回后配套那句"外壳只是背景"
            # 也一并撤掉了（两处成对，见 generate_freeform_block）。
    ...
    return (
        ...
        # 2026-07-30 精简：这里原本有一段**密度预算**（"字高不得小于画布高度的
        # 1.5%，排不下就砍条目/砍列，宁可少画三分之一"）。减法实验里砍掉之后
        # 字一点没糊——当前模型自己就不会把中文压到看不清，这条属于"替模型做它
        # 已经会做的事"。同批砍掉的还有控件形态、信息层级清单、配色使用规则，
        # 理由相同，详见文件末尾 _build_overview_sheet_prompt 的取舍记录。
        f"{separator_line}"
        f"设计需求：{design_brief}。"
        ...
        f"大面积底色仍走浅色（贴近 {hint['contentBg']} 或纯白，保证内容读得清），"
        f"强调浅底可参考 {hint['accentBg']}；"
        # 色板只给**事实**（主色/背景/强调/图表分类色），不再给使用规则——
        # 规则那部分（_CARD_VISUAL_NOTE：卡片装饰、每卡一色、彩色放开、可读性
        # 底线）已在 2026-07-30 的减法实验里砍掉，5 个题材实测配色照样跟着
        # 主题种子走，且各应用视觉互不雷同。
        f"图表（折线/环形/柱状）里区分类别时用这几个颜色：{charts}，不要另配一套糖果色。"
        f"{_PLACEHOLDER_STYLE_NOTE}"
        "不要出现水印或品牌字样；"
        "画面撑满画布，四周不留空白底色。"
        # 2026-07-30 加回：这条在精简那轮被砍过（砍掉当轮出图的图例/坐标轴/
        # 单位都还在，看着不需要），但紧接着"再砍版式要求"那轮暴露了更隐蔽的
        # 失败模式——**漏内容**：采购那张少画了 brief 里明确要求的第 4 张 KPI
        # 卡，整块 blockRef 逐行表格也没了。这种失败很安静（参照图少一张卡，
        # 设计 LLM 就照着少设计一张，门禁不拦），而这条清单正是逐项点名"每张卡
        # 要有哪几层、每个图表要有哪几件"，是针对漏内容最直接的一道正面约束。
        #
        # 位置必须压在**所有禁令之后**：f7f14a9 记过，这段 prompt 的禁令堆多了
        # 会把密度要求冲掉，模型退化成"少画点最安全"；正面清单放最后才压得住。
        "最后一条，优先级最高：**信息层级必须画满**——每张 KPI 卡要有图标、"
        "标题、数值占位和环比/趋势那一行；每个图表要有标题、图例、坐标轴刻度"
        "标签和单位；每条列表项要有图标、主标题、一行次要信息和状态标签。"
        "**设计需求里点名的每一项都必须画出来，一项都不许漏**。这些标签一律"
        "写成看得见的占位文字（照上面那套按字段形状写的占位法），不许用灰色"
