...
# 三区参照板单独一档尺寸：这张图上要同时塞下桌面版式 + 手机版式 + 一堆
# 色板/图标/字号样例，1792x1024 那档不够用——字小到糊、图标挤成一坨。
#
# 活体探针（2026-07-29，逐个尺寸真发一轮）测出这个端点认的是一份**白名单**，
# 不是"任意宽高"：
#   3840x2160  ✅ 实际返回 3840x2160   ← 本档选它，真 4K 16:9
#   2048x2048  ✅  2048x1536 ✅  2048x1152 ✅  1536x1024 ✅
#   1792x1024  ✅ 但**实际只返回 1672x941**（被内部档位吃掉了）
#   2304x1296 / 2048x1024 / 1920x1080 / 2560x1440 / "auto"  ❌ HTTP 400
# 所以"传什么尺寸就出什么尺寸"这个假设不成立，改尺寸必须实测返回的 PNG 头。
_SHEET_IMAGE_SIZE = "3840x2160"
...
def _build_overview_sheet_prompt(
    ...
    *,
    ...
    return (
        "生成一张 3840x2160 的 UI 设计参照板（design style sheet），"
        ...
        "面端的总览版式：顶部一行指标卡、下方图表区，多列横向排布。不要画左侧"
        "侧边栏和顶栏（那部分另有画面）。\n"
        "② 「手机首页」——放画布**右侧**，画成一根 9:16 的竖条（就当它是一块 "
        "360x640 的手机屏），高度接近画布满高，外面套一个简洁的手机轮廓。内容是"
        "同一个总览改成手机端的**单列纵向**排布：指标卡一行一张、图表压扁。这一"
        "区要明显比桌面区窄得多——窄到一眼就知道是手机。\n"
        "③ 「样式风格」——不给它单独分栏，**哪里有空位就填哪里**（桌面区下方留"
        "出的横条、手机区下方剩的方块）：主色/辅色的色板色块、一组简洁线性图标"
        "（6-10 个）、按钮与标签与徽标的样例、两三级字号层级示意。这些样例没有"
        "固有形状，随空位横排竖排都行，但不许反过来挤占①②的比例。\n"
...
def _generate_overview_sheet_b64(
    ...
    *,
    ...
    尺寸走 _SHEET_IMAGE_SIZE（3840x2160），比单区块参照图那档大一截——这张
