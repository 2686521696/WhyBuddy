...
class ControlRunStore:
    ...
    def submit(self, session_id: str, owner_id: str, idempotency_key: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for _ in range(20):
            ...
            if not existing:
                now = _now()
                # ⚠ 2026-09-22 RFZYDAVHG9 的修法是在这里就写上租约（status=running、
                #   lease_expires_at=now+lease_seconds），让别的 worker 抢不走。
                #   2026-09-23 revert：list_runnable 只捞 lease_expires_at<=now，
                #   所以接单进程一崩，这一发就**冻整个租约时长**没人能接手——
                #   `claim(run_id, 另一个 worker, …)` 返回 None，崩溃恢复那几条
                #   判据就是死在这里。跨版本抢单是部署顺序的事。这里只插 queued。
