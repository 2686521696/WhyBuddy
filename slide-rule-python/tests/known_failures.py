...
KNOWN_FAILURES: Dict[str, str] = {
    ...
    # ── 页面骨架：导航项跟 spec 的页面清单对不上 ────────────────
    #
    #   ⚠ 2026-09-05 更正：原来这里写的是「五条同源，一个因」。**不是。**
    #     真机第 4 轮（汉字消除小游戏 sr-20260904214530）追下去发现是**两个**因，
    #     混在一起记成了一笔，于是谁来修都只能修掉一半然后困惑。
    #
    #   因一（已还）：**两侧口径不一致**。导航项是 `nav_tab_label()` 渲染的
    #     （剥产品名前缀、剥「某某页」的「页」），而 `check_shell_consistency`
    #     拿 **spec 原名** 去比——只要有一页名字以「页」结尾就必然报错，
    #     跟壳对不对得上无关。外加 `主页` 没进白名单，「挑战主页」被剥成
    #     「挑战主」。两处都修了，`test_零问题` 与 `test_统一后这道硬校验也过`
    #     随之转绿，已从本表移除。
    #
    #   因二（未还，且是个**产品问题**不是 bug）：下面这三条判据期望导航里
    #     出现 spec 原名（"工单详情页"），而产线**有意**剥掉那个「页」——
    #     2026-08-20 芸编智管：五项 × 390px，「页」单独折成第三行（见
    #     page_naming._strip_page_suffix 的注释）。但那条理由是**手机底栏**的，
    #     现在 `nav_tab_label` 在桌面侧栏上也无条件剥，而 `<aside class="w-64">`
    #     放得下「工单详情页」。
    #     所以要么「剥」只在手机上做（判据照现在这样期望原名），要么判据改成
    #     期望剥完的短名。**这是个视觉裁决，等人拍板，别顺手改判据了事。**
    "test_page_shell.py::Test统一之后干净::test_导航项恰好是_spec_的页面清单": (
        "因二：判据期望 spec 原名，产线有意剥「页」——剥不剥要看设备，等裁决"
    ),
    "test_page_shell.py::Test统一之后干净::test_每页高亮自己那一项": (
        "因二：按 spec 原名找不到那一项，高亮判定跟着落空"
    ),
    "test_page_shell.py::Test身份替换与导航重排的先后顺序::test_角色名正好是菜单项时_导航仍然重排": (
        "因二 + 重排后 p1 的导航没按 spec 名重排"
    ),
