...
（`control_client.py` 那个 `for attempt in range(1, 4)`）。而当时一个控制面
回合最多 8 次调用，于是**一回合最多 24 次重试**，网关抖一阵可以安静地烧很久。

⚠ 2026-09-23：这笔账的前提（一回合 8 轮）**已经不成立**。2026-09-17 / 09-18
  之后 `loop_budget.max_rounds` 是 10_000，同样的算法给出的是「一回合最多
  三万次重试」。下面那三道闸拦不住它的理由一条没变，只是数量级换了——
  这一层（跨轮的重试账本）因此更要紧，不是更不要紧。
