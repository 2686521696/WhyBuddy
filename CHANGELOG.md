...
这一轮的主线是「首页交给 AI 设计，而且两个档位各设计一版」。

**总览首页变成一整块 AI 设计，不再是「设计区 + 外挂卡」**

- 此前排行榜、最近动态这类逐行内容画不进 AI 设计的版式（取不到逐行记录，硬画只有
  表头没有行），只能被赶到设计之外单独渲染成外挂卡，首页于是长成「AI 设计区 +
  两张外挂卡」，主次和留白都由不得设计者。现在设计里可以放 `blockRef`——由 AI
  决定这块积木摆在哪一格、占多宽，渲染仍交给积木自己的真渲染器。
- 顺带修了总览页四处老毛病：同一指标画两遍、区块标题与页面标题重复、总览页上出现
  无意义的筛选条、卡片套卡片。
- KPI 卡从「一个大数字」补齐到「大数字 + 环比 + 卡底迷你走势线」三层，形状对标
  `ant-design/pro-components` 的 StatisticCard。几条不撒谎的边界：上一期为 0 时显示
  「较上期 —」而不是编一个 +∞；只有一个数据桶时整条走势线不出；变化小于 0.5% 直说
  「持平」；主数字算不出来时环比和走势线一并不挂。

**配色约束从「写在 prompt 里」变成「机械校验」**

色板要求此前只写在提示词里，真跑下来模型会一字不差地干它被警告过的事——橘色系
应用里主色一次没出现、蓝色占了六成、还自己发明了一套色板外的绿。现在加了一层机械
防线，在 OKLCh 色彩空间里判两条规则（色相是否落在色板色相 ±25° 内、主色用量是否
不低于任何其他色系），违规先带着具体偏差重问模型，重试耗尽再机械纠偏放行。配色问题
绝不抛错——抛了就回落固定骨架，那正是这条链路一直在治的病。

**手机档也拿到 AI 设计的首页**

- 此前只有 PC 首页是 AI 自主规划的，手机首页还是固定骨架把组件堆在最上方。现在手机
  档同样渲染 AI 设计的版式，固定骨架自动让位。
- 而且**两个档位各设计一版**，不是把桌面版式用 CSS 挤窄——桌面版式与手机版式分别
  生成、分别存储（借鉴 react-grid-layout 的 `layouts={{lg, md, sm}}` 与"本档没有就
  回退到更大一档"的取值规则）。实测桌面版式出了两排三列横排，手机版式一个三列横排
  都没有、通篇单列。
- 生成两版之前先出一张**参照板**：同一张图上并排画「样式风格 / 桌面首页 / 手机首页」
  三块，两个档位照着同一张图设计。分三张图生成的话三张各自随机，拼起来不像一个产品；
  一张板上并排画，模型自己会保证三块同色同调——这也正是设计行业 Style Tile 这个交付物
  的用法。版面按真实屏幕比例切：桌面区 16:9、手机区 9:16，样式风格去捡剩下的空位，
  因为宽高比本身就是这张图要传达的信息。

**其他**

- 门禁补上实体字段类型校验。此前非法字段类型（如 `file`）一路无人校验，前端对认不出
  的类型一律当普通文本——用户以为这儿能传附件、实际只能打字，不报错不提示测试全绿。
- 手机端底部 TabBar 补上行数徽标，与桌面侧栏同源；徽标走主题色而不是 antd-mobile 默认
  的红色（红色是"未读/紧急"的意思，而这里表达的是"本页有 12 行数据"）。
- 详情页分区改用 Collapse，日历换 antd Calendar，手机顶栏换 NavBar，全站补中文 locale。
- 动态流新增宽行档，多列对齐。
