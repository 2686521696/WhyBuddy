...
`;
  ...
  const hasPanelData = !!(
    panelData &&
    Array.isArray(panelData.positions) &&
    panelData.positions.length > 0
  );
  ...
  const conclusionLine = grounded
    ? hasPanelData
      ...
      : `结论：针对「${goalSlug}」，基于上游路线与证据聚合已形成可验证假设；多角色面板尚未执行，建议在 critique.generate 完成后再做最终收敛裁决。`
...
目标回顾：本轮针对「${goalSlug}」进行可行性 / 产品推演，已聚合上游证据${hasPanelData ? "、多角色立场" : ""}与风险信息，形成当前收敛基线。
...
分歧：
${dissentBlock || `（当前轮次意见基本收敛，无显著角色分歧。${hasPanelData ? `针对「${goalSlug}」的多角色讨论已就上游证据形成一致基线；如后续引入新 upstream 或外部证据，建议触发重入轮次以更新分歧评估与收敛决策。` : '本轮未执行多角色面板（critique.generate / synthesis.merge），分歧评估仅基于单路径上游。'}）`}
...
`;
  ...
  const convergenceNote = hasStale
    ? "含 stale 警示与分歧提示。"
    : hasPanelData
      ? "多角色收敛良好。"
      : "单路径推演（未触发多角色面板）。";
  // V5.2/V5.3 audit hardening: never claim positive multi-role convergence in summary
  // if there was no actual panel data (prevents self-contradictory reports that say both
  // "多角色收敛良好" and "未触发多角色面板").
  const honestConvergenceNote = hasPanelData
    ? "多角色收敛良好。"
    : "单路径推演（未触发多角色面板）。";
  const summary = `基于 ${upstreams.length} upstreams 的证据级推演报告（目标：${goalSlug}）。${honestConvergenceNote}${roleId ? ` 角色：${roleId}。` : ""}`;
