...
- **Design pages on a canvas** — a full page designer (component palette + drag-and-drop canvas + outline tree + schema-driven property panel + undo/redo) opens on any business page; data components bind only to fields that really exist in the model, and the designed tree renders back into the running app with live data and two real events (open page / start approval). Designs live in a local, resettable overlay — the rehearsed model itself is never mutated.
- **Orchestrate AI capabilities** — a React-Flow free canvas wires declared capabilities by data-model fields (illegal connections are rejected at connect time), with local-eval condition branches and an in-browser flow executor.
