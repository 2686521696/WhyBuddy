...
`SLIDERULE_SPEC_FIRST` 缺省 off，翻默认要有对照。先例摆在那儿：目录窄化攒够
3 覆盖域 × 2 臂 × n=6（p=0.00004）才翻默认，agentic pick 十话题 4:0 才转正。
这条链一轮 8~9 分钟、烧十几次 LLM，**没有对照就翻默认是拿生产当实验台**。
...
（默认关 / 失败留痕 / 探针 effective / 不引编排依赖）。所以这里直接调两个
