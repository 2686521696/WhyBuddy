# WhyBuddy V5.1 — 三刀 Spec:让推演图「枝繁叶茂 + 走到终点」
...
| 刀 | 名称 | 性质 | 改动面 | C(X) 一句话 |
|---|---|---|---|---|
| **Knife A′** | 结构拆解调度补全 | runtime 调度 | `shared/blueprint/whybuddy-pick-heuristic.ts` | 关键词覆盖不全 + 无健康 spec_tree 去重 → 部分拆解意图漏排或重复排；执行器路径分裂导致「排了但链质量不一致」 |
...
| **Knife C** | 终点交付月台 | 呈现层 | 虚拟终点节点 + `WhyBuddyReportReader` + md 导出 | clear 后报告只是被裁切的小卡片,无固定终点节点、无报告全文视图、无交付导出 → 用户"摸不到最终结果" |
...
| 能力 | 位置 | 状态 |
|---|---|---|
| pick 含 `structure.decompose` | `whybuddy-pick-heuristic.ts` L117–119(`树/拆解/spec tree`);L145–148(`openQCount>0`);S19 `pickStructureBeforeDelivery` | ✅ 已有 |
| S13 pick 绿灯测试 | `whybuddy-runtime.fullpath-structure.test.ts` | ✅ 已有 |
| 服务端 structure 全链 | `structure-exec-map.ts` + `whybuddy-structure-chain.ts` | ✅ 已有 |
| pilot/demo 模拟拆解 | `whybuddy-runtime.ts` `simulateCapabilityExecution` L1351+ | ✅ 模板树,非 LLM 链 |
...
| `latestTrustedReport` / `replayCoverage` | `whybuddy-delivery-chain.ts` / `whybuddy-fullpath-fixtures.ts` | ✅ |
| nl-command `ReportView` | `components/nl-command/ReportView.tsx` | ⚠️ 绑定 `ExecutionReport`,**不可**直接用于 WhyBuddy |
...
2. **「pickNext 永不排 structure.decompose」已过时** — 实现已迁至 `shared/blueprint/whybuddy-pick-heuristic.ts`,且 `whybuddy-runtime.fullpath-structure.test.ts` 对「拆解成 SPEC Tree」已绿灯。
...
- **¬C_A 场景**(无 v2 新增关键词命中、无重复排去重触发)下:picks 与改动前 deep-equal(对 `whybuddy-pick-heuristic.ts` 当前行为做 snapshot,而非虚构 `pickNextCapabilities_original`)。
...
- **A′.3** 修复:仅改 `whybuddy-pick-heuristic.ts` keyword 段 + `hasSpecTree` 去重;必要时抽 `hasHealthySpecTree(state)` 到 shared 供 S19 复用。
- **A′.4** 翻转 + `verify:whybuddy-v5` 全绿。
...
对任一已完成的能力执行,会话中**可还原**出过程信号(当轮 `UiTurn.steps` 或历史的 `capabilityRuns` + `artifacts.evidenceRefs` + `conversation` T_LEDGER 行);但 `deriveWhyBuddyReasoningViewModel` 对每个能力主节点 **只产出 1 个图节点**,过程与证据被折叠进卡片正文(`ReasoningFlowSurface` **5 行** `FLOW_MAX_LINES` clamp)。结果:N 个能力 ≈ N 个节点,图稀疏。
...
- **B.3** 修复:
  ...
  - `WhyBuddy.tsx` / `WhyBuddyTopHud`:详略开关;详模式时 `ReasoningFlowSurface` 对子节点可放宽 clamp(或仅主节点 5 行、子节点 3 行)
...
- **无报告全文视图** — WhyBuddy 产品页未接入 report 阅读器;
...
| — | ✅ 新建 `WhyBuddyReportReader`(或 `pages/whybuddy/ReportReaderPanel.tsx`),输入 `Artifact`(kind=report) |
...
3. **三个动作** — 【查看报告】【研究思路】【交付导出】
   - **查看报告** → `WhyBuddyReportReader` 抽屉/全屏;段尾 evidenceRef 点击 → `highlightedNodeIds` 回跳画布节点(B 的子节点 id 须稳定)
   - **研究思路** → 沿 `dependencyGraph` 高亮 report ← synthesis ← risk/counter ← evidence 链
   - **交付导出** — `deliveryPhase === "shipped"` 或 RV 通过后启用 → **md 下载**:`serializeWhyBuddyDeliveryMd(state)` = 报告全文 + `structureGateLedger`/`flowBoundaryLedger` 摘要 + 证据出处 + `replayCoverage(state)`
...
- **C.1** Task 1(必败):
  ...
  - `WhyBuddyReportReader` 渲染 ≥7 个命名段;点击 evidenceRef 产出 `highlightTargetNodeId`。
  - `not_recommended` goal → 无导出按钮。
- **C.2** Task 2(必过):clear 前无 terminal;RV 打回 vs challenge deep-equal;`serializeWhyBuddyDeliveryMd` 前后 STATE equal。
- **C.3** 修复:
  - `derive-terminal-node.ts`(或并入 view-model):虚拟终点节点 + `deriveTrustSeal`
  - `WhyBuddyReportReader` + `WhyBuddy.tsx` 抽屉/全屏接线
  - `serialize-whybuddy-delivery-md.ts`
  - `ReasoningFlowSurface`:终点不受 5 行 clamp / 视口锚定
- **C.4** 翻转 + S2/S19/S20 场景 + `verify:whybuddy-v5`。
...
每刀合并后 `verify:whybuddy-v5` 须全绿。探索测试用普通 `it` + 明确「当前必败」注释,**不要**误加到 `it.fails` 债务池(当前约 20 个,在 budget/invariants/artifact-health;三刀应只减不增)。
...
当前 `github-pages-whybuddy-demo.ts` 种子仅含 RBAC goal + risk + web evidence。三刀落地后建议 demo 种子扩展为:
