...
%%     **判据自己得先可信，否则量出来的全是废数。**
%% ·
%%   【⚑⚑E 还欠的两样（写在图上，免得当成没人看见）】
%%     · **接主轴**：七步现在是七个可独立调用的模块，v5_capability_executor
%%       那条主轴仍走老路（⛔1）。这是新链路唯一还没做的结构性工作。
%%     · **宿主安全那一层**：运行时解释器是对 DOM 的纯操作，Shadow DOM 隔离 +
%%       DOMPurify 由挂载它的那层负责，那层还没写。⚠ 两件事各有各的判据，
%%       合在一个函数里两边都测不干净——这是有意分工，不是漏了。
%% ·
