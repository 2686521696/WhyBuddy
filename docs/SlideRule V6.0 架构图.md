...
%% ·
%%   【⚑3 第 3、4 步被实测否决 —— 图上那句「实测」只对 HTML 成立，不对图成立】
%%     V5.9「每条边靠什么撑着」那张表的第一行写着：
%%         图挪到上游 → 内容更厚　**实测**：字段 25→37 · 区块 10→14 · 臆造 0
%%     ⚠ **那次实测的唯一变量不是「有没有图」，是「有没有 HTML」。** A→D 那一对
%%     证的是 HTML 当输入比纯意图强；至于那 12 个字段是归「有 HTML」还是归
%%     「HTML 从图来」，**从来没有分开量过**。
%%     08-13 补了对照（experiments/visual-first，n=3，唯一变量就是那张图）：
...
%%     四项指标**组间差全部小于组内极差的一半**（字段 +8%，而单臂自己波动 ±12；
%%     对照 A→D 那个真信号是 +48%）。稳定的只有三件事：
%%       ① V 路每轮多 **84 秒**（+51%），三轮 +74 / +78 / +89，一次例外都没有
%%       ② V 路区块数极差 10（11/21/13），T 路只有 3（16/16/19）——加了图产出更不稳
%%       ③ **图是有损压缩**：T 路 HTML 均值 70232 字符，V 路 41170，**丢掉 41%**
%%     第 ③ 条解释了前两条，也解释了 screenshot-to-code 的文字入口
%%     （build_text_prompt_messages）与 OpenUI **都是文字直出 HTML、没有一个插图
%%     这一跳**——他们大概早就撞过这堵墙。
%%     ⚠ 判据边界要写清楚，别过度外推：**这组数只量了「图对反推数据模型有没有用」**。
%%     图的另一个用途（给设计段当版式参照、定内容区/菜单区/Header 区的观感）
%%     **没测过**，这组数说不了话。所以 ⛔4 那张首页参照板**不在本轮否决范围内**，
%%     要砍它得另测一次判据不同的（版面丰富度 / 是否溢出画布 / 列表区有没有参照）。
%% ·
%%   【⚑4 八步收成六步】
%%     1  澄清需求 + 定位缺口 + 全网检索外部证据 + 决策讨论　　✅ 现成能力
%%     2  起草 SPEC（成功判据 / 需求节点 + 验收条件 / 页面清单）✅ 已落地
%%     3  SPEC 的每一页 ──→ HTML（**直出**，原 3+4+5 三步合成这一步）
%%     4  HTML → 实体 / 字段 / 关联关系 / 页面结构
%%     5  （第4步产物 + SPEC）→ 权限 / 工作流 / 不变式 / AIGC 能力
%%     6  汇合 → 五系统模型 → 结构闸 → 设计
%%     省下来的：每页 60~85 秒、N 倍的图钱、以及区块数那个 ±10 的抖动。
%%     ⚠ 第 5 步吃两个输入这条**没有变**，V5.9 那段论证仍然成立：
%%       少了第4步产物 → act2 那轮不变式引用了不存在的 reassign_work_order；
%%       少了 SPEC → 4 份 HTML 里角色/权限词 0 次、五组流程拓扑全同（5 节点 6 转移）。
%%       **是相加，不是替代。** 这一条依旧一次没测过，仍是假设。
%% ·
%%   【⚑5 新画一格：HTML 载体 / HTMLCARRIER —— 图上此前完全没有这个东西】
%%     六步方案有一个死穴，V5.9 上没有对应节点：**纯 HTML 只有视觉，不能读数据、
...
%% ·
%%   【⚑6 GEN5 的状态变了，但图上看不出来 —— 从「上游是空的」变成「有货没通路」】
...
%% ⚑ 符号: ⚑ = V6.0 修正（08-13 晚） ; ✅绿实线 landed = 曾是提案·现已落地 ; ✂灰虚线 rejected = 曾是提案·已被实测否决
...
subgraph CLOSURE["09 五系统闭环装配层 / Five-System Closure（▲ 07-17 新增子图 · app 主舞台的数据源）"]
  ...
  VISPROMPT["☐ 提案 · 视觉线① 出图提示词由 spec 反推<br/>⚠ 这是个**新函数**·今天不存在:现有 _build_overview_sheet_facts 的入参是<br/>(design_brief, datamodel)·两个都是**五系统模型的产物**——上游版本一个都拿不到<br/>要写的是 facts_from_spec(页面清单条目) → 出图提示词<br/>━━━━━━━━━━ ⚑ V6.0：**作为反推链路的一跳已被实测否决** ━━━━━━━━━━<br/>⚑ n=3 对照(唯一变量就是那张图)四项指标组间差全部小于组内极差的一半·量不出收益<br/>⚠ 否决的**只是它在反推链路上的位置**·不是这个函数本身:「spec → 出图提示词」<br/>　　作为**版式参照**用途仍是开放提案(那个用途一次没测过·见 ⛔4)"]:::propose
  VISIMG["☐ 提案 · 视觉线② 并发出图 N 张(每页一张)<br/>**代价明确**:墙钟仍 60~85s(并发)·但请求数从 1 变 N·这笔是净增·得认<br/>今天只出 1 张(仅落地页)·且那一张在下游·只能给已定的 stats/charts 化妆<br/>━━━━━━━━━━ ⚑ V6.0：**作为反推链路的一跳已被实测否决** ━━━━━━━━━━<br/>⚑ experiments/visual-first · n=3 · 唯一变量=有没有那张图:<br/>　　　　　　字段 T/V　　实体 T/V　　页面 T/V　　区块 T/V　　墙钟 T/V<br/>　　r1　　 60/71　　　6/7　　　　5/4　　　　16/13　　　165s/239s<br/>　　r2　　 54/64　　　6/7　　　　4/8　　　　16/21　　　175s/253s<br/>　　r3　　 66/59　　　7/6　　　　6/4　　　　19/11　　　160s/249s<br/>⚑ 稳定的只有三条:①每轮多 **84s**(+51%·三轮 +74/+78/+89 无一例外) <br/>　　②V 路区块数极差 10 而 T 路只有 3(加了图产出更不稳) <br/>　　③**图是有损压缩**:HTML 均值 70232 → 41170·**丢 41%**<br/>⚑ 第③条解释了前两条·也解释了 screenshot-to-code 的文字入口与 OpenUI<br/>　　**都是文字直出 HTML·没有一个插图这一跳**<br/>⚠ 判据边界:这组数**只量了「图对反推数据模型有没有用」**。图给设计段当版式参照<br/>　　那个用途**没测过**·所以 ⛔4 那张首页参照板不在本轮否决范围内——<br/>　　要砍它得另测一次判据不同的(版面丰富度/是否溢出画布/列表区有没有参照)"]:::propose
  VISHTML["☐ 提案 · 第5步 图 → HTML【已裁决】:screenshot-to-code 原生<br/>⚠ **这一步不能省**:D 组那个 25→37 是 **HTML** 喂出来的·不是图喂出来的——<br/>runner.py 的 run_d 原文「以下是这个产品的 N 份界面 HTML·是你唯一的依据」·<br/>load_html 的 docstring 写着「图转出来的 HTML·由 screenshot-to-code 原生跑出」<br/>跳过这一步等于把唯一有实测的那条边拆了<br/>⚠ 只让它画版式与绑定·**别让它写行为逻辑**:F 组让模型现写 JS·六项通过 12/18·<br/>三份里只有 1 份全过·且失败是静默的(能点·不报错·没反应)<br/>━━━━━━━━━━ ⚑ V6.0：**改成六步·第3步:spec 每一页 → HTML(直出)** ━━━━━━━━━━<br/>⚑ 原 3+4+5 三步合成这一步·**不再经图**(理由见 VISIMG 那格的 n=3)<br/>⚑ 25→37 那个信号仍然成立·但它证的是「**HTML 当输入**比纯意图强」·不是「图有用」<br/>⚠ 上面那条「这一步不能省」的原话仍然对·只是主语要换:不能省的是 **HTML**·不是图<br/>⚑ 「只画版式与绑定·别让它写行为逻辑」这条**不变**(F 组六项通过 12/18·失败静默)"]:::propose
  VISDERIVE["☐ 提案 · 第6步 HTML → 实体 / 字段 / 关联关系 / 页面结构<br/>**唯一有实测撑着的一条边**:同 55 字意图·唯一差别是有没有 HTML 当输入·<br/>字段 25→37 · 区块 10→14 · 臆造 0(A/D 两组同意图·这一对是干净的)<br/>⚠ 但 D 组跑在实验台里·用的是手工 HTML + 手工 spec——**没面对**结构闸 /<br/>316 区块目录 / 权限与工作流要求。生产链上没跑过<br/>⚑ V6.0:步号改为**六步方案第4步**(输入换成 spec 直出的 HTML·不再是图转的)"]:::propose
  HTMLCARRIER["⚑ V6.0 新增 · HTML 载体 / data-* 接线孔（**图上此前完全没有这个东西**）<br/>六步方案有个死穴:**纯 HTML 只有视觉·不能读数据·更不能写数据与动作**。<br/>用户 zip 里那四份 HTML 逐个 grep 过·`data-` 属性**一个都没有**·是纯视觉<br/>⚑ 答案不是留着自由树·是**让生成的 HTML 自己带接线孔**——而这件事 08-12 已经写完一半·<br/>随那批回退躺在 backup/2026-08-12-before-revert:<br/>　　services/overview_html.py　　data-fact 19 · data-field 26 · data-chart 11 · data-rows 31<br/>　　client/…/OverviewHtmlSurface.tsx　Shadow DOM 隔离 + DOMPurify(宿主安全)<br/>　　client/…/field-text.ts　　　　值格式化 / 空值显 —<br/>　　tests/test_overview_html.py · tests/test_overview_html_self_verify.py<br/>⚑ 纪律一句话:**HTML 只负责版式·一个数字都不写**。data- 是洞·运行时按 schema 填<br/>⚑ data-rows 那 31 处就是逐行展开·与自由树 rowsRef **同一套词汇**<br/>　　(entityRef / fieldRefs / sortByRef / order / limit)<br/>⚠ 缺口:**data-action 那半还没写**(对应自由树刚补的 actionRef)<br/>⚠ 措辞纪律:「抛弃自由树」的准确说法是**换载体·不是扔能力**。值钱的是那套绑定词汇·<br/>　　树只是它的一个载体;扔掉词汇就等于回到 zip 那种纯视觉 HTML——那是已经判定不行的东西"]:::propose
  SEMLINE["☐ 提案 · 第7步 (第6步产物 + SPEC) → 权限 / 工作流 / 不变式<br/>⚠ **两个输入都要·串行在第6步之后·不是跟它并列**(2026-08-13 用户裁决·<br/>推翻了本图上一版画的「语义线与视觉线并行」——那个画法是错的):<br/>· 第6步产物给「**挂在什么上**」:真实的 entityRef / fieldRef / pageRef<br/>· SPEC 给「**该有什么规则**」:哪几类角色 · 什么审批流 · 什么约束<br/>⚠ 少了第6步产物会怎样·今天就有现场证据(act2 那轮结构闸 findings=1):<br/>invariants[reassignment_preserves_audit_context].refs: invariant ref<br/>'reassign_work_order' not found in model——**不变式引用了一个不存在的东西**<br/>⚠ 少了 SPEC 会怎样·也是实测:4 份 HTML 里「角色/权限/主管/管理员」出现 **0 次**·<br/>「成交/流失/归档/阶段」**0 次**;五组推出来的流程拓扑完全相同(5 节点 6 转移)<br/>——那是模型的行业常识·不是从画面里读到的证据<br/>⚠ 纠正一个曾经的误译:实测说的是「**光有 HTML** 推不出权限/工作流」·<br/>不是「别看 HTML」。**是相加·不是替代**<br/>⚑ V6.0:步号改为**六步方案第5步**·两个输入这条论证**一个字没变**<br/>⚠ 「SPEC + 结构一起喂就推得出」仍然**一次没测过**·是整套方案里最后一条纯假设"]:::propose
...
%%  ⚑ V6.0：原 3+4+5（写提示词 → 生图 → 图转 HTML）被 n=3 实测否决，合成一步「spec → HTML 直出」。
%%  三条旧边保留成 -x（否决态），不删——删掉就没人知道它曾经被以为是必需的。
SPECREAL -.V6否决:图这一跳量不出收益·每轮多84s·HTML丢41%.-x VISPROMPT
VISPROMPT -.V6否决:仅指反推用途.-x VISIMG
VISIMG -.V6否决:图是有损压缩·HTML 70232→41170.-x VISHTML
SPECREAL ==>|"V6.0 六步·第3步：spec 的每一页 → HTML（直出·不经图）"| VISHTML
VISHTML ==>|"V6.0 六步·第4步"| VISDERIVE
VISHTML ==>|"V6.0 HTML 带 data-* 洞（版式归 HTML·数字一个不写）"| HTMLCARRIER
HTMLCARRIER -.V6运行时按schema填洞·页面这才能读能写·词汇同rowsRef.-> SAFEREND
%%  第7步两个输入：结构给「挂在什么上」，SPEC 给「该有什么规则」。缺任一个都塌，
%%  两侧各有实测（见 SEMLINE 节点标签里那两条）。
VISDERIVE ==>|"V6.0 六步·第5步输入①：结构先出来才有东西可挂"| SEMLINE
SPECREAL ==>|"V6.0 六步·第5步输入②：规则从这来·HTML 上一个字都没有"| SEMLINE
%%  第8步：两股产物汇进 GEN5。GEN5 本身不动，动的是喂给它什么——
%%  今天它只收一个 goal 字符串（⛔1），改后收结构 + 规则，签名与 prompt 装配要重写。
VISDERIVE ==>|"V6.0 六步·第6步：实体/字段/关联/页面结构"| GEN5
SEMLINE ==>|"V6.0 六步·第6步：权限/工作流/不变式"| GEN5
%%  ⚑ V6.0：这两条边是**今天唯一关着的门**。GEN5 签名仍只收 goal 字符串（⛔1），
%%  而 spec 已经造出来了——「上游是空的」已变成「有货没通路」。先开消费端。
SPECREAL -.V6有货没通路:spec已落地·但GEN5签名收不了它·⛔1仍在.-x GEN5
...
%% 符号: ⚑ = V6.0 修正(08-13 晚) ; ✅landed = 曾是提案·已落地 ; ✂ = 已被实测否决的边 ; ☐ = 08-13 提案(未实施·虚线红框) ; ⛔ = 08-13 标红(现状形状问题·实线粗红) ; ❖ = 08-11 升版 ; ✪ = 08-03 升版 ; ✧ = 07-31 升版 ; ✱ = 07-30 升版 ; ✦ = 07-26 审查修复升版 ; ★ = 07-24 体验层升版 ; ▲ = 07-17 升版 ; ■ = V5.3 新增/修订 ; ◆ = V5.2 新增/外环 ; ● = Ux 修订 ; 虚线 = 跨层或待补
...
%%   rejected（灰虚线）= 曾是提案·已被实测否决·边保留成 -x 不删
