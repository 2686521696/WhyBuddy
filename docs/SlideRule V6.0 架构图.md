...
%% ·
%%   【⚑3 第 3、4 步：**该轮对照实验作废，结论撤回** —— 原因是我把 V 路测残了】
%%     ⚠ 本节曾经写的是「第 3、4 步被实测否决」，并据此把八步改成六步、把三条边
%%     改成否决态。**那是错的，已全部回滚。** 记在这里不删，因为犯的错本身有价值。
%% ·
%%     跑过的数是真的（n=3，唯一变量是那张图）：
...
%%     但**这组数不能用来判第 3、4 步该不该存在**，两条独立的硬伤：
%% ·
%%     ① **V 路没有在它最好的状态下被测**（判据来自本实验自己的纪律，见
%%        runner.py:220 那段 docstring 原话：「用成熟工具跑，C 组才是在它的最好
%%        状态下被测——否则测出来的差可能只是我 prompt 没调好」）：
%%          T 路 —— 逐字抄 screenshot-to-code 的 create/text.py，是它的成熟状态 ✅
%%          V 路 —— img_hop.py:126 `image_prompt_from_text` 先让一个 LLM 把页面说明
%%                  改写成 400~800 字的出图提示词，**再**去生图 ❌
%%        那个改写跳正是本仓记录过会掉东西的地方：V5.7 ✧4「改写 LLM 漏掉哪一条，
%%        那一张图就会复发对应的老 bug」，✪ 段又记了它**确实复发了**（灰条）。
%%        **对 T 路照做了纪律、对 V 路没照做**，测出来的差分不清是「图没用」还是
%%        「我提示词没调好」。
%%     ② **判据量不到关键维度**：字段/实体/页面/区块数量的是**反推出来的数据模型
%%        有多厚**；而图真正该负责的是**版面结构的丰富度**——筛选栏分组、日历格里的
%%        状态徽标与冲突标签、右栏的检测/待办/提醒/周视图、底部列表。
%%        这四个数一个都碰不到那些东西。
%% ·
%%     【对照物：用户手上那份出了好效果的提示词（materials/previews/provenance.json）】
%%     它跟我 V 路那份的差别不是「更长」，是**确定性模板 vs LLM 改写**：
%%         为产品「<产品一句话>」的「<页面名>」这个页面，生成一张 Web 界面草样(UI mockup)。
%%         该页面要能体现：<acceptance 原文>。
%%         设计要点：<design notes>。
%%         要求：画出真实的页面布局（顶部导航 + 主操作区 + 列表/卡片/侧栏等），
%%         中文占位文案、只示意不写真实数据、右上角明显标注 PREVIEW。
%%         每个页面布局要各不相同。
%%     ⚠ 关键差异逐条：①产品一句话进 prompt（我的 page_brief 没有）②acceptance 原文
%%     直接进，不经改写 ③有 design notes ④固定的版式要求句 + PREVIEW 标记
%%     ⑤**「每个页面布局要各不相同」**——一句话按住通用后台网格 ⑥**中间没有 LLM 改写**。
%% ·
%%     【重测设计（未跑）】
%%       · V 路提示词换成上面这份**确定性模板**（spec 推演出来，去掉改写跳）
%%       · 出图分辨率 **2560x1440**
%%       · 口径优先抄 skills/sliderule（仓内现有 validate_spec_tree.py /
%%         check_previews_real.py / gate.py / check_content_quality.py 四个脚本；
%%         ⚠ 那份 image_settings.py 是**外部技能包**里的，仓里没有，别当它在）
%%       · 判据换成能量到版面的：区域/区块数 · 交互控件种类数 · 有没有列表区 ·
%%         是否溢出画布 —— 而不是继续数字段
%% ·
%%     【仍然成立、不受本次撤回影响的两条】
%%       · 25→37 那个信号证的是「**HTML 当输入**比纯意图强」——这条没变
%%       · 「图对反推数据模型有没有用」与「图对版面质量有没有用」是**两个问题**，
%%         本轮连第一个都没测准，第二个更是一次没测过
%% ·
%%   【⚑4 六步方案**撤回**，回到八步】
%%     ⚠ 六步是建立在 ⚑3 那个结论上的，⚑3 撤回，六步跟着撤回。八步原样保留：
%%     1  澄清需求 + 定位缺口 + 全网检索外部证据 + 决策讨论　　✅ 现成能力
%%     2  起草 SPEC（成功判据 / 需求节点 + 验收条件 / 页面清单）✅ 已落地
%%     3  按页面清单，逐页写出图提示词　　　　　　　　　☐ 待建（重测后定）
%%     4  并发生图 N 张（2560x1440）　　　　　　　　　　☐ 待建（重测后定）
%%     5  图 → HTML　　　　　　　　　　　　　　　　　　☐ 待建
%%     6  HTML → 实体 / 字段 / 关联关系 / 页面结构　　　☐ 待建
%%     7  （第6步产物 + SPEC）→ 权限 / 工作流 / 不变式　☐ 待建
%%     8  汇合 → 五系统模型 → 结构闸 → 设计　　　　　　☐ 入口是关的（⛔1）
%%     ⚠ 现场证据支持第 3、4 步存在：用户手上那份按 provenance 模板出的图，
%%     反推出来的页面（内容排期助手）版面明显更满——四组筛选 / 日历格状态徽标 /
%%     冲突与撞期标签 / 右栏检测+待办+提醒+周视图 / 底部列表。**这是观察不是实测**，
%%     所以第 3、4 步仍是 ☐ 提案，不是 ✅；但也不再是「已否决」。
...
subgraph CLOSURE["09 五系统闭环装配层 / Five-System Closure（▲ 07-17 新增子图 · app 主舞台的数据源）"]
  ...
  VISPROMPT["☐ 提案 · 视觉线① 出图提示词由 spec 反推<br/>⚠ 这是个**新函数**·今天不存在:现有 _build_overview_sheet_facts 的入参是<br/>(design_brief, datamodel)·两个都是**五系统模型的产物**——上游版本一个都拿不到<br/>要写的是 facts_from_spec(页面清单条目) → 出图提示词<br/>━━━━━━━━━━ ⚑ V6.0：**曾被判否决·已撤回** ━━━━━━━━━━<br/>⚠ 本格一度写着「已被实测否决」·**那是错的·已撤回**(理由见头部 ⚑3)<br/>⚑ 那轮 V 路先让一个 LLM 把页面说明改写成 400~800 字提示词再去生图——<br/>　　**而那个改写跳正是本仓记录过会掉东西的地方**(V5.7 ✧4 · ✪ 段记了它确实复发)。<br/>　　T 路照抄了 screenshot-to-code 的成熟实现·V 路却套了个已知会退化的两段式:<br/>　　**不对称的比较·测出来的差分不清是「图没用」还是「我提示词没调好」**<br/>⚑ 重测要写的形状(spec 推演·确定性模板·**不经 LLM 改写**):<br/>　　为产品「产品一句话」的「页面名」这个页面·生成一张 Web 界面草样(UI mockup)。<br/>　　该页面要能体现:acceptance 原文。设计要点:design notes。<br/>　　要求:画出真实的页面布局(顶部导航 + 主操作区 + 列表/卡片/侧栏等)·中文占位文案·<br/>　　只示意不写真实数据·右上角明显标注 PREVIEW。**每个页面布局要各不相同**。<br/>⚑ 最后那句是按住通用后台网格的关键·比任何禁令都管用(给的是分化要求·不是禁止)"]:::propose
  VISIMG["☐ 提案 · 视觉线② 并发出图 N 张(每页一张)<br/>**代价明确**:墙钟仍 60~85s(并发)·但请求数从 1 变 N·这笔是净增·得认<br/>今天只出 1 张(仅落地页)·且那一张在下游·只能给已定的 stats/charts 化妆<br/>━━━━━━━━━━ ⚑ V6.0：**曾被判否决·已撤回** ━━━━━━━━━━<br/>⚠ 数是真跑的·但**不能用来判这一步该不该存在**(两条硬伤见头部 ⚑3):<br/>　　①V 路提示词被削弱(多一个已知会退化的 LLM 改写跳)<br/>　　②判据量的是「反推数据模型有多厚」·量不到「版面结构有多丰富」——<br/>　　　而后者才是这张图该负责的东西<br/>⚑ 原始数据留档:experiments/visual-first · n=3 · 唯一变量=有没有那张图:<br/>　　　　　　字段 T/V　　实体 T/V　　页面 T/V　　区块 T/V　　墙钟 T/V<br/>　　r1　　 60/71　　　6/7　　　　5/4　　　　16/13　　　165s/239s<br/>　　r2　　 54/64　　　6/7　　　　4/8　　　　16/21　　　175s/253s<br/>　　r3　　 66/59　　　7/6　　　　6/4　　　　19/11　　　160s/249s<br/>⚑ 稳定的只有三条:①每轮多 **84s**(+51%·三轮 +74/+78/+89 无一例外) <br/>　　②V 路区块数极差 10 而 T 路只有 3(加了图产出更不稳) <br/>　　③**图是有损压缩**:HTML 均值 70232 → 41170·**丢 41%**<br/>⚑ 第③条解释了前两条·也解释了 screenshot-to-code 的文字入口与 OpenUI<br/>　　**都是文字直出 HTML·没有一个插图这一跳**<br/>⚑ **重测设计**(未跑):V 路换确定性模板(见 VISPROMPT 那格)·分辨率 **2560x1440**·<br/>　　口径优先抄 skills/sliderule(仓内四个脚本;⚠ image_settings.py 是外部技能包的·仓里没有)·<br/>　　判据换成能量到版面的:区域/区块数 · 交互控件种类数 · 有没有列表区 · 是否溢出画布<br/>⚑ **现场反证**:用户手上按 provenance 模板出的那份·反推出来的页面(内容排期助手)版面明显更满——<br/>　　四组筛选 / 日历格状态徽标 / 冲突与撞期标签 / 右栏检测+待办+提醒+周视图 / 底部列表。<br/>　　⚠ 这是**观察不是实测**·所以本格仍是 ☐ 提案·但不再是「已否决」"]:::propose
  VISHTML["☐ 提案 · 第5步 图 → HTML【已裁决】:screenshot-to-code 原生<br/>⚠ **这一步不能省**:D 组那个 25→37 是 **HTML** 喂出来的·不是图喂出来的——<br/>runner.py 的 run_d 原文「以下是这个产品的 N 份界面 HTML·是你唯一的依据」·<br/>load_html 的 docstring 写着「图转出来的 HTML·由 screenshot-to-code 原生跑出」<br/>跳过这一步等于把唯一有实测的那条边拆了<br/>⚠ 只让它画版式与绑定·**别让它写行为逻辑**:F 组让模型现写 JS·六项通过 12/18·<br/>三份里只有 1 份全过·且失败是静默的(能点·不报错·没反应)<br/>━━━━━━━━━━ ⚑ V6.0：步号回到**第5步**(六步方案已撤回) ━━━━━━━━━━<br/>⚠ 本格一度被改成「六步·第3步·spec 直出 HTML·不经图」·**已撤回**(理由见头部 ⚑3)<br/>⚑ 25→37 那个信号仍然成立·它证的是「**HTML 当输入**比纯意图强」——<br/>　　至于那 12 个字段归「有 HTML」还是归「HTML 从图来」·**到今天仍然没分开量准**<br/>⚑ 「只画版式与绑定·别让它写行为逻辑」这条**不变**(F 组六项通过 12/18·失败静默)"]:::propose
  VISDERIVE["☐ 提案 · 第6步 HTML → 实体 / 字段 / 关联关系 / 页面结构<br/>**唯一有实测撑着的一条边**:同 55 字意图·唯一差别是有没有 HTML 当输入·<br/>字段 25→37 · 区块 10→14 · 臆造 0(A/D 两组同意图·这一对是干净的)<br/>⚠ 但 D 组跑在实验台里·用的是手工 HTML + 手工 spec——**没面对**结构闸 /<br/>316 区块目录 / 权限与工作流要求。生产链上没跑过<br/>⚠ V6.0 一度把步号改成「六步第4步·输入换成 spec 直出的 HTML」·**已撤回**·仍是第6步"]:::propose
...
%%  ⚑ V6.0：这三条边一度被改成 -x（否决态），**已撤回**——那轮对照 V 路提示词被削弱、
%%  判据也量不到版面，结论不成立（详见头部 ⚑3）。八步链路原样恢复。
SPECREAL ==>|"☐ 第3步：按页面清单逐页"| VISPROMPT
VISPROMPT ==>|"☐ 第4步：并发生图 N 张（2560x1440）"| VISIMG
VISIMG ==>|"☐ 第5步：screenshot-to-code"| VISHTML
VISHTML ==>|"☐ 第6步"| VISDERIVE
VISHTML ==>|"⚑ V6.0 补:HTML 带 data-* 洞（版式归 HTML·数字一个不写）"| HTMLCARRIER
...
%%   rejected（灰虚线）= 预留给「曾是提案·已被实测否决」的格子。
%%     ⚠ **当前没有任何节点在用它**——V6.0 一度把第 3、4 步标成否决，后来撤回了。
%%     定义留着，是因为下次真有格子被测掉时该有个一致的画法；但**在没有节点用它之前，
%%     不许有人拿它去标一个「我觉得不行」的格子**——这个 class 的准入是实测，不是判断。
