...
- 扫描文件 **357** 个，模块 **357** 个
- 内部依赖边 **1192** 条（包含普通包初始化依赖）
- 内部 import 语句 **1103** 条，其中函数体内 **474** 条语句（基线 474，只许变少）
- 未声明的跨包依赖 **0** 条（基线 0 条）
...
| 层 | 模块数 | 可以依赖 | 是什么 |
|---|---|---|---|
| `util` | 156 | （谁都不依赖） | 纯工具：不依赖 services 里任何其它模块 |
...
```mermaid
flowchart TB
  util["util<br/>156 个模块<br/>纯工具：不依赖 services 里任何其它模块"]
...
flowchart TB
  ...
  services["services<br/>273 个模块<br/>业务"]
  ...
  routes -->|6| models
  routes -->|164 · 其中 77 条边来自函数体 import| services
