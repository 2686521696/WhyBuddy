...
- 扫描文件 **341** 个，模块 **341** 个
- 内部依赖边 **1132** 条（包含普通包初始化依赖）
- 内部 import 语句 **1044** 条，其中函数体内 **474** 条语句（基线 474，只许变少）
- 未声明的跨包依赖 **0** 条（基线 0 条）
...
| 层 | 模块数 | 可以依赖 | 是什么 |
|---|---|---|---|
| `util` | 146 | （谁都不依赖） | 纯工具：不依赖 services 里任何其它模块 |
...
```mermaid
flowchart TB
  util["util<br/>146 个模块<br/>纯工具：不依赖 services 里任何其它模块"]
...
flowchart TB
  ...
  services["services<br/>260 个模块<br/>业务"]
