...
- 扫描文件 **295** 个，模块 **295** 个
- 内部依赖边 **880** 条，其中 **476** 条写在函数体里（54%；基线 476，只许变少）
- 未声明的跨包依赖 **0** 条（基线 0 条）
- 模块级循环依赖 **0** 个（基线 0 个）
...
| 层 | 模块数 | 可以依赖 | 是什么 |
|---|---|---|---|
| `util` | 130 | （谁都不依赖） | 纯工具：不依赖 services 里任何其它模块 |
...
```mermaid
flowchart TB
  util["util<br/>130 个模块<br/>纯工具：不依赖 services 里任何其它模块"]
...
flowchart TB
  ...
  services["services<br/>221 个模块<br/>业务"]
