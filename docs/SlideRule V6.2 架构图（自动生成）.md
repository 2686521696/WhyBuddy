...
- 扫描文件 **324** 个，模块 **324** 个
- 内部依赖边 **1056** 条（包含普通包初始化依赖）
- 内部 import 语句 **968** 条，其中函数体内 **474** 条语句（基线 474，只许变少）
- 未声明的跨包依赖 **0** 条（基线 0 条）
...
| `flow` | 36 | util、core | 编排：驱动器 / 流水线 / 控制面 / 会话 |
...
flowchart TB
  ...
  flow["flow<br/>36 个模块<br/>编排：驱动器 / 流水线 / 控制面 / 会话"]
  core -->|148| util
  flow -->|115| core
  flow -->|132| util
```
...
flowchart TB
  ...
  services["services<br/>244 个模块<br/>业务"]
