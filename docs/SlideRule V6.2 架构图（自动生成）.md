...
- 扫描文件 **325** 个，模块 **325** 个
- 内部依赖边 **1062** 条（包含普通包初始化依赖）
- 内部 import 语句 **974** 条，其中函数体内 **474** 条语句（基线 474，只许变少）
- 未声明的跨包依赖 **0** 条（基线 0 条）
...
| `flow` | 37 | util、core | 编排：驱动器 / 流水线 / 控制面 / 会话 |
...
flowchart TB
  ...
  flow["flow<br/>37 个模块<br/>编排：驱动器 / 流水线 / 控制面 / 会话"]
  core -->|148| util
  flow -->|116| core
  flow -->|135| util
```
...
flowchart TB
  ...
  services["services<br/>245 个模块<br/>业务"]
