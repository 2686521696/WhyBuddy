...
- 最大模块：`xai-grok-pager`（446,079 source LOC，520,203 raw LOC）
...
| `xai-grok-workspace-types` |  | Wire types for the xAI workspace API (request/chunk/event enums shared by client and server) | 6,072 | 9,054 | 50 | 4 |
| `xai-grok-compaction` |  | Shared, transport-agnostic compaction engine for Grok chat and Grok Build. | 5,394 | 7,804 | 34 | 3 |
| `xai-fsnotify` |  | Local-filesystem event source: single causal stream of semantic FsEvents | 4,130 | 5,650 | 19 | 2 |
| `xai-tool-types` |  | Canonical tool-description types for the xAI platform | 3,348 | 4,368 | 7 | 12 |
| `xai-workflow` | 关键叶子 | Rhai-scripted dynamic workflow engine: scripts orchestrate agents through a host channel | 3,248 | 3,501 | 8 | 1 |
...
|---|---|---:|---:|---:|---:|---:|
| `xai-grok-pager` | xai-grok-pager | 446,079 | 520,203 | 604 | 2 | 36 |
| `xai-grok-shell` | Grok | 345,801 | 400,975 | 633 | 4 | 58 |
| `xai-grok-tools` | Grok tools library | 120,499 | 149,673 | 270 | 14 | 22 |
| `xai-grok-workspace` | Core host-local workspace library (FS, VCS, execution, discovery) for xai-grok-shell and remote sampler | 87,185 | 111,136 | 113 | 6 | 34 |
...
| `xai-fast-worktree` | High-performance git worktree creation using CoW cloning | 30,752 | 38,775 | 76 | 3 | 4 |
...
| `xai-grok-markdown` | Streaming markdown renderer for terminal UIs | 17,635 | 21,379 | 27 | 2 | 2 |
| `xai-grok-agent` | Agent builder, definition parsing, and system prompt assembly | 17,574 | 22,093 | 31 | 5 | 8 |
...
| `codegen` | 83 | 1,419,398 | 产品主干：agent、tools、shell、pager、workspace |
...
| `xai-grok-tools` | 14 | 22 | 120,499 | Grok tools library |
...
| `xai-grok-sandbox` | 8 | 2 | 7,184 | OS-level sandboxing for Grok Build using kernel primitives (Landlock/Seatbelt) via nono |

## 出度排行（谁在编排最多模块）

| crate | 出度 | 入度 | source LOC | 用途 |
|---|---:|---:|---:|---|
| `xai-grok-shell` | 58 | 4 | 345,801 | Grok |
| `xai-grok-pager` | 36 | 2 | 446,079 | xai-grok-pager |
| `xai-grok-workspace` | 34 | 6 | 87,185 | Core host-local workspace library (FS, VCS, execution, discovery) for xai-grok-shell and remote sampler |
| `xai-grok-tools` | 22 | 14 | 120,499 | Grok tools library |
...
| `xai-grok-agent` | 8 | 5 | 17,574 | Agent builder, definition parsing, and system prompt assembly |
...
| `xai-chat-state` | `codegen` | Actor-based chat state management for xAI agents | 12,013 | 14,573 | 0 | 18 | 1 | 4 | internal | `crates/codegen/xai-chat-state` |
| `xai-circuit-breaker` | `common` | Shared circuit breaker. | 1,736 | 2,335 | 0 | 19 | 2 | 0 | leaf | `crates/common/xai-circuit-breaker` |
...
| `xai-computer-hub-sdk` | `common` | SDK for the xAI Computer Hub: connection pool, transparent reconnect, tool harness, and tool-server runtime. | 14,765 | 18,559 | 0 | 23 | 6 | 5 | internal | `crates/common/xai-computer-hub-sdk` |
...
| `xai-fast-worktree` | `codegen` | High-performance git worktree creation using CoW cloning | 30,752 | 38,775 | 770 | 76 | 3 | 4 | internal | `crates/codegen/xai-fast-worktree` |
| `xai-file-utils` | `codegen` | Local data collection: upload queueing and blob storage | 11,616 | 13,777 | 0 | 11 | 7 | 7 | internal | `crates/codegen/xai-file-utils` |
| `xai-fsnotify` | `codegen` | Local-filesystem event source: single causal stream of semantic FsEvents | 4,130 | 5,650 | 49 | 19 | 2 | 0 | leaf | `crates/codegen/xai-fsnotify` |
...
| `xai-grok-agent` | `codegen` | Agent builder, definition parsing, and system prompt assembly | 17,574 | 22,093 | 0 | 31 | 5 | 8 | internal | `crates/codegen/xai-grok-agent` |
...
| `xai-grok-config` | `codegen` | Shared config loading for Grok — grok_home, effective config (requirements > user > managed), TOML merge | 10,470 | 12,435 | 0 | 30 | 21 | 5 | internal | `crates/codegen/xai-grok-config` |
| `xai-grok-config-types` | `codegen` | Leaf configuration value types for the grok CLI, extracted from xai-grok-shell for dependency inversion. | 3,447 | 4,137 | 0 | 8 | 4 | 3 | internal | `crates/codegen/xai-grok-config-types` |
| `xai-grok-dashboard-store` | `codegen` | SQLite-backed persistent dashboard workspace: membership, layout ranks, grouping | 2,857 | 3,403 | 45 | 13 | 1 | 2 | internal | `crates/codegen/xai-grok-dashboard-store` |
...
| `xai-grok-extra-ca` | `codegen` | TLS policy for the grok CLI: rustls backend pin, process crypto provider, shared root store, and opt-in GROK_EXTRA_CA_BUNDLE roots | 723 | 845 | 212 | 9 | 14 | 0 | leaf | `crates/codegen/xai-grok-extra-ca` |
| `xai-grok-feedback` | `codegen` | Shared feedback taxonomy, durable local draft storage, and the session trace archive builder for Grok Build | 2,330 | 2,623 | 0 | 11 | 3 | 0 | leaf | `crates/codegen/xai-grok-feedback` |
| `xai-grok-foreign-sessions` | `codegen` | Bounded, metadata-only discovery of foreign coding-agent sessions | 4,771 | 5,087 | 1,396 | 15 | 2 | 2 | internal | `crates/codegen/xai-grok-foreign-sessions` |
...
| `xai-grok-hooks` | `codegen` | Runtime hook system for Grok — file-based discovery, command execution, and policy enforcement | 10,773 | 12,464 | 743 | 15 | 4 | 4 | internal | `crates/codegen/xai-grok-hooks` |
...
| `xai-grok-image` | `codegen` | Image validation and transcoding shared by Grok tools and clients | 815 | 1,037 | 0 | 2 | 2 | 0 | leaf | `crates/codegen/xai-grok-image` |
| `xai-grok-login` | `codegen` | Authentication subsystem for the grok shell crate family: login flows, token refresh, credential providers, and auth storage. | 23,221 | 26,653 | 120 | 45 | 4 | 13 | internal | `crates/codegen/xai-grok-login` |
| `xai-grok-markdown` | `codegen` | Streaming markdown renderer for terminal UIs | 17,635 | 21,379 | 655 | 27 | 2 | 2 | internal | `crates/codegen/xai-grok-markdown` |
...
| `xai-grok-mcp` | `codegen` | MCP integration crate. Quarantines rmcp + reqwest 0.13 (rmcp 3.x requires reqwest >= 0.13.2 while the rest of the workspace uses reqwest 0.12) and owns the MCP credential store and OAuth flow orchestrator. | 11,761 | 14,078 | 382 | 18 | 3 | 11 | internal | `crates/codegen/xai-grok-mcp` |
| `xai-grok-memory` | `codegen` | Cross-session memory for Grok. | 12,642 | 15,283 | 0 | 25 | 2 | 7 | internal | `crates/codegen/xai-grok-memory` |
...
| `xai-grok-pager` | `codegen` | xai-grok-pager | 446,079 | 520,203 | 78,230 | 604 | 2 | 36 | internal | `crates/codegen/xai-grok-pager` |
...
| `xai-grok-pager-minimal` | `codegen` | Minimal (scrollback-native) render mode: `grok --minimal`. | 5,227 | 6,279 | 0 | 12 | 1 | 6 | internal | `crates/codegen/xai-grok-pager-minimal` |
...
| `xai-grok-pager-render` | `codegen` | 根据 crate 名称和目录推断 | 32,565 | 40,553 | 0 | 79 | 1 | 12 | internal | `crates/codegen/xai-grok-pager-render` |
...
| `xai-grok-sampler` | `codegen` | Actor-based sampling/inference layer for xAI grok (HTTP streaming + retry, no shell coupling) | 14,750 | 17,195 | 2,020 | 40 | 4 | 4 | internal | `crates/codegen/xai-grok-sampler` |
| `xai-grok-sampling-types` | `codegen` | Pure data types for the xAI sampling / chat-completion API layer | 11,544 | 13,963 | 202 | 16 | 6 | 3 | internal | `crates/codegen/xai-grok-sampling-types` |
| `xai-grok-sandbox` | `codegen` | OS-level sandboxing for Grok Build using kernel primitives (Landlock/Seatbelt) via nono | 7,184 | 9,353 | 1,309 | 22 | 8 | 2 | internal | `crates/codegen/xai-grok-sandbox` |
...
| `xai-grok-session-search` | `codegen` | SQLite FTS5 index over local grok sessions: lease-guarded bootstrap, debounced incremental upserts, and BM25 ranked query | 3,126 | 3,806 | 0 | 10 | 1 | 3 | internal | `crates/codegen/xai-grok-session-search` |
| `xai-grok-shared` | `codegen` | Shared utilities used by both `xai-grok-shell` and its downstream clients (e.g. `xai-grok-pager-render`). | 3,758 | 4,954 | 0 | 9 | 3 | 7 | internal | `crates/codegen/xai-grok-shared` |
| `xai-grok-shell` | `codegen` | Grok | 345,801 | 400,975 | 16,803 | 633 | 4 | 58 | internal | `crates/codegen/xai-grok-shell` |
| `xai-grok-shell-base` | `codegen` | Foundation modules for the grok shell crate family: environment presets, CPU profiling, and process/filesystem utilities. | 2,597 | 3,176 | 0 | 13 | 2 | 7 | internal | `crates/codegen/xai-grok-shell-base` |
...
| `xai-grok-shell-terminal` | `codegen` | Local, ACP, and PTY terminal runners extracted from xai-grok-shell so they compile in parallel. | 5,246 | 6,198 | 0 | 11 | 1 | 7 | internal | `crates/codegen/xai-grok-shell-terminal` |
| `xai-grok-status-line` | `codegen` | The status-line contract: the `[ui.status_line]` config a user writes and the payload the agent sends clients. | 925 | 1,107 | 0 | 6 | 3 | 0 | leaf | `crates/codegen/xai-grok-status-line` |
| `xai-grok-subagent-resolution` | `codegen` | Shared subagent definition, runtime, prompt, and resume resolution | 2,344 | 2,890 | 0 | 7 | 1 | 4 | internal | `crates/codegen/xai-grok-subagent-resolution` |
| `xai-grok-telemetry` | `codegen` | Telemetry engine: product events + Mixpanel emission + Sentry error reporting for Grok Build sessions | 20,481 | 24,059 | 2,156 | 59 | 10 | 14 | internal | `crates/codegen/xai-grok-telemetry` |
...
| `xai-grok-tools` | `codegen` | Grok tools library | 120,499 | 149,673 | 2,282 | 270 | 14 | 22 | internal | `crates/codegen/xai-grok-tools` |
...
| `xai-grok-update` | `codegen` | 根据 crate 名称和目录推断 | 8,667 | 10,929 | 3,763 | 16 | 2 | 7 | internal | `crates/codegen/xai-grok-update` |
...
| `xai-grok-workspace` | `codegen` | Core host-local workspace library (FS, VCS, execution, discovery) for xai-grok-shell and remote sampler | 87,185 | 111,136 | 191 | 113 | 6 | 34 | internal | `crates/codegen/xai-grok-workspace` |
...
| `xai-grok-workspace-types` | `codegen` | Wire types for the xAI workspace API (request/chunk/event enums shared by client and server) | 6,072 | 9,054 | 53 | 50 | 4 | 0 | leaf | `crates/codegen/xai-grok-workspace-types` |
...
| `xai-hunk-tracker` | `codegen` | Track file hunks (diffs) with agent/external attribution | 8,638 | 12,027 | 0 | 17 | 2 | 1 | internal | `crates/codegen/xai-hunk-tracker` |
...
| `xai-message-delivery-core` | `common` | Source-typed message delivery values and operation authorization. | 921 | 1,056 | 0 | 8 | 2 | 0 | leaf | `crates/common/xai-message-delivery-core` |
...
| `xai-ratatui-inline` | `codegen` | ratatui-inline | 2,543 | 3,375 | 181 | 10 | 3 | 0 | leaf | `crates/codegen/xai-ratatui-inline` |
| `xai-ratatui-textarea` | `codegen` | 根据 crate 名称和目录推断 | 11,674 | 14,746 | 0 | 14 | 3 | 0 | leaf | `crates/codegen/xai-ratatui-textarea` |
...
| `xai-tty-utils` | `codegen` | Lightweight process-spawning utilities for TTY safety — detach from controlling terminal, suppress interactive pagers, process-group lifecycle | 3,116 | 4,195 | 0 | 12 | 19 | 0 | leaf | `crates/codegen/xai-tty-utils` |
| `xai-workflow` | `codegen` | Rhai-scripted dynamic workflow engine: scripts orchestrate agents through a host channel | 3,248 | 3,501 | 0 | 8 | 1 | 0 | leaf | `crates/codegen/xai-workflow` |
