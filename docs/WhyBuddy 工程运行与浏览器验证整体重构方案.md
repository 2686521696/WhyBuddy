...
日期：2026-09-11。状态：待实施的工程方案；本次交付是方案文档，不表示下述新模块已经实现。
...
建议在对应阶段新增 `smoke:project-runtime`、`smoke:project-browser`、`project:contracts:check` 命令；**这些命令目前未创建**。烟测报告记录源码版本、模板/依赖、运行 ID、步骤、原始日志和证据文件，输出明确通过/失败/缺条件。
