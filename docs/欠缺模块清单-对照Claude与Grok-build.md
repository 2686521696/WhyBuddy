...
⚠ **2026-08-30 订正**：下表是 2026-08-29 动手前的快照，其中 grok「347 条边 / 边上写着为什么」量错了——`wc` 分批 total 被加过、注释在依赖行上方不是行内。重测与当前对照见 `docs/WhyBuddy与Grok-build架构对照.md`。一等 crate 92、一等内部边 351，带注释的内部边 17%。
...
§18 收工时模块级的账全清零了，但那时说过一句实话：**粒度对不上，所以不可比**。
    ...
    grok      一等 crate / 边以对照文档为准（2026-08-30：92 / 351）
...
组间 **103 条边**，与 grok 的内部边同量纲了（他们 crate 更多；2026-08-30 重测一等 351）。
...
grok 那边没有"覆盖率"这个概念：一个 cargo workspace 全包（2026-08-30：一等 92 crate / 351 边），
全包。要抄的正是那个"全包"。
