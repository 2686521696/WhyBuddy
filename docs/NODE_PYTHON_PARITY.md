...
| executor 执行面（index.ts POST /api/executor/events 回调决策 + tasks.ts 派发/取消决策） | 🟡 **事件→动作投影已接 Python，`EXECUTOR_EVENTS_PYTHON_PROJECTION` 默认开**（2026-07，`python-thin-proxy.ts` 语义：显式 "false" 关闭，vitest 环境默认走 Node 内联）：仅 **STATE-CHANGING 事件**（job.started/progress/waiting/completed/failed/cancelled + status 兜底 waiting/completed/failed/cancelled）经 `server/routes/executor-events-python-projection.ts` 委托 Python `/api/executor/events/project` 取动作决策，再由 Node 用**原有 runtime 调用**落地（markMissionRunning/waitOnMission/finishMission/failMission/cancelMission + 心跳清除）；**高频流事件（job.log/job.log_stream/job.screenshot）100% 内联**，Socket.IO 透传不增加 HTTP hop；Python 不可达/5xx/封套非法时逐事件回退内联映射，行为与关闭开关逐字节一致。**切片2（2026-07）**：① **dedup 落地已接线，`EXECUTOR_EVENTS_DEDUP_ENFORCED` 默认关**（仅显式 "true" 开启，vitest/生产同为关——跳过 duplicate/outOfOrder 投递是相对现状的行为变更，历史内联路径从不查 delivery；开启且投影委托生效时，Python duplicate/out_of_order 裁决使 Node 跳过 runtime 落地并 console.warn 记录，伪造终态重放被拦截；关闭时逐字节不变）；② **artifacts 归一化改为消费 Python 投影返回的 `artifacts`**（投影委托生效且 Python 声明了该决策——投递携带数组且每条校验通过——时采用，空列表按 Node 语义折叠为 undefined 保留 current.artifacts；字段缺失/任一条目非法则整体回退 Node 本地 normalizeExecutorArtifacts，Node 侧解析层再重验 `..` 遍历守卫）；③ **派发/取消决策面已接线，`EXECUTOR_DISPATCH_PYTHON_DECISIONS` 默认关**（派发系金链路：先接线、默认翻转后议；显式 "true" 开启：dispatchMissionToExecutor 的 requestId/idempotencyKey 派生、jobPayload 补丁（applyMissionDispatchPayload）、callbackUrl 透传经 `server/routes/executor-dispatch-python-decisions.ts` 委托 Python `/api/executor/dispatch/plan`；/:id/cancel 的 alreadyFinal 短路、reason/requestedBy/source 归一、转发裁决、cancelUrl/requestBody 及 downstream 结果裁决（404 容忍/非404→502/2xx→executorForwarded）委托 `/api/executor/cancel/decision`；封套非法或 Python 不可达逐请求回退内联同构推导）。**仍 Node-owned**：HMAC 签名校验（EXECUTOR_CALLBACK_SECRET）、heartbeatMonitor 重置/清除执行、missionRuntime 持久化写入、instance/securitySummary/previewSession 归一化、blueprint 前缀 202 短路、Socket.IO emit、buildExecutionPlan（LLM 规划）、ExecutorClient HTTP 传输/重试/能力探测、traceId 生成（randomUUID）、取消下游 fetch 与 503 传输错误、lifecycle/store/scheduler 咨询调用、lobster-executor runner | **`services/executor_event_projection.py`**：`mapExecutorEventToAction`（server/core/executor-event-mapper.ts）+ `resolveExecutorCallbackRouting`/`isBlueprintExecutorMissionId`（executor-callback-routing.ts）+ index.ts 内联分支 apply plan（有效进度/detail 回退链/分支顺序/心跳复位与清除）的逐行移植；切片2 增补：`normalize_executor_artifacts`（index.ts normalizeExecutorArtifacts 逐行移植 + artifact-utils.ts `validateArtifactPath` 遍历守卫**前移到摄入期**（Node 仅在读取内容时校验，摄入即拒绝含 `..` 路径的条目，端到端不弱于 Node；previewType 白名单校验，Node 系透传）与封套一等 `dedup` 字段（`resolve_executor_delivery_dedup`）；`routes/executor_events.py` **已挂载 /api/executor/events**（X-Internal-Key）：POST `/project`（畸形投递 fail-closed 400 封套，`source:"python"` + `provenance:"python-executor-event-projection"`，投递含 artifacts 数组时封套带归一化 `artifacts`）。**`services/executor_dispatch_decisions.py`**（切片2）：dispatchMissionToExecutor 纯决策逐行移植（sourceText 派生、buildExecutionPlan 输入、LOBSTER_EXECUTION_MODE 解析、applyMissionDispatchPayload mock/real 双分支、requestId/idempotencyKey `mission_<id>_attempt_<n>`/`mission:<id>:attempt:<n>`、buildServerBaseUrl + EXECUTOR_API_ROUTES.events 回调 URL 组装、无 job 失败决策）+ cancel 决策（FINAL 短路、normalizeCancelSource/toExecutorCancelSource 表、jobId trim 转发裁决、buildExecutorUrl + encodeURIComponent cancelUrl、requestBody undefined 字段丢弃、downstream 裁决——含 Node 终态检查恒 true 的怪癖如实移植并暴露 downstreamTerminal）；`routes/executor_dispatch.py` **已挂载 /api/executor**：POST `/dispatch/plan`、POST `/cancel/decision`（fail-closed 400，`provenance:"python-executor-dispatch-decisions"`）。注意两套决策面并存且如实移植：纯 mapper 含 dedup（duplicate/outOfOrder → duplicate 裁决），内联 apply plan 与 Node 一致**不查** delivery（dedup 落地由 `EXECUTOR_EVENTS_DEDUP_ENFORCED` 单独门控） | 迁移到Python（事件→动作投影已完成且默认开；~~① dedup/乱序裁决接入落地路径 ② artifacts 归一化 ④ 派发/取消决策面~~ 2026-07 切片2 已完成——per-flag 默认值：`EXECUTOR_EVENTS_PYTHON_PROJECTION` 默认**开**、`EXECUTOR_EVENTS_DEDUP_ENFORCED` 默认**关**（行为变更，翻转后议）、`EXECUTOR_DISPATCH_PYTHON_DECISIONS` 默认**关**（金链路，翻转后议）；下一步切片：③ blueprint 回调分支；再往后：HMAC/传输面与默认翻转决策） | 测试：test_executor_event_projection.py（46 例）+ test_executor_events_http_surface.py（9 例）+ test_executor_dispatch_decisions.py（29 例）+ test_executor_dispatch_http_surface.py（9 例）；Node 侧 executor-events.python-projection.test.ts（24 例，含伪造终态重放拦截/关闭开关不变）+ executor-dispatch.python-decisions.test.ts（10 例）；tasks.python-proxy / mission-cancel / executor-callback-* / execution-bridge* / task-artifact-routes 既有套件回归通过（134 例）；python 全量 1525 passed |
