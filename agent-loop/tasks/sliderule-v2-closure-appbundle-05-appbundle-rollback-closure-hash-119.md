...
Reviewed and landed as part of the AppBundle runtime closure batch. `compareAppBundleRollbackTargetSnapshotsByClosureHash` compares rollback target snapshots by closure hash and fails closed with `APPBUNDLE_ROLLBACK_UNPINNED` when hash/pins are missing.
