...
- `agent-loop/tasks/sliderule-v2-closure-report-01-sliderule-delivery-md-publish-closure-119.md`
...
This lands explicit AppBundle publish/runtime closure status and top-blocker text into SlideRule delivery markdown. The serializer reuses the shared final-report formatter so closed and blocked report output stay aligned with queue final report semantics while preserving the existing fail-closed absent-evidence path.
